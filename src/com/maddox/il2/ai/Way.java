...
public class Way {
    ...
    public Way(Way way) {
        this.WList = new ArrayList();
        this.prevdist2 = 1.0E8;
        this.prevdistToNextWP2 = Float.MAX_VALUE;//3.4028234663852886E38;
        this.set(way);
    }
    ...
    public void set(Way way) {
        this.WList.clear();
        this.Cur = 0;
        for (int waypointIndex = 0; waypointIndex < way.WList.size(); waypointIndex++) {
            WayPoint waypoint = new WayPoint();
            waypoint.set(way.get(waypointIndex));
            this.WList.add(waypoint);
            if (waypoint.Action == WayPoint.LANDING && waypoint.sTarget != null) this.landingOnShip = true;
        }
        this.landing = way.landing;
        this.landingAirport = way.landingAirport;
        if (this.takeoffAirport == null) this.takeoffAirport = way.takeoffAirport;
    }
    ...
    public void setLanding2(boolean landing2)
    {
...
//				waypoint.Action = 0;
            ...
            if (s1.startsWith("NORMFLY")) {
                waypoint.Action = WayPoint.NORMFLY;
                if (s1.endsWith("_401")) waypoint.waypointType = WayPoint.WP_NORMFLY_CAP_TRIANGLE;
                else if (s1.endsWith("_402")) waypoint.waypointType = WayPoint.WP_NORMFLY_CAP_SQUARE;
                else if (s1.endsWith("_403")) waypoint.waypointType = WayPoint.WP_NORMFLY_CAP_PENTAGON;
                else if (s1.endsWith("_404")) waypoint.waypointType = WayPoint.WP_NORMFLY_CAP_HEXAGON;
                else if (s1.endsWith("_405")) waypoint.waypointType = WayPoint.WP_NORMFLY_CAP_RANDOM;
                else if (s1.endsWith("_407")) waypoint.waypointType = WayPoint.WP_NORMFLY_ART_SPOT;
            } else if (s1.startsWith("TAKEOFF")) {
                waypoint.Action = WayPoint.TAKEOFF;
                if (s1.endsWith("_001")) waypoint.waypointType = WayPoint.WP_TAKEOFF_NORMAL_DELAY;
                else if (s1.endsWith("_002")) waypoint.waypointType = WayPoint.WP_TAKEOFF_PAIRS;
                else if (s1.endsWith("_003")) waypoint.waypointType = WayPoint.WP_TAKEOFF_LINE;
                else if (s1.endsWith("_004")) waypoint.waypointType = WayPoint.WP_TAKEOFF_TAXI;
                else if (s1.endsWith("_005")) waypoint.waypointType = WayPoint.WP_TAKEOFF_TAXI_FSP;
            } else if (s1.startsWith("LANDING")) {
                waypoint.Action = WayPoint.LANDING;
                if (s1.endsWith("_101")) waypoint.waypointType = WayPoint.WP_LANDING_RIGHT;
                else if (s1.endsWith("_102")) waypoint.waypointType = WayPoint.WP_LANDING_SHORTLEFT;
                else if (s1.endsWith("_103")) waypoint.waypointType = WayPoint.WP_LANDING_SHORTRIGHT;
                else if (s1.endsWith("_104")) waypoint.waypointType = WayPoint.WP_LANDING_STRAIGHTIN;
            ...
            if (s2 != null) if (s2.equals("&0")) {
                ...
                waypoint.setTargetWaypointIndex(s3);
            }
