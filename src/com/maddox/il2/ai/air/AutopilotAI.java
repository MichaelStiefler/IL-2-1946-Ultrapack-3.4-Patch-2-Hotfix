...
public class AutopilotAI extends Autopilotage {
    ...
    public void update(float f) {
        ...
        // ---
//        System.out.println("AutopilotAI update(" + f + "), actor name=" + this.FM.actor.name() + ", bWayPoint=" + this.bWayPoint + ", aircIndex=" + ((Aircraft) this.FM.actor).aircIndex());
        do {
            if (this.bWayPoint) {
//                Point3d LdLoc = new Point3d(PlLoc);
//                if (this.FM.Group != null && this.FM.Group.airc != null && this.FM.Group.airc[0] != this.FM.actor) {
//                    System.out.println(this.FM.actor.name() + " LdLoc set!");
//                    this.FM.Group.airc[0].FM.getLoc(LdLoc);
//                }
                if (this.WWPoint != this.way.auto(PlLoc) || this.way.isReached(PlLoc)) {
                    this.WWPoint = this.way.auto(PlLoc);
//                if (this.WWPoint != this.way.auto(LdLoc) || this.way.isReached(LdLoc)) {
//                    this.WWPoint = this.way.auto(LdLoc);
                    this.WWPoint.getP(this.WPoint);
                    // TODO: +++ Backport from 4.13
...
//                        this.voiceCommand(this.WPoint, LdLoc);
                        byte formationType = FM.formationType = (byte)way.curr().formation;
                        if(FM.Group != null) {
                            FM.Group.setFormationAndScale(formationType, 1.0F, true);
                        }
                    } //else System.out.println("AutopilotAI update aircIndex=" + ((Aircraft) this.FM.actor).aircIndex());
                    // ---
                    this.StabSpeed = this.WWPoint.Speed - 2.0F * ((Aircraft) this.FM.actor).aircIndex();
                    this.StabAltitude = this.WPoint.z;
                    if (this.WWPoint.Action == WayPoint.GATTACK) {
                        Actor groundTarget = this.WWPoint.getTarget();
                        if (groundTarget != null) this.FM.target_ground = null;
                        else if (((Aircraft)this.FM.actor instanceof TypeBomber || MistelTools.isDockedQueen(this.FM.actor)) && this.FM.CT.Weapons[3] != null && this.FM.CT.Weapons[3][0] != null && this.FM.CT.Weapons[3][this.FM.CT.Weapons[3].length - 1].haveBullets()) {
...
//                        if ((Aircraft) this.FM.actor instanceof TypeBomber) {
                        if ((Aircraft) this.FM.actor instanceof TypeBomber || MistelTools.isDockedQueen(this.FM.actor) || FM.CT.bHasBayDoorControl) {
                        // ---
                        ...
                        Actor curTarget = this.WWPoint.getTarget();
                        if (curTarget instanceof Aircraft) if (curTarget.getArmy() == this.FM.actor.getArmy()) this.FM.airClient = ((Aircraft) curTarget).FM;
                        else this.FM.target = ((Aircraft) curTarget).FM;
                    }
...
//                            }
                        ...
                        if (this.way.Cur() == 6 || this.way.Cur() == 7) {
                            int landingFeedback = 0;
                            if (Actor.isAlive(this.way.landingAirport)) landingFeedback = this.way.landingAirport.landingFeedback(this.WPoint, (Aircraft) this.FM.actor);
                            if (landingFeedback == Airport.LANDING_PERMITTED) {
//                                if (Main.cur().mission != null) {
//                                    /* empty */
//                                }
                                // TODO: Altered by |ZUTI|: fixed variables references
                            ...
                            if (landingFeedback == Airport.LANDING_DENIED) {
//                                if (Main.cur().mission != null) {
//                                    /* empty */
//                                }
                                // TODO: Altered by |ZUTI|: fixed variables references
...
//                                }
                            ...
                            if (landingFeedback == Airport.LANDING_WAVEOFF) {
//                                if (Main.cur().mission != null) {
//                                    /* empty */
//                                }
                                // TODO: Altered by |ZUTI|: fixed variables references
...
//                                    }
                ...
                if ((this.way.Cur() == this.way.size() - 1 && this.getWayPointDistance() < 2000.0F && this.way.curr().getTarget() == null && this.FM.M.fuel < 0.2F * this.FM.M.maxFuel || this.way.curr().Action == 2) && !this.way.isLanding()) {
                    ...
                    this.FM.set_task(Maneuver.FLY_WAYPOINT);
                    this.FM.set_maneuver(Maneuver.EMERGENCY_LANDING);
...
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("d_5_ no1=" + d_5_);
            if (d_5_ > 1.0) d_5_ = 1.0;
            if (d_5_ < 0.0) d_5_ = 0.0;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("d_5_ no2=" + d_5_);
            this.Ev -= d_5_ * d_1_ + (1.0 - d_5_) * d_2_;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("Ev no1=" + Ev);
            this.Ev += 1.0 * this.FM.getW().y + 0.5 * this.FM.getAW().y;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("Ev no2=" + Ev);
            if (this.FM.getSpeed() < 1.3F * this.FM.VminFLAPS) this.Ev -= 0.0040F * f;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("Ev no3=" + Ev);
            float f_6_ = 9.0F * this.FM.getSpeed() / this.FM.VminFLAPS;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("f_6_ no1=" + f_6_);
            if (this.FM.VminFLAPS < 28.0F) f_6_ = 10.0F;
            if (f_6_ > 25.0F) f_6_ = 25.0F;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("f_6_ no2=" + f_6_);
            float f_7_ = (f_6_ - this.FM.Or.getTangage()) * 0.1F;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("f_7_=" + f_7_);
            float f_8_ = -15.0F + this.FM.M.mass * aEARAF;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("f_8_ no1=" + f_8_);

            if (f_8_ < -4.0F) f_8_ = -4.0F;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("f_8_ no2=" + f_8_);
            float f_9_ = (f_8_ - this.FM.Or.getTangage()) * 0.2F;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("f_9_=" + f_9_);
            if (this.Ev > f_7_) this.Ev = f_7_;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("Ev no4=" + Ev);
            if (this.Ev < f_9_) this.Ev = f_9_;
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) System.out.println("Ev no5=" + Ev);
...
//            if (this.FM.Group.numInGroup((Aircraft)this.FM.actor) == 0) {
//                HUD.training("SA=" + df.format(this.SA) + ", Alt=" + df.format(this.FM.getAltitude()) + ", EC=" + df.format(this.FM.CT.ElevatorControl) + ", Ev=" + df.format(this.Ev));
//            }
            // ------------------------------------------------------
...
//            }
        ...
        if (this.bStabDirection || this.bWayPoint) {
            ...
            // TODO: +++ Backport from 4.13
            if(FM.Group != null && FM.Group.getAaaNum() > 3F && ((Aircraft)FM.actor).aircIndex() == 0)
        ...
        if (this.bWayPoint && this.way.isLanding()) {
            ...
            if (this.way.Cur() > 5) this.FM.set_maneuver(Maneuver.LANDING);
