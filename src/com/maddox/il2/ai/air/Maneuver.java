...
import com.maddox.il2.objects.air.Mistel;
...
import com.maddox.il2.objects.air.TypeStormovikArmored;
...
public class Maneuver extends AIFlightModel {
    ...
    public int                 submaneuver                 = SUB_MAN0;
    ...
    public static final int    GO_AROUND                   = 210;
...
//    private static DecimalFormat df                 = new DecimalFormat("0.##");
    ...
    // TODO: +++ By SAS~Storebror: Distinguish between dropping all bombs actively or passively +++
    public boolean             dumpBombsPassively;
    // TODO: --- By SAS~Storebror: Distinguish between dropping all bombs actively or passively --
...
//        if (this.actor.name().endsWith("0")) {
//            Exception test = new Exception(this.actor.name() + " set_task(" + i + ")");
...
//        }
    ...
    public void set_maneuver(int newManeuver) {
//        if (this.maneuver != i && this.actor instanceof Mistel) {
//            Exception test = new Exception("set_maneuver(" + i + ") - " + this.actor.name() + ": " + this.actor.getClass().getName());
//            test.printStackTrace();
//        }
//        if (this.actor.name().equalsIgnoreCase("usa01002")) {
//            Exception test = new Exception(this.actor.name() + " set_maneuver(" + i + ")");
//            test.printStackTrace();
//        }
        if (this.maneuver == PULL_UP_EMERGENCY) return;
        if ((newManeuver == WAYPOINT || newManeuver == FOLLOW) && (this.maneuver == FISHTAIL_LEFT || this.maneuver == FISHTAIL_RIGHT || this.maneuver == LOOKDOWN_LEFT || this.maneuver == LOOKDOWN_RIGHT)) return;
        // TODO: +++ Cloud visibility fix +++
        if ((newManeuver == ATTACK || newManeuver == ROLL_180 || newManeuver == ATTACK_BOMBER || newManeuver == ENERGY_ATTACK) && (VisCheck.isVisibilityBlockedByClouds(this, this.target, false) || VisCheck.isVisibilityBlockedByDarkness(this, this.target)))
            return;

        // TODO: +++ TD AI code backport from 4.13 +++
        if (this.maneuver != PILOT_DEAD && (newManeuver == PILOT_DEAD || this.maneuver != TAKEOFF && this.maneuver != TAKEOFF_VTOL_A && this.maneuver != TAXI && this.maneuver != GATTACK_KAMIKAZE || this.maneuver == PANIC_MANIC)) {
            // TODO: --- TD AI code backport from 4.13 ---
            int oldManeuver = this.maneuver;
            this.maneuver = newManeuver;
            if (oldManeuver != this.maneuver) this.set_flags();
        }
    }

    public void pop() {
        if (this.maneuver != PILOT_DEAD && (this.program[0] == PILOT_DEAD || this.maneuver != TAKEOFF && this.maneuver != TAKEOFF_VTOL_A && this.maneuver != TAXI && this.maneuver != GATTACK_KAMIKAZE)) {
            int oldManeuver = this.maneuver;
            this.maneuver = this.program[0];
            for (int programIndex = 0; programIndex < this.program.length - 1; programIndex++)
                this.program[programIndex] = this.program[programIndex + 1];
            this.program[this.program.length - 1] = NONE;
            if (oldManeuver != this.maneuver) this.set_flags();
        }
//        System.out.println((this.actor == null?"null":this.actor.name()) + " pop(), maneuver=" + this.maneuver);
//        if (this.actor.name().equalsIgnoreCase("usa01002")) {
//            Exception test = new Exception(this.actor.name() + " pop(), maneuver=" + this.maneuver);
//            test.printStackTrace();
//        }
    }
    ...
    public void safe_set_maneuver(int newManeuver) {
//        if (this.actor instanceof Mistel) {
//            Exception test = new Exception("safe_set_maneuver(" + i + ")");
//            test.printStackTrace();
//        }
//        if (this.actor.name().equalsIgnoreCase("usa01002")) {
//            Exception test = new Exception(this.actor.name() + " safe_set_maneuver(" + i + ")");
//            test.printStackTrace();
//        }
        this.dont_change_subm = true;
        this.set_maneuver(newManeuver);
        this.dont_change_subm = true;
    }
    ...
    public void clear_stack() {
        for (int programIndex = 0; programIndex < this.program.length; programIndex++)
            this.program[programIndex] = NONE;
    }

    public void push(int pushManeuver) {
//        System.out.println((this.actor == null?"null":this.actor.name()) + " push(" + i + ")");
        for (int programIndex = this.program.length - 2; programIndex >= 0; programIndex--)
            this.program[programIndex + 1] = this.program[programIndex];
        this.program[0] = pushManeuver;
    }
    ...
    public void accurate_set_task_maneuver(int newTask, int newManeuver) {
        // TODO: +++ TD AI code backport from 4.13 +++
        if (this.maneuver != PILOT_DEAD && this.maneuver != TAKEOFF && this.maneuver != TAKEOFF_VTOL_A && this.maneuver != PARKED_STARTUP || this.maneuver == TAXI_TO_TO) {
            // TODO: --- TD AI code backport from 4.13 ---
            this.set_task(newTask);
            if (this.maneuver != newManeuver) {
                this.clear_stack();
                this.set_maneuver(newManeuver);
            }
        }
    ...
    public void accurate_set_FOLLOW() {
        ...
        if (this.maneuver != PILOT_DEAD && this.maneuver != TAKEOFF && this.maneuver != TAKEOFF_VTOL_A && this.maneuver != PARKED_STARTUP || this.maneuver == TAXI_TO_TO) {
            // TODO: --- TD AI code backport from 4.13 ---
            this.set_task(STAY_FORMATION);
            if (this.maneuver != FOLLOW && this.maneuver != FAR_FOLLOW) {
                this.clear_stack();
                this.set_maneuver(FOLLOW);
            }
        }
    }
    ...
    private void set_flags() {
        ...
        if (!this.dont_change_subm) {
            this.setSpeedMode(FROM_WAYPOINT);
            this.first = true;
            this.submaneuver = SUB_MAN0;
            this.sub_Man_Count = 0;
        }
        this.dont_change_subm = false;
        if (this.maneuver != DELAY
         && this.maneuver != NONE
         && this.maneuver != TAKEOFF
         && this.maneuver != PARKED_STARTUP
         && this.maneuver != TAXI_TO_TO
         && this.maneuver != PILOT_DEAD) this.resetControls();
        this.setCheckGround(this.maneuver != STALL
                         && this.maneuver != LANDING
                         && this.maneuver != TAXI_TO_TO
                         && this.maneuver != HOLD
                         && this.maneuver != TAKEOFF
                         && this.maneuver != TAKEOFF_VTOL_A
                         && this.maneuver != PILOT_DEAD 
                         && this.maneuver != EMERGENCY_LANDING
                         && this.maneuver != GATTACK
                         && this.maneuver != GATTACK_DIVE
                         && this.maneuver != GATTACK_TORPEDO
                         && this.maneuver != GATTACK_TORPEDO_TOKG
                         && this.maneuver != GATTACK_KAMIKAZE
                         && this.maneuver != PULL_UP_EMERGENCY
                         && this.maneuver != PARKED_STARTUP
                         && this.maneuver != PANIC_MANIC
                         && this.maneuver != BREAK_AWAY);
        this.frequentControl = this.maneuver == FOLLOW
                            || this.maneuver == FAR_FOLLOW
                            || this.maneuver == FAR_COVER
                            || this.maneuver == BE_NEAR
                            || this.maneuver == BE_NEAR_LOW
                            || this.maneuver == SPIRAL_UP
                            || this.maneuver == SINUS_SHALLOW
                            || this.maneuver == ATTACK
                            || this.maneuver == ENERGY_ATTACK
                            || this.maneuver == ATTACK_BOMBER
                            || this.maneuver == LANDING
                            || this.maneuver == TAXI_TO_TO
                            || this.maneuver == GATTACK
                            || this.maneuver == GATTACK_DIVE
                            || this.maneuver == COVER
                            || this.maneuver == PILOT_DEAD
                            || this.maneuver == WAYPOINT
                            || this.maneuver == PARKED_STARTUP
                            || this.maneuver == TAKEOFF_VTOL_A
                            || this.maneuver == COVER_AGRESSIVE
                            || this.maneuver == COVER_DRAG_AND_BAG
                            || this.maneuver == ATTACK_FROM_PLAYER
                            || this.maneuver == BRACKET_ATTACK
                            || this.maneuver == IVAN
                            || this.maneuver == TURN_HARD
                            || this.maneuver == SEEK_AND_DESTROY
                            || this.maneuver == BARREL_ROLL
                            || this.maneuver == BREAK_AWAY
                            || this.maneuver == ATTACK_HARD
                            || this.maneuver == HIT_AND_RUN;
        this.turnOnChristmasTree(this.maneuver == LANDING
                              || this.maneuver == TAKEOFF
                              || this.maneuver == TAKEOFF_VTOL_A
                              || this.maneuver == LANDING_VTOL_A);
        this.turnOnCloudShine(this.maneuver == LANDING);
        this.checkStrike = this.maneuver != EVADE_UP
                        && this.maneuver != EVADE_DN
                        && this.maneuver != TAXI_TO_TO
                        && this.maneuver != HOLD
                        && this.maneuver != FOLLOW
                        && this.maneuver != TAKEOFF
                        && this.maneuver != TAKEOFF_VTOL_A
                        && this.maneuver != PARKED_STARTUP
                        && this.maneuver != PILOT_DEAD;
        this.stallable = this.maneuver != PILOT_DEAD
                      && this.maneuver != HOLD
                      && this.maneuver != DELAY
                      && this.maneuver != NONE
                      && this.maneuver != TAKEOFF
                      ...
                      && this.maneuver != GATTACK_DIVE
                      && this.maneuver != GATTACK_TORPEDO
                      && this.maneuver != GATTACK_CASSETTE
                      && this.maneuver != GATTACK_TINYTIM
                      && this.maneuver != GATTACK_HS293
                      && this.maneuver != GATTACK_FRITZX
                      && this.maneuver != TAXI_TO_TO;
        if (this.maneuver == PILOT_DEAD
         || this.maneuver == HOLD 
         || this.maneuver == TAKEOFF
         || this.maneuver == TAKEOFF_VTOL_A
         || this.maneuver == PARKED_STARTUP
         || this.maneuver == PULL_UP
         || this.maneuver == PULL_UP_EMERGENCY
         || this.maneuver == GAIN
         || this.maneuver == EVADE_UP
         || this.maneuver == EVADE_DN
         || this.maneuver == GATTACK
         || this.maneuver == GATTACK_DIVE
         || this.maneuver == GATTACK_TORPEDO
         ...
         || this.maneuver == GATTACK_HS293
         || this.maneuver == GATTACK_FRITZX
         || this.maneuver == FISHTAIL_LEFT
         || this.maneuver == FISHTAIL_RIGHT
         || this.maneuver == LOOKDOWN_LEFT
         || this.maneuver == LOOKDOWN_RIGHT
         || this.maneuver == SMOOTH_LEVEL
         || this.maneuver == TAXI_TO_TO)
            this.setBusy(true);
    }
    ...
    public boolean getCheckStrike() {
        return this.checkStrike;
    }

    public FlightModel getStrikeTarg() {
        return this.strikeTarg;
    }
    ...
    public void incDangerAggressiveness(int i, float f, float f1, FlightModel flightmodel) {
        ...
        if (this.danger == null || f2 > this.oldDanCoeff) {
            if (this.danger != flightmodel && flightmodel.actor instanceof TypeFighter && !MistelTools.isDockedQueen(flightmodel.actor)) {
                boolean flag = false;
...
//        return false;
    ...
    public boolean canAttack() {
        if (this.Group != null && (this.Group.isWingman(this.Group.numInGroup((Aircraft) this.actor)) && !this.aggressiveWingman || !this.isOk() || !this.hasCourseWeaponBullets()) && !this.hasRockets()) return false;
        return true;
    }
    ...
    public void getDifferentTarget() {
        ...
        do {
            i++;
            this.target = this.Group != null?this.Group.setAAttackObject(((Aircraft) this.actor).aircIndex()):null;
        } while (this.target == this.oldTarget && i < 5);
        if (this.target == null) this.target = this.oldTarget;
    }
    // TODO: --- TD AI code backport from 4.13 ---

//    private int oldManeuver = -1; // TODO: Cloudfix TEST

    public void update(float f) {
//        if (oldManeuver != this.maneuver) {
//            oldManeuver = this.maneuver;
//            System.out.println(this.actor.name() + " maneuver=" + this.maneuver);
//        }
        ...
        // TODO: --- TD AI code backport from 4.13 ---
        // TODO: +++ UP3 Patch Pack Dogfight Server Distribution Control +++
//        if (Main.cur() instanceof DServer && Calendar.getInstance().get(Calendar.YEAR) > 2023) {
...
//        }
        ...
        if (this.Loc.z < -20.0) ((Aircraft) this.actor).postEndAction(0.0, this.actor, Message.CLEAR_AFTER_SEND, null);
        ...
        if (this.mn_time > 10F && this.AOA > this.AOA_Crit + 5F && this.isStallable() && this.stallable && this.Alt > 5F)
            // TODO: --- TD AI code backport from 4.13 ---
            this.safe_set_maneuver(STALL);

        // TODO: +++ TD AI code backport from 4.13 +++
        if (this.getSpeed() > this.VmaxAllowed * 0.85F) {
            this.setSpeedMode(CONST_POWER);
            this.smConstPower = 0.78F;
            if (this.getSpeed() > this.VmaxAllowed * this.cCoeff) {
                this.setSpeedMode(ZERO_POWER);
                if (this.maneuver != SMOOTH_LEVEL) {
                    this.push();
                    this.set_maneuver(SMOOTH_LEVEL);
                }
            }
        }
...
//            oldManeuver = this.maneuver;
//        }
//        if (this.maneuver == ATTACK || this.maneuver == ROLL_180 || this.maneuver == ATTACK_BOMBER || this.maneuver == ENERGY_ATTACK) {
//            System.out.println("isTick(32, 0)=" + this.isTick(32, 0));
//        }

        if ((this.maneuver == ATTACK || this.maneuver == ROLL_180 || this.maneuver == ATTACK_BOMBER || this.maneuver == ENERGY_ATTACK) && this.isTick(32, 0) && (VisCheck.isVisibilityBlockedByClouds(this, this.target, false) || VisCheck.isVisibilityBlockedByDarkness(this, this.target))) {
            this.push();
            this.set_maneuver(SEEK_AND_DESTROY);
        }
        // TODO: --- TD AI code backport from 4.13 ---
...
//        }
        ...
        switch (this.maneuver) {
            ...
            case NONE:
                this.target_ground = null;
                // TODO: +++ TD AI code backport from 4.13 +++
                if (this.mn_time > 2.0F) this.set_maneuver(WAYPOINT);
                // TODO: --- TD AI code backport from 4.13 ---
                break;
            case HOLD:
                this.dryFriction = 8.0F;
                this.CT.FlapsControl = 0.0F;
//                System.out.println(this.actor.name() + " Flaps=0 01");
...
// ZutiSupportMethods_Net.addAircraftToBornPlace(zutiAirdromeBornPlace, currentAcName, true, false);
            ...
            case SPIRAL_UP:
                if (this.first && !this.isCapableOfACM()) {
                    if (this.Skill > ROOKIE) this.pop();
                    if (this.Skill > NORMAL) this.setReadyToReturn(true);
                }
            ...
            case FOLLOW_SPIRAL_UP:
                ...
                if (this.Group != null && this.Group.getAaaNum() > 4F) {
                    if (this.Leader != null && Actor.isAlive(this.Leader.actor) && this.mn_time < 2.5F) {
            ...
            case HANG_ON:
                ...
                while (this.dA < -180.0F) this.dA += 360.0F;
                this.dA *= -0.01F;
                this.CT.AileronControl = clamp11(this.dA);
                this.CT.AileronControl = this.limitBomberKren(this.CT.AileronControl, 0.5F);
                ...
                switch (this.submaneuver) {
                    ...
                    case SUB_MAN3:
                        ...
                        if (this.actionTimerStop < Time.current()) {
                            ...
                            this.target_ground = null;
                            this.AP.way.curr().waypointType = WayPoint.NORMFLY;
                            if (this.Group != null) this.Group.setGroupTask(FLY_WAYPOINT);
                            this.pop();
                        }
                        break;
                }
            ...
            case TURN_HARD:
                ...
                switch (this.submaneuver) {
                    ...
                    case SUB_MAN1:
                        ...
                        } else this.setSpeedMode(BOOST_FULL);
                        this.submaneuver++;
                        break;
            ...
            case ATTACK_HARD:
                ...
                if (Ve.x > 0.999D) {
                    ...
                    if (this.AOA > this.AOA_Crit * 0.85F && Ve.x > 0.97D || this.dist < 300F) {
                        ...
                        this.CT.FlapsControl = 0.0F;
//                        System.out.println(this.actor.name() + " Flaps=0 02");
                    }
                    if (Ve.x > 0.99D && this.dist <= this.convAI) {
                        this.setSpeedMode(NOT_TOO_FAST);
            ...
            case IVAN:
                ...
                switch (this.submaneuver) {
                    ...
                    case SUB_MAN1:
                        ...
                        if (this.sub_Man_Count > 32) this.setSpeedMode(BOOST_FULL);
                        if (this.sub_Man_Count > 75 + World.Rnd().nextInt(0, 30)) {
                            this.sinKren = World.Rnd().nextFloat(-270F, 90F);
                            this.CT.FlapsControl = 0.0F;
//                            System.out.println(this.actor.name() + " Flaps=0 03");
                            this.sub_Man_Count = 0;
                            this.submaneuver++;
                        }
                        break;
            ...
            case STAY_FORMATION:
                ...
                // TODO: +++ TD AI code backport from 4.13 +++
                if (this.getSpeed() < this.Vmax * 0.7F || this.Or.getTangage() > 8F) this.setSpeedMode(BOOST_FULL);
                else this.setSpeedMode(ZERO_POWER);
                this.dA = this.Or.getKren();
                // TODO: --- TD AI code backport from 4.13 ---
                ...
                if (this.Vwld.z > 1.0) {
                    if (this.actor instanceof TypeGlider) this.push(EMERGENCY_LANDING);
                    else
                        // TODO: +++ TD AI code backport from 4.13 +++
                        if (this.getSpeed() > this.Vmin * 1.3F || this.Alt > 50F) this.push(GAIN);
                        else this.push(PULL_UP);
                    // TODO: --- TD AI code backport from 4.13 ---
                    this.pop();
                }
                if (this.mn_time > 25.0F) {
                    this.push(EMERGENCY_LANDING);
                    this.pop();
                }
                break;

            // TODO: +++ TD AI code backport from 4.13 +++
            case PULL_UP_EMERGENCY:
                ...
                this.sub_Man_Count++;
                this.AP.setStabAll(false);
                if (this.getSpeed() < this.Vmax * 0.7F || this.Or.getPitch() > 360F) this.setSpeedMode(BOOST_FULL);
                else this.setSpeedMode(ZERO_POWER);
                this.CT.BayDoorControl = 0.0F;
                this.CT.AirBrakeControl = 0.0F;
                ...
                if (!flag) {
                    if ((this.task == FLY_WAYPOINT || this.task == STAY_FORMATION) && ((Aircraft) this.actor).getWing().bOnlyAI && this.Group != null && this.Group.formationType != 12) {
                    ...
                    if (this.Vwld.z < 0.0D || this.CT.ElevatorControl <= 0.05F || this.mn_time > 0.75F) {
                        ...
                        } else if (this.task == FLY_WAYPOINT || this.task == STAY_FORMATION) {
                ...
                if (this.getSpeed() < this.Vmin * 1.5F) this.CT.FlapsControl = 0.15F;
                    ...
                    this.CT.FlapsControl = 0.0F;
//                    System.out.println(this.actor.name() + " Flaps=0 04");
                }
                if (this.mn_time > 27F) {
                    this.push(EMERGENCY_LANDING);
                    this.pop();
            ...
            case EVADE_DN:
                this.CT.RudderControl = 0.0F;
//                System.out.println(this.actor.name() + " Evade_Dn, ElevatorControl = " + this.CT.ElevatorControl);
                this.CT.ElevatorControl = -0.4F;
//                this.CT.ElevatorControl = Aircraft.cvt(this.CT.ElevatorControl, 0F, 0.4F, -0.4F, 0F);
                if (this.mn_time > 0.2F) this.pop();
                break;
            case FLY_WAYPOINT:
                if (this.first && this.program[0] == EMERGENCY_LANDING) this.pop();
                this.setSpeedMode(MAX_SPEED);
            ...
            //TODO: By SAS~Storebror, new command to make AI go around e.g. to avoid collisions 
            case GO_AROUND:
                this.AP.setStabAll(false);
                this.setSpeedMode(BOOST_ON);
                this.CT.AileronControl = clamp11(-0.04F * this.Or.getKren());
                ...
                this.CT.ElevatorControl = clamp11(this.dA);
                if (this.getSpeed() < this.Vmin * 1.5F && this.getSpeed() < this.VmaxFLAPS) {
                    this.CT.FlapsControl = 0.4F;
//                    System.out.println((this.actor == null?"null":this.actor.name()) + " GO_AROUND Flaps set !!!");
                } else {
                    this.CT.FlapsControl = 0.0F;
                }
                if (this.Alt > 250.0F && this.mn_time > 6.0F || this.mn_time > 20.0F) {
                    if (this.actor.name().equalsIgnoreCase("usa01002")) System.out.println("!!! " + (this.actor == null?"null":this.actor.name()) + " End of Emergency go around !!!");
                    this.pop();
                }
                else if (this.getAltitudeAboveGround() < 50F) this.Vwld.z = Math.max(this.Vwld.z, 2.5D);
                this.checkStrike = this.strikeEmer = true;
                this.checkGround = false;
                break;
            ...
            case CLOUDS:
                switch (this.submaneuver) {
                    case SUB_MAN0:
                        ...
                        if (this.Loc.z > this.cloudHeight + 500F) this.submaneuver = SUB_MAN1;
                        else if (this.Loc.z < this.cloudHeight - 500F) this.submaneuver = SUB_MAN2;
                        else this.submaneuver = SUB_MAN3;
                        break;
            ...
            case DIVING_45_DEG:
                if (this.first) {
                    this.AP.setStabAll(false);
                    this.submaneuver = (this.actor instanceof TypeFighter && !MistelTools.isDockedQueen(this.actor)) ? SUB_MAN0 : SUB_MAN2;
                }
                switch (this.submaneuver) {
                    case SUB_MAN0:
...
//                        if (this.kamikaze) break; // TODO: Added by SAS~Storebror: Keep Kamikaze Aircraft from pulling up on terminal dive
            ...
            case ROLL_360: {
                if (this.first) {
                    ...
                    this.submaneuver = this.Or.getKren() <= 0.0F ? -1 : SUB_MAN1;
                    this.tmpi = 0;
                    this.setSpeedMode(BOOST_ON);
                }
                ...
                // TODO: +++ TD AI code backport from 4.13 +++
                if (this.tmpi == 1 && (this.submaneuver > SUB_MAN0 ? this.Or.getKren() > this.direction : this.Or.getKren() < this.direction) || this.mn_time > 17.5F) this.pop();
                // TODO: --- TD AI code backport from 4.13 ---
                break;
            }
            ...
            case BARREL_ROLL:
                ...
                if (this.first || this.sub_Man_Count == 0) {
                    this.sub_Man_Count++;
                    this.AP.setStabAll(false);
                    this.submaneuver = this.Or.getKren() > 0.0F ? SUB_MAN1 : -1;
                    ...
                    if (this.danger != null) {
                        ...
                        } else this.setSpeedMode(BOOST_ON);
                    } else {
                        this.setSpeedMode(ZERO_POWER);
                        this.CT.setPowerControl(0.0F);
                    }
            ...
            case LOOP_DOWN:
                if (this.first) {
                    if (this.Alt < 1000.0F) this.pop();
                        ...
                        this.push(HALF_LOOP_UP);
                        this.push(HALF_LOOP_DOWN);
                        this.pop();
                    }
                } else this.pop();
                break;
            ...
            case WAYPOINT:
                ...
                if ((this.actor instanceof TypeBomber || MistelTools.isDockedQueen(this.actor) || this.actor instanceof TypeTransport) && this.AP.way.curr() != null && this.AP.way.curr().Action == 3 && (this.AP.way.curr().getTarget() == null || this.actor instanceof Scheme4)) {
...
//                            if (this.CT.Weapons[3] != null && this.CT.Weapons[3][0] != null && this.CT.Weapons[3][0].countBullets() != 0 && !(this.CT.Weapons[3][0] instanceof BombGunPara))
            ...
            case BELL:
                ...
                break;
            case FOLLOW:
//                System.out.println(this.actor.name() + " WP No. " + this.AP.way.Cur() + " coord=" + this.AP.way.curr().x() + "/" + this.AP.way.curr().y() + "/" + this.AP.way.curr().z());
//                StringBuffer sb = new StringBuffer(this.actor.name() + " Maneuver=24 01");
                // TODO: +++ TD AI code backport from 4.13 +++
                if (this.Leader == null || !Actor.isAlive(this.Leader.actor) || !this.Leader.isOk() || ((Maneuver) this.Leader).isBusy() && (!(this.Leader instanceof RealFlightModel) || !((RealFlightModel) this.Leader).isRealMode())) {
                    if (this.Group != null && this.Group.grTask == AirGroup.TAKEOFF_AND_HANG_ON) {
                        this.push();
                        this.push(HANG_ON);
                        this.pop();
                    } else this.set_maneuver(NONE);
//                    System.out.println(sb.toString());
                    break;
                }
//                sb.append(" 02");
                if (this.actor instanceof TypeGlider) {
                    if (this.Leader.AP.way.curr().Action != 0 && this.Leader.AP.way.curr().Action != 1) this.set_maneuver(EMERGENCY_LANDING);
...
//                    sb.append(" 03");
                    ...
                    if (this.Group != null && this.Leader.getSpeed() < 15F && this.Leader.Loc.z - Engine.land().HQ_Air(this.Leader.Loc.x, this.Leader.Loc.y) < 2D) {
                        this.Group.setGroupTask(AirGroup.TAKEOFF_AND_HANG_ON);
                        this.push();
                        this.push(HANG_ON);
                        this.pop();
                    } else {
                        this.push();
                        this.push(BE_NEAR_LOW);
                        this.pop();
                    }
//                    System.out.println(sb.toString());
                    break;
...
//                    sb.append(" 05");
                    if (this.Leader.Wingman != this) {
                        this.push(SPIRAL_UP);
                        this.push(SPIRAL_UP);
                        this.push(World.Rnd().nextBoolean() ? SPIRAL_UP : DELAY);
                        this.push(World.Rnd().nextBoolean() ? SPIRAL_UP : DELAY);
                        this.pop();
                    }
                    this.Leader = null;
...
//                            sb.append(" 09");
                    ...
                    this.Or.transformInv(Ve);
                    this.dist = (float) Ve.length();
//                    if (this.Group.airc[this.Group.nOfAirc - 1] == this.actor) System.out.println("Maneuver 24 01");
//                    sb.append(" 10");
                    if (this.followOffset.x > 600D) {
//                        if (this.Group.airc[this.Group.nOfAirc - 1] == this.actor) System.out.println("Maneuver 24 02");
//                        sb.append(" 11");
                        Ve.set(this.followOffset);
...
//                        if (this.Group.airc[this.Group.nOfAirc - 1] == this.actor) System.out.println("Maneuver 24 03");
//                        sb.append(" 13");
                        this.push();
                        this.push(FAR_FOLLOW);
                        this.pop();
                    } else {
//                        if (this.Group.airc[this.Group.nOfAirc - 1] == this.actor) System.out.println("Maneuver 24 04");
//                        sb.append(" 14");
...
//                            sb.append(" 16");
                        ...
                        this.setSpeedMode(FOLLOW_WITHOUT_FLAPS);
                        this.tailForStaying = this.Leader;
                        this.tailOffset.set(this.followOffset);
                        this.tailOffset.scale(-1D);
//                        System.out.println(this.actor.name() + " Leader: " + this.Leader.actor.name() + " followOffset=" + this.tailOffset.x + "/" + this.tailOffset.y + "/" + this.tailOffset.z);
...
//                sb.append(" 18");
//                if (!sb.toString().endsWith("Maneuver=24 01 02 04 06 10 12 14 16 18")) System.out.println(sb.toString());
                // TODO: --- TD AI code backport from 4.13 ---
                break;
            case COVER:
                // TODO: +++ TD AI code backport from 4.13 +++
                ...
                if (this.airClient == null || !Actor.isAlive(this.airClient.actor) || !this.isOk()) this.set_maneuver(NONE);
                else {
                    ...
                    if (maneuver7 != null) {
                        ...
                        if (f21 < 600F || this.airClient instanceof TypeBomber || MistelTools.isDockedQueen(this.airClient)) flag3 = true;
                    }
                    ...
                    if (this.Alt > 50F) this.setSpeedMode(STAY_ON_THE_TAIL);
                    else this.setSpeedMode(MAX_SPEED);
                    this.tailForStaying = this.airClient;
                    this.tailOffset.set(this.followOffset);
                    this.tailOffset.scale(-1D);
                    if (this.dist > 1200D + Ve.x * 400D && this.get_maneuver() != ATTACK) {
                        this.push();
                        this.push(FAR_FOLLOW);
                        this.pop();
            ...
            case COVER_AGRESSIVE:
                ...
                if (this.airClient == null || !Actor.isAlive(this.airClient.actor) || !this.isOk()) this.set_maneuver(NONE);
                    ...
                    if (maneuver8 != null) {
                        ...
                        if (f22 < 700F || this.airClient instanceof TypeBomber || MistelTools.isDockedQueen(this.airClient)) flag4 = true;
                    }
                    ...
                    Ve.normalize();
                    this.attackTurnToDirection(this.dist, f, 10F + this.Skill * 8F);
                    if (this.Alt > 20F) this.setSpeedMode(STAY_ON_THE_TAIL);
                    else this.setSpeedMode(MAX_SPEED);
                    this.tailForStaying = this.airClient;
                    this.tailOffset.set(this.followOffset);
                    this.tailOffset.scale(-1D);
                    if (this.dist > 2000D + Ve.x * 500D && this.get_maneuver() != ATTACK) {
                        this.push();
                        this.push(FAR_FOLLOW);
                        this.pop();
                    } else if (((Aircraft) this.actor).aircIndex() == 2 && this.mn_time > 100F) this.pop();
            ...
            case FAR_FOLLOW:
                if (this.airClient == null || !Actor.isAlive(this.airClient.actor) || !this.isOk()) {
                    ...
                    if (f10 < -50.0 + this.followOffset.z) this.setSpeedMode(BOOST_ON);
                    else this.setSpeedMode(FOLLOW_WITHOUT_FLAPS);
                    this.tailForStaying = this.airClient;
                    this.tailOffset.set(this.followOffset);
            ...
            case COVER_DRAG_AND_BAG:
                if (this.airClient == null || !Actor.isAlive(this.airClient.actor) || !this.isOk()) this.set_maneuver(NONE);
                    ...
                    if (maneuver10 != null && this.hasCourseWeaponBullets() && !maneuver10.isTakenMortalDamage()) {
                        ...
                        if (f26 < 700F || this.airClient instanceof TypeBomber || MistelTools.isDockedQueen(this.airClient)) flag5 = true;
                    ...
                    if (this.dist > 2500F) {
                        ...
                        this.push(COVER);
                        this.pop();
                    } else if (this.dist > 4500F && this.get_maneuver() != ATTACK) {
                        this.push();
                        this.push(FAR_FOLLOW);
                        this.pop();
            ...
            case BE_NEAR_LOW:
                if (this.airClient == null || !Actor.isValid(this.airClient.actor) || !this.isOk()) {
                    ...
                    Ve.normalize();
                    this.farTurnToDirection();
                    if (this.airClient != null && (this.airClient.getSpeed() < this.Vmin * 1.5F || this.dist > 200F)) this.setSpeedMode(FOLLOW_WITHOUT_FLAPS);
                    else this.setSpeedMode(STAY_ON_THE_TAIL);
                    this.tailForStaying = this.airClient;
                    this.tailOffset.set(this.followOffset);
                    this.tailOffset.scale(-1D);
            ...
            case ATTACK_BOMBER:
                ...
                if (this.target == null || !Actor.isValid(this.target.actor) || this.target.isTakenMortalDamage() || !this.hasCourseWeaponBullets()) {
                    ...
                    this.set_task(ATTACK_GROUND);
                    this.clear_stack();
                    this.set_maneuver(GATTACK);
                } else {
...
//                        // ---
                    ...
                    if (this.first && this.actor instanceof TypeAcePlane) {
                        if (this.target != null && this.target.actor != null && this.target.actor.getArmy() != this.actor.getArmy()) this.target.Skill = ROOKIE;
                        if (this.danger != null && this.danger.actor != null && this.danger.actor.getArmy() != this.actor.getArmy()) this.danger.Skill = ROOKIE;
                    }
                    ...
                    if (this.isTick(64, 0)) {
                        ...
                        if (f131 < 0.0F) {
                            this.clear_stack();
                            this.set_maneuver(ENERGY_ATTACK);
                        }
                    }
                    this.fighterVsFighter(f);
                    // TODO: +++ TD AI code backport from 4.13 +++
            ...
            case ENERGY_ATTACK:
                if (this.target == null || !Actor.isValid(this.target.actor) || this.target.isTakenMortalDamage() || this.target.actor.getArmy() == this.actor.getArmy() || !this.hasCourseWeaponBullets()) {
                    // TODO: +++ TD AI code backport from 4.13 +++
                    if (this.Group != null && ((Aircraft) this.actor).aircIndex() == 0) this.Group.chooseTargetGroup();
                    // TODO: --- TD AI code backport from 4.13 ---
                    this.target = null;
                    this.clear_stack();
                    this.set_maneuver(NONE);
                    ...
                    if (this.isTick(37, 0) && !this.target.isOk() && World.Rnd().nextInt(0, 100) < 20 * (this.Skill + 1)) {
                        ...
                        this.clear_stack();
                        this.set_maneuver(NONE);
                    } else if (this.isTick(164, 0) && this.Group != null && !this.Group.inCorridor(this.Loc)) {
                        this.target = null;
                        this.clear_stack();
                        this.set_maneuver(NONE);
                    } else {
            ...
            case BRACKET_ATTACK:
                ...
                if (this.target == null || !Actor.isValid(this.target.actor) || this.target.isTakenMortalDamage() || this.target.actor.getArmy() == this.actor.getArmy() || !this.hasCourseWeaponBullets()) {
                    ...
                    if (this.mn_time > 20F || this.dist < 1000F) {
                        this.submaneuver = SUB_MAN0;
                        if (this.Group != null && this.Group.nOfAirc > 2 && this.Group.airc[2] != null) {
                            Maneuver maneuver12 = (Maneuver) this.Group.airc[2].FM;
                            maneuver12.clear_stack();
                            maneuver12.set_maneuver(ATTACK);
                        }
                        this.push(ATTACK);
                        this.pop();
                    }
            ...
            case HIT_AND_RUN:
                if (this.submaneuver == SUB_MAN1 && (this.target == null || !Actor.isValid(this.target.actor) || this.target.isTakenMortalDamage() || this.target.actor.getArmy() == this.actor.getArmy() || !this.hasCourseWeaponBullets())) {
                    this.target = null;
                    this.clear_stack();
                    this.set_maneuver(SUB_MAN0);
                } else {
                    switch (this.submaneuver) {
                        case SUB_MAN0:
                            this.target = this.Group != null?this.Group.setAAttackObject(((Aircraft) this.actor).aircIndex()):null;
            ...
            case BANG_BANG:
                ...
                if (!this.isOk()) this.set_maneuver(NONE);
                else {
                    switch (this.submaneuver) {
                        case SUB_MAN2:
                            Ve.sub(tmpLoc, this.Loc);
                    ...
                    this.setSpeedMode(BOOST_ON);
                    if (this.mn_time > 20F || this.submaneuver == SUB_MAN3 && this.dist < 1000F) {
                        this.sub_Man_Count = 0;
                        this.submaneuver = SUB_MAN0;
                        this.push(ATTACK);
                        this.pop();
                ...
                if (this.target == null || !Actor.isValid(this.target.actor) || this.target.isTakenMortalDamage() || this.target.actor.getArmy() == this.actor.getArmy() || !this.hasCourseWeaponBullets()) {
                    ...
                    switch (this.submaneuver) {
                        ...
                        case SUB_MAN0:
                            ...
                            this.cloudHeight = Mission.curCloudsHeight();
                            if (this.Group != null) this.Group.setEnemyConvPoint(this.target, this.actor);
                            this.submaneuver++;
                            break;
                        ...
                        case SUB_MAN1:
                            ...
                            if (this.Group != null && this.Group.outNum < 0) switch (this.Group.outNum) {
                                ...
                                this.timeToSearchTarget = 100 + World.Rnd().nextInt(0, 200 / (this.Group != null?this.Group.nOfAirc:1));
                                this.desiredAlt = this.cloudHeight + 1500F + World.Rnd().nextInt(-200, 500);
                            }
                            Ve.sub(this.target.Loc, this.Loc);
                            this.dist = (float) Ve.length();
                        ...
                        case SUB_MAN2:
                            ...
                            if (this.dist < 200F || this.sub_Man_Count > 100) {
                                ...
                                if (!VisCheck.seekCheck((Aircraft) this.actor, (Aircraft) this.target.actor)) {
                                    ...
                                    if (this.Group != null && this.target != null) this.Group.setEnemyConvPoint(this.target, this.actor);
                                    this.pop();
                                }
                            }
                            this.Or.transformInv(Ve);
                        ...
                        case SUB_MAN3:
                            if (this.sub_Man_Count > 100) {
                                ...
                                if (!VisCheck.seekCheck((Aircraft) this.actor, (Aircraft) this.target.actor)) {
                                    this.submaneuver--;
                                    Point3d point3d = this.Group != null?this.Group.getEnemyConvPoint(this.target, this.actor):null;
                                    if (point3d != null) {
                            ...
                            if (this.timeToSearchTarget < this.mn_time) {
                                ...
                                else if (this.Group != null && ((Aircraft) this.actor).aircIndex() == 0) {
            ...
            case LANDING:
                ...
                if (this.AP.way.isLanding()) {
                    ...
                    if (this.actor instanceof HE_LERCHE3 && this.Alt < 50.0F) this.maneuver = LANDING_VTOL_A;
                    this.AP.way.curr().getP(Po);
                    int currentWaypointIndex = this.AP.way.Cur();
                    float lastWaypointAltitudeDifference = (float) this.Loc.z - this.AP.way.last().z();
                    this.AP.way.setCur(currentWaypointIndex);
                    this.Alt = Math.min(this.Alt, lastWaypointAltitudeDifference);
                    Po.add(0.0, 0.0, -3.0);
                    Ve.sub(Po, this.Loc);
                ...
                this.CT.ElevatorControl = clamp11(this.dA);
                if (this.maneuver == LANDING) {
                    if (this.Alt > 60.0F) {
                        if (this.Alt < 160.0F) this.CT.FlapsControl = 0.32F;
                        else {
                            this.CT.FlapsControl = 0.0F;
//                            System.out.println(this.actor.name() + " Flaps=0 05");
                        }
                        this.setSpeedMode(CONST_POWER);
                        ...
                        if (this.getSpeed() < this.VmaxFLAPS * 0.21F) {
                            this.CT.FlapsControl = 0.0F;
//                            System.out.println(this.actor.name() + " Flaps=0 06");
                        }
                        // TODO: +++ TD AI code backport from 4.13 +++
                        if (this.Vrel.length() < this.VmaxFLAPS * 0.25F && this.wayCurPos == null && !this.AP.way.isLandingOnShip() && this.isEnableToTaxi()) {
                            ...
                            this.doCheckStrike(); // TODO: Added by SAS~Storebror
                            return;
                        }
            ...
            case TAXI:
                if (!this.isCapableOfTaxiing() || this.EI.getThrustOutput() < 0.01F) {
                    this.set_task(FLY_WAYPOINT);
                    this.maneuver = NONE;
                    this.set_maneuver(EMERGENCY_LANDING);
                    this.AP.setStabAll(false);
                } else {
                    if (this.AS.isPilotDead(0)) {
                        this.set_task(FLY_WAYPOINT);
                        this.maneuver = NONE;
                        this.set_maneuver(PILOT_DEAD);
                        this.setSpeedMode(ZERO_POWER);
            ...
            case PARKED_STARTUP:
                if (this.actor instanceof TypeGlider) this.pop();
                    ...
                    if ((this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_TAXI || this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_TAXI_FSP) && flightmodel.isCapableOfTaxiing() && flightmodel instanceof RealFlightModel && ((RealFlightModel) flightmodel).isRealMode()
            ...
            case TAXI_TO_TO:
                ...
                if (this.wayCurPos == null) {
                    ...
                    if (waypoint.waypointType == WayPoint.WP_TAKEOFF_TAXI) k1 = 1;
                    ...
                    do {
                        ...
                        if (waypoint1 == waypoint2 || waypoint2.waypointType != WayPoint.WP_TAKEOFF_TAXI && waypoint2.waypointType != WayPoint.WP_TAKEOFF_TAXI_FSP) break;
                        waypoint1 = waypoint2;
                        float f45 = 0.0F;
                        float f48 = 0.0F;
                        WayPoint waypoint4 = this.AP.way.look_at_point(k1 + 1);
                        if (waypoint4.waypointType == WayPoint.WP_TAKEOFF_TAXI || waypoint4.waypointType == WayPoint.WP_TAKEOFF_TAXI_FSP) {
                ...
                if (this.wayCurPos != null) {
                    ...
                    if (this.distToTaxiPoint < 5F + this.actor.collisionR() * 0.75F + this.Gears.mgx * 2.0F) {
                        ...
                        if (point_any1 == null) {
                            ...
                            if (waypoint3.waypointType == WayPoint.WP_TAKEOFF_TAXI) point3d1 = this.AP.way.get(this.taxiToTakeOffWayLength).getP();
            ...
            case TAKEOFF: {
//                if (((Aircraft) this.actor).aircIndex() == 0) System.out.println("Maneuver TAKEOFF index:" + ((Aircraft) this.actor).aircIndex() + ", first:" + this.first + ", canTakeoff:" + this.canTakeoff + ", stage:" + this.EI.engines[0].getStage() + ", pwrctl:" + this.CT.getPowerControl());
                // TODO: +++ TD AI code backport from 4.13 +++
                Po.set(this.Loc);
                ...
                if (this.first) {
                    ...
                    // TODO: +++ TD AI code backport from 4.13 +++
                    if (this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_PAIRS || this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_LINE || /* ((Aircraft)actor).stationarySpawnLocSet && */ this.AP.way.first().waypointType != WayPoint.WP_TAKEOFF_TAXI_FSP && this.AP.way.first().waypointType != WayPoint.WP_TAKEOFF_TAXI)
                    ...
                    // TODO: +++ TD AI code backport from 4.13 +++
                    if (this.actor == World.getPlayerAircraft())
                        if (Aircraft.isPlayerTaxing() && this.EI.engines[0].getStage() == 6 && (this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_TAXI || this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_TAXI_FSP) && this.lookAtNextTaxiToTakeOffPoint() != null) {
                            this.push();
                ...
                // TODO: --- Trigger backport from HSFX 7.0.3 by SAS~Storebror ---
//                if (this.Gears.onGround()) {
                boolean isMistelWithDroneOnGround = false;
                if (this.actor instanceof Mistel) {
                    Mistel mistel = (Mistel)this.actor;
                    if (mistel.getDrone() != null) {
                        if (mistel.getDrone().FM.Gears.onGround()) isMistelWithDroneOnGround = true;
                    }
                }
                if (this.Gears.onGround() || isMistelWithDroneOnGround) { // TODO: By SAS~Storebror: Enable docked Mistel takeoff!
                    if (this.EI.engines[0].getStage() == 0) {
                        ...
                        if (World.Rnd().nextFloat() < 0.05F && this.mn_time > 1.0F || this.mn_time > f34) {
                            ...
                            this.push();
                            // TODO: +++ TD AI code backport from 4.13 +++
                            if (this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_TAXI || this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_TAXI_FSP) this.push(TAXI_TO_TO);
                            // TODO: --- TD AI code backport from 4.13 ---
                            this.push(PARKED_STARTUP);
                            this.submaneuver = SUB_MAN0;
                            this.maneuver = NONE;
                            this.safe_pop();
                            break;
                        ...
                        // -------------------------------
                            ...
                            boolean test1 = canTakeoff;
                            if (this.mn_time < 8.0F) canTakeoff = false;
                            
//                            if (((Aircraft) this.actor).aircIndex() == 0) System.out.println("Maneuver TAKEOFF mn_time:" + this.mn_time + ", getNearestFriendAtPoint:"
//                                    + (War.getNearestFriendAtPoint(Pd, (Aircraft) this.actor, this.fNearestDistance)==null?"null":War.getNearestFriendAtPoint(Pd, (Aircraft) this.actor, this.fNearestDistance).getClass().getName()) + ", hash:"
//                                            + (War.getNearestFriendAtPoint(Pd, (Aircraft) this.actor, this.fNearestDistance)==null?"null":""+War.getNearestFriendAtPoint(Pd, (Aircraft) this.actor, this.fNearestDistance).hashCode()) + "/" + (((Mistel)this.actor).getDrone()==null?"null":""+((Mistel)this.actor).getDrone().hashCode()));
                            
                            // TODO: CTO Mod
                            // ---------------------------------------------------------
                            
                            boolean test2 = canTakeoff;
                            
//                            // TODO: By SAS~Storebror: Hooked Mistel z-Offset ruins "getNearestFriendAtPoint" results, therefore remove that offset.
//                            if (this.actor instanceof Mistel && Actor.isValid(((Mistel)this.actor).getDrone())) {
//                                Pd.z -= this.Loc.z - this.actor.pos.getAbsPoint().z;
//                            }
                            
//                            if (((Aircraft) this.actor).aircIndex() == 0) {
//                                System.out.println("Maneuver TAKEOFF mn_time:" + this.mn_time + ", dist:" + Pd.distance(this.Loc));
//                            }
                            
                            if (this.actor != War.getNearestFriendAtPoint(Pd, (Aircraft) this.actor, this.fNearestDistance)) {
                                if (this.actor instanceof Mistel) {
                                    if (War.getNearestFriendAtPoint(Pd, (Aircraft) this.actor, this.fNearestDistance) != ((Mistel)this.actor).getDrone()) canTakeoff = false;
                                } else if (this.actor instanceof TypeDockable) {
                                    if (War.getNearestFriendAtPoint(Pd, (Aircraft) this.actor, this.fNearestDistance) != Reflection.invokeMethod(this.actor, "typeDockableGetDrone")) canTakeoff = false;
                                } else canTakeoff = false;
                            }
                            boolean test3 = canTakeoff;
                            // ---------------------------------------------------------
                            if (Actor.isAlive(this.AP.way.takeoffAirport) && this.AP.way.takeoffAirport.takeoffRequest > 0) canTakeoff = false;
//                            if (((Aircraft) this.actor).aircIndex() == 0) System.out.println("Maneuver TAKEOFF mn_time:" + this.mn_time + ", canTakeoff:" + test1 + "/" + test2 + "/" + test3 + "/" + canTakeoff + ", takeoffAirport:"
//                                    + (this.AP.way.takeoffAirport==null?"null":""+this.AP.way.takeoffAirport.hashCode()) + ", isAlive:" + Actor.isAlive(this.AP.way.takeoffAirport)
//                                    + ", takeoffRequest:" + this.AP.way.takeoffAirport.takeoffRequest);
                ...
                // TODO: +++ TD AI code backport from 4.13 +++
                boolean flag11 = true;
                if (this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_PAIRS || this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_LINE || this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_TAXI_FSP || this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_TAXI/* || ((Aircraft)actor).stationarySpawnLocSet */) flag11 = false;
                // TODO: --- TD AI code backport from 4.13 ---
                this.AFo.setDeg(this.Or.getAzimut(), this.direction);
                ...
                if (f151 > 120.0F * f28 || this.getSpeed() > this.Vmin * 1.8F * f28 || f151 > 80.0F * f28 && this.getSpeed() > this.Vmin * 1.6F * f28
                    ...
                    this.CT.FlapsControl = 0.0F;
//                    System.out.println(this.actor.name() + " Flaps=0 07");
                    this.CT.GearControl = 0.0F;
                    this.rwLoc = null;
                    if (this.actor instanceof TypeGlider) this.push(FOLLOW);
                    this.maneuver = NONE;
                    this.brakeShoe = false;
                    ...
                    this.pop();
                    // TODO: +++ TD AI code backport from 4.13 +++
                    if (this.Group != null && this.Group.numInGroup((Aircraft) this.actor) == 0 && this.Group.grTask == ATTACK_GROUND) this.Group.setGroupTask(WAIT);
                    if (this.AP.way.first().waypointType == WayPoint.WP_TAKEOFF_TAXI || this.AP.way.prev().waypointType == WayPoint.WP_TAKEOFF_TAXI_FSP) {
            ...
            case TAKEOFF_VTOL_A: {
                ...
                if (f161 > 200.0F) {
                    ...
                    this.CT.ElevatorControl = -1.0F;
                    this.CT.AirBrakeControl = 0.0F;
                    if (this.Or.getTangage() < 25.0F) this.maneuver = NONE;
                    this.brakeShoe = false;
                    this.turnOffCollisions = false;
                    if (this.CT.bHasCockpitDoorControl) this.AS.setCockpitDoor(this.actor, 0);
                    this.pop();
            ...
            case ZIGZAG_SPIT:
                ...
                switch (this.submaneuver) {
                    ...
                    case SUB_MAN1:
                        ...
                        if (this.danger != null) {
                            if (this.Skill >= VETERAN && this.Or.getTangage() < -30.0F && Vpl.x > 0.999D) this.submaneuver++;
                            if (this.Skill >= ACE && Math.abs(this.Or.getTangage()) > 145.0F && Math.abs(this.danger.Or.getTangage()) > 135.0F && this.dist < 400.0F) this.submaneuver++;
                        }
                        break;
                    case SUB_MAN2:
            ...
            case HARTMANN_REDOUT:
                ...
                break;
            case STALL_POKRYSHKIN:
            case BARREL_POKRYSHKIN:
                if (this.first) {
                    if (!this.isCapableOfACM()) this.pop();
                ...
                switch (this.submaneuver) {
                    ...
                    case SUB_MAN1:
                        ...
                        break;
                    case SUB_MAN2:
                        this.push(this.maneuver != STALL_POKRYSHKIN ? ROLL_360 : SPIRAL_BRAKE);
                        this.pop();
                }
                break;
            case SLIDE_LEVEL:
            ...
            case RANVERSMAN:
                this.push(SLIDE_DESCENT);
                this.push(GAIN);
                this.pop();
                break;
            case CUBAN:
                this.push(DIVING_45_DEG);
                this.push(HALF_LOOP_UP);
                this.pop();
                break;
            case CUBAN_INVERT:
            ...
            case BREAK_AWAY:
                ...
                this.minElevCoeff = 20F;
                this.dA = Math.abs(this.Or.getKren()) * (this.Group != null?this.Group.bracketSideGr:1);
                if (this.dA > 0.0F) this.dA -= 30F;
                else this.dA -= 330F;
                if (this.dA < -180F) this.dA += 360F;
                if (Math.abs(this.dA) > 60F) this.set_maneuver(PULL_UP);
            ...
            case GATTACK_KAMIKAZE:
                if (this.target_ground == null || this.target_ground.pos == null) {
                    ...
                    if (this.target_ground == null || this.target_ground.pos == null) {
                        ...
                        waypoint.Action = WayPoint.LANDING;
            ...
            case GATTACK_TORPEDO_TOKG:
                if (this.first && !this.isCapableOfACM()) {
                    ...
                    // TODO: +++ TD AI code backport from 4.13 +++
                    if (this.CT.dropWithPlayer != null && this.CT.dropWithPlayer.isAlive() && this.hasBombs() && this.maneuver == GATTACK) {
                        this.set_task(STAY_FORMATION);
                        this.clear_stack();
                        this.set_maneuver(FOLLOW);
                    }
                    ...
                    if (this.target_ground == null || this.target_ground.pos == null || !Actor.isAlive(this.target_ground)) {
                        int backupManeuver = this.maneuver;
                        if (this.Group != null) if (this.Group.grTask == DEFENCE) {
                            this.dont_change_subm = true;
                        ...
                        if (this.target_ground == null || this.target_ground.pos == null || !Actor.isAlive(this.target_ground)) {
                            if (backupManeuver == GATTACK_DIVE) {
                                this.bombsOut = true;
                                this.dumpBombsPassively = false;
                            }
                            if (this.Group != null) this.Group.waitGroup(this.Group.numInGroup((Aircraft) this.actor));
                                ...
                                this.AP.way.next();
                                // TODO: GATTACK set_task(FLY_WAYPOINT);
...
//                            System.out.println("Skipped Waypoint No. " + this.AP.way.Cur() + " since it's a GATTACK waypoint and this plane (" + this.actor.name() + ") has neither bombs nor rockets nor Gun/Cannon Ammo left");
                        ...
                        switch (this.maneuver) {
                            ...
                            case GATTACK:
                                ...
                                if (this.mn_time > 120.0F) this.set_maneuver(NONE);
                                break;
                            case GATTACK_DIVE:
                                ...
                                if (this.mn_time > 120.0F) {
                                    ...
                                    this.CT.AirBrakeControl = 0.0F;
                                    this.CT.FlapsControl = 0.0F;
        //                            System.out.println(this.actor.name() + " Flaps=0 08");
                                    this.pop();
                                    this.sub_Man_Count = 0;
                                }
                            break;
        ...
        this.strikeEmer = false;
        this.setSpeedControl(f);
        // TODO: +++ By SAS~Storebror: Keep Bombers, Transports, Gliders etc. from flipping over +++
        this.CT.AileronControl = this.limitBomberKren(this.CT.AileronControl, 0.5F);
        // TODO: --- By SAS~Storebror: Keep Bombers, Transports, Gliders etc. from flipping over ---
        // TODO: +++ TD AI code backport from 4.13 +++
        ...
        switch (this.submaneuver) {
            case SUB_MAN0:
                ...
                if (this.sub_Man_Count == 0) {
                    ...
                    double d1 = 0.0D;
                    if (d < 0.0001D || this.Group != null && this.Group.getAaaNum() > 8.5F && !(actor instanceof TgtShip)) {
                ...
                if (Ve.x < 0.0D && f3 < this.koeff) {
                    ...
                    Ve.normalize();
                    if (this.Loc.z < this.constVtarg1.z) this.setSpeedMode(MAX_SPEED);
                    else {
                        this.setSpeedMode(CONST_SPEED);
            ...
            case SUB_MAN1:
                ...
                this.Or.transformInv(Ve);
                Ve.normalize();
                if (this.Loc.z < this.constVtarg.z) this.setSpeedMode(MAX_SPEED);
                else {
                    this.setSpeedMode(CONST_SPEED);
                    this.smConstSpeed = 100F;
                }
            ...
            case SUB_MAN2:
                ...
                Ve.z += f8 + (this.Group != null?this.Group.getAaaNum():1F) * 0.5F;
                this.Or.transformInv(Ve);
                if (f5 < 800F && (this.shotAtFriend <= 0 || this.distToFriend > f5)) {
                    ...
                    if (Math.abs(Ve.y) < 15D && Math.abs(Ve.z) < 10D) {
                        if (f5 < 800F && this.Group != null && this.Group.getAaaNum() > 10F || f5 < 650F) this.CT.WeaponControl[0] = true;
                        if (f5 < 700F && this.Group != null && this.Group.getAaaNum() > 10F || f5 < 550F) this.CT.WeaponControl[1] = true;
                        // TODO: Storebror: +++ Bomb Release Bug hunting
//                        if (this.CT.Weapons[2] != null && this.CT.Weapons[2][0] != null && this.CT.Weapons[2][this.CT.Weapons[2].length - 1].countBullets() != 0 && this.rocketsDelay < 1 && f5 < 570F) {
                        if (this.CT.hasBulletsLeftOnTrigger(2) && this.rocketsDelay < 1 && f5 < 570F) {
                            ...
                            if (this.Group != null && this.Group.getAaaNum() > 2.0F) this.rocketsDelay += 30F - this.Group.getAaaNum();
                ...
                if (this.sub_Man_Count > 200 && Ve.x < this.getSpeed() * 2.4F || this.actor instanceof TypeStormovik && this.Alt < 80F || this.Alt < 40F) {
                    ...
                    this.push(CLIMB);
                    if (this.Group != null && this.Group.getAaaNum() > 3F) this.push(STRAIGHT_AND_LEVEL);
...
//        }
        ...
        } else if (!(this.actor instanceof TypeStormovik) && (!(this.actor instanceof TypeFighter) || MistelTools.isDockedQueen(this.actor)) && !(this.actor instanceof TypeDiveBomber)) {
            this.set_maneuver(NONE);
            return;
        }
        // TODO: Storebror: --- Bomb Release Bug hunting
...
//            if (this.CT.Weapons[3][0] instanceof BombGunTorp45_36AV_A)
        ...
        if ((this.actor instanceof TypeFighter  && !MistelTools.isDockedQueen(this.actor)) || this.actor instanceof TypeStormovik) {
...
//                if (this.CT.Weapons[3] != null && this.CT.Weapons[3][0] != null && this.CT.Weapons[3][0].countBullets() != 0 && !(this.CT.Weapons[3][0] instanceof BombGunPara))
    ...
    private void groundAttackDiveBomber(Actor actor, float f) {
        ...
        float f1 = (float) Math.sqrt(Ve.x * Ve.x + Ve.y * Ve.y);
        if (f1 > 1000F || this.submaneuver == SUB_MAN3 && this.sub_Man_Count > 100) {
            this.Vtarg.set(actor.pos.getAbsPoint());
            actor.getSpeed(tmpV3d);
            float f5 = 0.0F;
            f5 = this.Alt / 120F;
        ...
        if (this.submaneuver < SUB_MAN2) Ve.z = 0.0D;
        Vf.set(Ve);
        Ve.normalize();
        Vpl.set(this.Vwld);
        Vpl.normalize();
        switch (this.submaneuver) {
            ...
            case SUB_MAN1:
                ...
                this.CT.AirBrakeControl = 1.0F;
                if (this.actor instanceof TypeFighter && !MistelTools.isDockedQueen(this.actor)) this.CT.FlapsControl = 1.0F;
                this.push();
                this.push(ROLL_180);
                this.safe_pop();
                this.submaneuver++;
        ...
        this.Or.transformInv(Ve);
        Ve.normalize();
        if (this.submaneuver == SUB_MAN3) this.attackTurnToDirection(1000F, f, 30F);
        else if (this.submaneuver != SUB_MAN2) this.turnToDirection(f);
    }
...
//            TorpedoGun torpedogun = (TorpedoGun) this.CT.Weapons[3][0];
        ...
        if (this.Alt < f4 - 5F) {
            ...
            if (this.Alt < 8F) this.set_maneuver(PULL_UP);
            if (this.Alt < 20F && f2 < 75F) this.set_maneuver(PULL_UP);
        } else if (this.Alt > f4 + 5F && this.submaneuver == SUB_MAN1 && this.Vwld.z > 0.0D) this.Vwld.z--;
        switch (this.submaneuver) {
            default:
                break;
            ...
            case SUB_MAN1:
                this.setSpeedMode(CONST_SPEED);
                if (this.actor instanceof Swordfish) this.setSpeedMode(BOOST_ON);
                this.sub_Man_Count++;
...
//            TorpedoGun torpedogun = (TorpedoGun) this.CT.Weapons[3][0];
        ...
        if (this.Alt < f2 - 5F) {
            ...
            if (this.Alt < 8F) this.set_maneuver(PULL_UP);
            if (this.Alt < 20F && f1 < 75F) this.set_maneuver(PULL_UP);
        } else if (this.Alt > f2 + 5F && this.submaneuver == SUB_MAN1 && this.Vwld.z > 0.0D) this.Vwld.z--;
        switch (this.submaneuver) {
            default:
                break;
            ...
            case SUB_MAN2:
                ...
                if (d1 < 0.0D) d1 = 0.0D;
                if (this.Skill == VETERAN) d1 += 100D;
                if (f1 < f6 - d1 && f1 > 1800F && d < 0.2D) {
                    actor.getSpeed(tmpV3d);
                    float f7 = 1.0F;
                    if (this.Skill == VETERAN) f7 = World.Rnd().nextFloat(0.8F, 1.2F);
                    else if (this.Skill == ACE) f7 = World.Rnd().nextFloat(0.9F, 1.1F);
                    f7 = (float) (f7 + 0.1D);
                ...
                if (f1 < 1500F) {
                    ...
                    this.set_maneuver(GATTACK_TORPEDO);
                }
                break;
            ...
            case SUB_MAN3:
                ...
                this.push(SPIRAL_UP);
                this.pop();
                this.task = 61; // FIXME: This is weird. There is no task 61...
                this.sub_Man_Count++;
                boolean flag = false;
    ...
    private void groundAttackCassette(Actor actor, float f) {
        ...
        Ve.sub(this.Loc);
        float f1 = (float) Math.sqrt(Ve.x * Ve.x + Ve.y * Ve.y);
        if (this.submaneuver == SUB_MAN3 && this.sub_Man_Count > 0 && this.sub_Man_Count < 45 && f1 > 200F) {
            tmpV3f.set(this.Vxy);
        ...
        if (this.submaneuver == SUB_MAN0) Ve.set(1.0D, 0.0D, 0.0D);
        this.turnToDirection(f);
    }
    // TODO: --- TD AI code backport from 4.13 ---
...
//        f = 10.0F; // TESTING!
        ...
        switch (i) {
            ...
            case FVSB_BOOM_ZOOM_TO_ENGINE:
                ...
                this.TargZS = 0.43F * f;
                break;
            case FVSB_SHALLOW_DIVE_TO_ENGINE:
                // TODO: +++ TD AI code backport from 4.13 +++
    ...
    public void fighterVsBomber(float f) {
        ...
        switch (this.submaneuver) {
            case SUB_MAN0: {
                ...
                this.Or.transformInv(Ve);
                Ve.normalize();
                if (d3 > -200.0) this.setSpeedMode(BOOST_ON);
                else {
                    this.setSpeedMode(CONST_SPEED);
        ...
        switch (this.submaneuver) {
            ...
            case SUB_MAN1: {
                ...
                if (f2 > 3200.0F || this.Vtarg.z > 1500.0) {
                    ...
                    if (f2 < 600F) {
                        this.tailForStaying = this.target;
                        this.setSpeedMode(STAY_ON_THE_TAIL);
...
//                    if (f3 < 100F && this.CT.Weapons[2] != null && this.CT.Weapons[2][0] != null && this.CT.Weapons[2][this.CT.Weapons[2].length - 1].countBullets() != 0 && this.rocketsDelay < 1) {
        ...
        if (this.Alt > 5F + (4 - this.Skill) * 5 && Ve.x > 0.975D && f3 < this.convAI * 1.25F) {
            if (this.followType == LIVE) this.setSpeedMode(NOT_TOO_FAST);
            else if (this.followType == RETURN) this.setSpeedMode(STAY_ON_THE_TAIL);
            else this.setSpeedMode(BOOST_ON);
            this.tailForStaying = this.target;
    ...
    public void boomAttack(float f) {
        ...
        if (this.Vtarg.x > 0.0D && f3 < 700F && (this.shotAtFriend <= 0 || this.distToFriend > f3) && this.friendCheck(f3, this.target.Loc) && Math.abs(this.Vtarg.y) < 5.5F + 2.5F * this.gunnery && Math.abs(this.Vtarg.z) < 5.5D + 2.5D * this.gunnery
            ...
            if (this.maneuver == HIT_AND_RUN) this.gattackCounter++;
...
//        if (this.Group.airc[this.Group.nOfAirc - 1] == this.actor) System.out.println("Maneuver.attackTurnToDirection(" + f + ", " + f1 + ", " + f2 + ") Ve.x=" + Ve.x);
        ...
        if (Ve.x > 0.95) {
            ...
            if (this.Skill >= VETERAN && Ve.z > 0.5 && f < 600.0F) this.CT.FlapsControl = 0.1F;
            else {
                this.CT.FlapsControl = 0.0F;
//                System.out.println(this.actor.name() + " Flaps=0 09");
            }
    ...
    private void attackTurn(float f, float f1, float f2) {
        if (Ve.x < 0.01D && this.subSkill > 6 && this.maneuver == ATTACK && f < 800F && this.getMinHeightTurn(100F) < ((Maneuver) this.target).getMinHeightTurn(50F) + this.Skill * 0.33F) this.set_maneuver(ATTACK_HARD);
        ...
        if (Ve.x > 0.95D) {
            ...
            if (this.Skill >= VETERAN && Ve.z > 0.5D && f < 600F) this.CT.FlapsControl = 0.1F;
            else {
                this.CT.FlapsControl = 0.0F;
//                System.out.println(this.actor.name() + " Flaps=0 10");
            }
    ...
    private void doCheckStrike() {
        ...
        if (!(this.actor instanceof Aircraft)) return;
        if (!Actor.isValid(this.actor)) return;
        if (!(this.strikeTarg instanceof Maneuver)) return;
        ...
        Vpl.sub(this.strikeTarg.Loc, this.Loc);
        double d = this.strikeTarg.Loc.distance(this.Loc);
        tmpV3f.set(Vpl);
        this.Or.transformInv(tmpV3f);
        double criticalDistance = this.actor == null?20D:this.actor.collisionR();
        criticalDistance += this.strikeTarg == null||this.strikeTarg.actor == null?20D:this.strikeTarg.actor.collisionR();
//        criticalDistance *= 1.5D;
        Aircraft nearestAircraft = War.getNearestFriend((Aircraft)this.actor, (float)criticalDistance);
        if (tmpV3f.x < 0D || this.maneuver == GO_AROUND) {
            if (nearestAircraft != null && nearestAircraft != this.strikeTarg.actor) {
                FlightModel tmpStrikeTarg = this.strikeTarg;
                this.strikeTarg = nearestAircraft.FM;
                this.doCheckStrike();
                this.strikeTarg = tmpStrikeTarg;
            }
            if (this.maneuver != GO_AROUND) {
//                if (d < criticalDistance * 1.5D && this.actor.name().equalsIgnoreCase("usa01013")) System.out.println("Checking Strike skipped (tmpV3f.x < 0): " + (this.actor == null?"null":this.actor.name()) + " & " + (this.strikeTarg == null || this.strikeTarg.actor == null?"null":this.strikeTarg.actor.name()) + ", criticalDistance=" + criticalDistance + ", distance=" + d + ", TaxiMode=" + this.TaxiMode + ", Gears.onGround=" + (this.Gears == null?"null":""+this.Gears.onGround()) + ", maneuver=" + this.maneuver + ", nearestAircraft=" + (nearestAircraft == null?"null":nearestAircraft.name()));
                return;
            }
        }
        tmpV3f.sub(this.strikeTarg.Vwld, this.Vwld);
...
//        if (d < criticalDistance * 1.5D && this.actor.name().equalsIgnoreCase("usa01013")) System.out.println("Checking Strike: " + (this.actor == null?"null":this.actor.name()) + " & " + (this.strikeTarg == null || this.strikeTarg.actor == null?"null":this.strikeTarg.actor.name()) + ", criticalDistance=" + criticalDistance + ", distance=" + d + ", TaxiMode=" + this.TaxiMode + ", Gears.onGround=" + (this.Gears == null?"null":""+this.Gears.onGround()) + ", maneuver=" + this.maneuver);
        if (this.maneuver == GO_AROUND) {
            if (d < criticalDistance * 1.5D && Vpl.z > -20D) {
//                if (this.actor.name().equalsIgnoreCase("usa01002")) System.out.println("!!! " + (this.actor == null?"null":this.actor.name()) + " Emergency go around collision avoidance with " + (this.strikeTarg == null || this.strikeTarg.actor == null?"null":this.strikeTarg.actor.name()) + ", Vwld.z=" + Vwld.z + " !!!");
//                this.Vwld.x *= 0.99D;
//                this.Vwld.y *= 0.99D;
//                this.Vwld.z = 0.5D; //this.Vwld.z * 0.95D + 0.5D * 0.05D;
                if (this.strikeTarg.getSpeed() < this.VminFLAPS && tmpV3f.x >= 0D) {
                    double newZ = Math.max(this.Loc.z, Aircraft.cvt((float)d, (float)criticalDistance, (float)criticalDistance * 1.5F, (float)this.strikeTarg.Loc.z + 20F, (float)this.strikeTarg.Loc.z));
                    this.Vwld.z = Math.max(this.Vwld.z, (newZ - this.Loc.z) * 10D);
                    this.Loc.z = newZ;
                } else {
                    double stXySquared = this.strikeTarg.Vwld.x * this.strikeTarg.Vwld.x + this.strikeTarg.Vwld.y * this.strikeTarg.Vwld.y;
                    double thisXySquared = this.Vwld.x * this.Vwld.x + this.Vwld.y * this.Vwld.y;
                    if (thisXySquared > stXySquared && tmpV3f.x >= 0D) {
                        this.Vwld.scale(Aircraft.cvt((float)d, (float)criticalDistance, (float)criticalDistance * 1.5F, (float)(stXySquared / thisXySquared), 1.0F));
                    }
//                    this.Vwld.x = Aircraft.cvt((float)d, (float)criticalDistance, (float)criticalDistance * 1.5F, (float)this.strikeTarg.Vwld.x, (float)this.Vwld.x);
//                    this.Vwld.y = Aircraft.cvt((float)d, (float)criticalDistance, (float)criticalDistance * 1.5F, (float)this.strikeTarg.Vwld.y, (float)this.Vwld.y);
                    float direction = Vpl.z > 0D && this.getAltitudeAboveGround() > 300F?-1F:1F;
                    this.Vwld.z = Aircraft.cvt((float)d, (float)criticalDistance, (float)criticalDistance * 1.5F, 10F * direction, 2.5F * direction); //this.Vwld.z * 0.95D + 0.5D * 0.05D;
//                    if (this.actor.name().equalsIgnoreCase("usa01013")) System.out.println("GO_AROUND Vwld.z=" + this.Vwld.z);
                }
            }
            this.safe_pop();
            return;
        }
//        if (this.Gears.nOfGearsOnGr > 0) { // || (this.strikeTarg != null && this.strikeTarg.Gears != null && this.strikeTarg.Gears.onGround())) {
        if (this.Gears.onGround() && this.strikeTarg != null && this.strikeTarg.Gears != null && this.strikeTarg.Gears.onGround() && this.getSpeed() > 0.1F) {
            if (d < criticalDistance * 1.5D) {
//                if (this.actor.name().equalsIgnoreCase("usa01002")) System.out.println("!!! " + (this.actor == null?"null":this.actor.name()) + " Slow down on ground, collision avoidance with " + (this.strikeTarg == null || this.strikeTarg.actor == null?"null":this.strikeTarg.actor.name()) + " !!!");
                this.Vwld.x = Aircraft.cvt((float)d, (float)criticalDistance, (float)criticalDistance * 1.5F, (float)this.strikeTarg.Vwld.x, (float)this.Vwld.x);
                this.Vwld.y = Aircraft.cvt((float)d, (float)criticalDistance, (float)criticalDistance * 1.5F, (float)this.strikeTarg.Vwld.y, (float)this.Vwld.y);
//                this.Vwld.scale(Aircraft.cvt((float)d, (float)criticalDistance, (float)criticalDistance * 1.5F, 0.0F, 0.995F));
            }
            this.safe_pop();
            return;
        }
        if (this.maneuver == LANDING && this.CT.GearControl == 1.0F) {
            if (d < criticalDistance * 1.5D && this.strikeTarg instanceof Maneuver && ((Maneuver)this.strikeTarg).maneuver == LANDING/* && !this.Gears.nearGround()*/) {
//                this.Vwld.z += 0.3D;
//                if (this.actor.name().equalsIgnoreCase("usa01002")) System.out.println("!!! " + (this.actor == null?"null":this.actor.name()) + " Emergency go around because of imminent collision with " + (this.strikeTarg == null || this.strikeTarg.actor == null?"null":this.strikeTarg.actor.name()) + ", Speed=" + this.getSpeed() + ", Vmin=" + this.Vmin  + ", VmaxFLAPS=" + this.VmaxFLAPS + " !!!");
//                if (this.Group != null) this.Group.delAircraft((Aircraft)this.actor);
                this.AP.way.first();
                this.push(GO_AROUND);
                this.pop();
                ...
                if (this.getSpeed() < this.Vmin * 1.5F && this.getSpeed() < this.VmaxFLAPS) {
                    this.CT.FlapsControl = 0.4F;
//                    System.out.println((this.actor == null?"null":this.actor.name()) + " Flaps set !!!");
                }
                return;
            }
        } else
        if (Vpl.z > 0.0D && this.Alt > 300F && this.forceEvade != EVADE_UP) {
//            if (this.actor.name().equalsIgnoreCase("usa01002")) System.out.println("!!! " + (this.actor == null?"null":this.actor.name()) + " Evade DOWN to avoid collision with " + (this.strikeTarg == null || this.strikeTarg.actor == null?"null":this.strikeTarg.actor.name()) + " !!!");
            this.push(EVADE_DN);
            if (this.strikeTarg instanceof Maneuver) ((Maneuver)this.strikeTarg).forceEvade = EVADE_UP;
            this.forceEvade = NONE;
        } else {
//            if (this.actor.name().equalsIgnoreCase("usa01002")) System.out.println("!!! " + (this.actor == null?"null":this.actor.name()) + " Evade " + (this.forceEvade == EVADE_DN && this.Alt > 300F?"DOWN":"UP") + " to avoid collision with " + (this.strikeTarg == null || this.strikeTarg.actor == null?"null":this.strikeTarg.actor.name()) + " !!!");
            this.push(this.forceEvade == EVADE_DN && this.Alt > 300F?EVADE_DN:EVADE_UP);
            if (this.strikeTarg instanceof Maneuver) ((Maneuver)this.strikeTarg).forceEvade = EVADE_DN;
            this.forceEvade = NONE;
        }
        // TODO: --- Avoid mid air collision - by SAS~Storebror ---
        this.safe_pop();
    ...
    public void setStrikeTarget(FlightModel flightmodel) {
         this.strikeTarg = flightmodel;
    }
...
//                if (f1 < -22F) f1 = -22F;
//                if (f1 > 57F) f1 = 57F;
//            } else if (this.maneuver == FISHTAIL_LEFT) {
//                f1 = 0.0F;
//                f2 = 107F;
//            } else if (this.maneuver == FISHTAIL_RIGHT) {
//                f1 = 0.0F;
//                f2 = -107F;
//            } else if (this.maneuver == LOOKDOWN_LEFT) {
//                f1 = 0.0F;
//                f2 = 90F;
//            } else if (this.maneuver == LOOKDOWN_RIGHT) {
//                f1 = 0.0F;
//                f2 = -90F;
//            } else if (this.get_maneuver() == PILOT_DEAD) {
//                f2 = -15F;
//                f1 = -15F;
//            } else {
//                f1 = 0.0F;
...
//            if (this.CT.Weapons[3] != null && this.CT.Weapons[3][0] != null) {
    ...
    private boolean doGroundCheckFront(float f) {
        if (this.maneuver == PULL_UP && (this.Or.getPitch() < 360F || this.Vrel.z < 0.0D)) return false;
        ...
        if (f2 > 500F) f2 = 500F;
        float f3 = 1.0F;
        if (this.maneuver == FOLLOW) f3 = 0.5F;
        else if (this.task == FLY_WAYPOINT) f3 = 1.5F;
        float f4 = (f1 * 20F * f - Math.abs(f2)) * f3;
        if (f4 < 800F) f4 = 800F;
        if (this.maneuver == WAYPOINT) {
    ...
    protected void doCheckGround(float f) {
        ...
        float f5 = 10.0F;
        float f6 = 80.0F;
        if (this.maneuver == FOLLOW) {
            f5 = 15.0F;
            f6 = 120.0F;
        }
        // TODO: +++ TD AI code backport from 4.13 +++
        if (this.Alt < 2000F && this.isTick(16, 0)) if (this.doGroundCheckFront(f4)) return;
        if (this.maneuver == GAIN || this.maneuver == PULL_UP) return;
        // TODO: --- TD AI code backport from 4.13 ---
        ...
        if (-this.Vwld.z * 2.5D > this.Alt || this.Alt < 20F / (this.Skill + 1)) {
            if (this.maneuver == ATTACK || this.maneuver == GATTACK || this.maneuver == WAYPOINT || this.maneuver == FOLLOW || this.maneuver == FAR_COVER || this.maneuver == FAR_FOLLOW || this.maneuver == COVER || this.maneuver == COVER_AGRESSIVE || this.maneuver == COVER_DRAG_AND_BAG || this.maneuver == ATTACK_FROM_PLAYER) // TODO:
        // --- TD AI code backpor from 4.13 ---
            this.push();
            this.push(PULL_UP);
            this.pop();
            this.submaneuver = SUB_MAN0;
...
//        if (this.Group.airc[this.Group.nOfAirc - 1] == this.actor) System.out.println("Maneuver.setSpeedControl(" + f + "), speedMode=" + this.speedMode);
        float f1 = 0.8F;
        float f4 = 0.02F;
        float f5 = 1.5F;
            ...
            case FOLLOW_WITHOUT_FLAPS:
                if (this.tailForStaying == null) f1 = 1.0F;
                    ...
                    if (f7 > 0.0F && f11 < 1000.0F) {
                        ...
                        float f13 = f11 > 0.0F?0.0070F * f11 + 0.0050F * f15:0.03F * f11 + 0.0010F * f15;
                        float f24 = this.getSpeed() - this.tailForStaying.getSpeed();
//                        if (this.actor.name().equalsIgnoreCase("usa01002")) System.out.println(this.actor.name() + " distance to target pos = " + f11);
                        // TODO: +++ By SAS~Storebror: Make AI catch up faster when falling behind slot position!
//                      float f26 = -0.3F * f24;
                        float f26 = -Aircraft.cvt(f11, 0F, 10F, 0.3F, 0.1F) * f24;
                        // ---
        ...
        float f26 = f22 * 15F;
        int c = 0x800;
        if (this.Skill >= VETERAN) c = 0x400;
        if (f22 > f142) {
            if (f22 > 0.0F && this.speedMode != FOLLOW_WITHOUT_FLAPS) f1 *= 1.0F - f26;
...
// TODO: --- TD AI code backport from 4.13 ---
    ...
    private void emergencyLanding(float f) {
        ...
        if (this.Alt < 10.0F) {
            ...
            if (this.Alt < 5.0F) this.setSpeedMode(ZERO_POWER);
...
//        }
//        if (this.actor.name().equalsIgnoreCase("usa01002")) {
//            Exception test = new Exception(this.actor.name() + " set_maneuver_imm(" + i + ")");
//            test.printStackTrace();
//        }
        int j = this.maneuver;
        this.maneuver = i;
    ...
    public boolean canHideInClouds() {
        if (Mission.curCloudsType() >= 4) {
            float f = 1.0F;
            if (this.actor instanceof TypeFighter && !MistelTools.isDockedQueen(this.actor)) f = 2.0F;
    ...
    private void checkBlindSpots() {
        if (this.maneuver == WAYPOINT && (this.Group == null || this.Group.nOfAirc == 1) || this.maneuver == FOLLOW && (this.Group == null || this.Group.airc != null && this.Group.airc[this.Group.nOfAirc - 1] == this.actor)) {
            if (this.formationType == Formation.F_LINEABREAST) return;
            if (MistelTools.isDockedQueen(this.actor)) return; // Keep Mistels from Tail swaying / checking for blind spots
            if (this.Group != null && this.Group.nOfAirc > 4) return;
                ...
                if (this.lookAroundTime < Time.current()) {
                    if (Math.abs(this.Or.getRoll() - 360F) < 10F && Math.abs(this.Or.getPitch() - 360F) < 20F) {
                        ...
                        if (byte0 < byte1) switch (World.Rnd().nextInt(byte0, byte1)) {
                            case 2:
                                this.push();
                                this.push(LOOKDOWN_LEFT);
                                this.pop();
                                break;

                            case 3:
                                this.push();
                                this.push(LOOKDOWN_RIGHT);
                                this.pop();
                                break;
    ...
    private boolean isStopOnStalemate(Aircraft aircraft) {
        ...
        if (this.Group == ((Maneuver) aircraft.FM).Group) {
            int i = this.Group != null?this.Group.numInGroup((Aircraft) this.actor):1;
            int j = this.Group != null?this.Group.numInGroup(aircraft):1;
            return i > j;
        }
    ...
    // TODO: +++ Added by SAS~Storebror: Keep Bombers etc. from flipping over +++
    private float limitBomberKren(float aileronInput, float maxAileron) {
//        if (true) return aileronInput;
        if (this.actor instanceof TypeFighter) return aileronInput;
        if (this.actor instanceof TypeTNBFighter) return aileronInput;
        if (this.actor instanceof TypeBNZFighter) return aileronInput;
        if (this.actor instanceof TypeSupersonic) return aileronInput;
        if (this.actor instanceof TypeSupersonic) return aileronInput;
        if (this.actor instanceof TypeFastJet) return aileronInput;
        if (this.actor instanceof TypeStormovikArmored) return aileronInput;
        if (this.actor instanceof TypeDiveBomber) return aileronInput;
        boolean applicable = false;
        if (this.actor instanceof TypeBomber) applicable = true;
        if (this.actor instanceof TypeGlider) applicable = true;
        if (this.actor instanceof TypeTransport) applicable = true;
        if (MistelTools.isDockedQueen(this.actor)) applicable = true;
        if (!applicable) return aileronInput;
        float kren = this.Or.getKren();
        while (kren < -180.0F) kren += 360.0F;
        while (kren > 180.0F) kren -= 360.0F;
        if (Math.abs(kren) < 25F) return aileronInput;
        kren = clamp11(-0.04F * kren);
        if (kren < -maxAileron) kren = -maxAileron;
        else if (kren > maxAileron) kren = maxAileron;
        if (Math.abs(aileronInput) < Math.abs(kren)) return aileronInput;
        return kren;
    }
    // TODO: --- Added by SAS~Storebror: Keep Bombers etc. from flipping over ---
}