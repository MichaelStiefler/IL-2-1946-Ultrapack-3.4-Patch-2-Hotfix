...
public class Pilot extends Maneuver {
    ...
    protected void setPriorities() {
        ...
        if (this.get_maneuver() == Maneuver.GATTACK_KAMIKAZE) {
            this.setBusy(true);
        ...
        if (this.get_maneuver() == Maneuver.PILOT_DEAD || this.get_maneuver() == Maneuver.LANDING && this.AP.way.Cur() > 6 || this.get_maneuver() == Maneuver.EMERGENCY_LANDING || this.get_maneuver() == Maneuver.TAKEOFF || this.get_maneuver() == Maneuver.PARKED_STARTUP || this.get_maneuver() == Maneuver.TAXI_TO_TO || this.get_maneuver() == Maneuver.PULL_UP
                || this.get_maneuver() == Maneuver.PULL_UP_EMERGENCY || this.get_maneuver() == Maneuver.GAIN || this.get_maneuver() == Maneuver.EVADE_UP || this.get_maneuver() == Maneuver.EVADE_DN) {
            this.setBusy(true);
            this.dontSwitch = true;
            return;
        }
//        System.out.println("" + this.getDangerAggressiveness() + " " + (1.0F - 0.12F * this.Skill));
        if (this.getDangerAggressiveness() > (1.0F - 0.12F * this.Skill) * World.Rnd().nextFloat(0.95F, 1.01F) && this.danger != null && (this.danger.actor instanceof TypeFighter || this.danger.actor instanceof TypeStormovik) && !MistelTools.isDockedQueen(this.danger.actor) && this.danger.isOk()) {
            if (this.courage <= 1 && World.Rnd().nextInt(0, 30000) < 2 - this.courage) {
                this.setBusy(true);
                ((Aircraft) this.actor).hitDaSilk();
                this.set_task(Maneuver.NO_TASK);
                if (this.Group != null) this.Group.delAircraft((Aircraft) this.actor);
            }
            if (this.get_task() != Maneuver.DEFENCE) {
                this.set_task(Maneuver.DEFENCE);
                this.clear_stack();
                this.set_maneuver(Maneuver.NONE);
                if (this.actor instanceof TypeStormovik && this.Group != null) {
                    ...
                    if (((Aircraft) this.danger.actor).aircNumber() < this.Group.nOfAirc && !this.hasBombs() && this.Group.nOfAirc >= l + 2) {
                        ...
                        this.set_task(Maneuver.ATTACK_AIR);
                        this.clear_stack();
                        maneuver.target = this.danger;
                        this.set_maneuver(Maneuver.ATTACK);
                        this.setBusy(true);
                        return;
                    }
                }
        ...
        if (this.isReadyToReturn() && !this.AP.way.isLanding()) {
            if (this.Group != null && this.Group.grTask != Maneuver.WAIT) {
        ...
        if (this.get_task() == Maneuver.ATTACK_AIR) {
        ...
        if (this.get_task() == Maneuver.ATTACK_GROUND && this.target_ground != null && this.get_maneuver() != Maneuver.NONE) {
            this.setBusy(true);
            return;
        }
        if (this.get_task() == Maneuver.FLY_WAYPOINT && this.AP.way.isLanding()) this.setBusy(true);
    }
    ...
    private void setTaskAndManeuver() {
        ...
        if (!this.isBusy()) {
            if ((this.wasBusy || this.get_maneuver() == Maneuver.NONE) && this.Group != null) {
                this.clear_stack();
                this.Group.setTaskAndManeuver(this.Group.numInGroup((Aircraft) this.actor));
            }
        } else if (this.get_maneuver() == Maneuver.NONE) {
            this.clear_stack();
    ...
    public void setManeuverByTask() {
        ...
        switch (this.get_task()) {
            ...
            case Maneuver.STAY_FORMATION:
                if (this.Leader != null && Actor.isValid(this.Leader.actor) && !this.Leader.isReadyToReturn() && !this.Leader.isReadyToDie() && this.Leader.getSpeed() > 35F) this.set_maneuver(Maneuver.FOLLOW);
                else this.set_task(Maneuver.FLY_WAYPOINT);
                break;

            case Maneuver.FLY_WAYPOINT:
                this.set_maneuver(Maneuver.WAYPOINT);
                ...
                while (pilot.Wingman != null) {
                    ...
                    if (!pilot.AP.way.isLanding() && pilot.get_task() == Maneuver.FLY_WAYPOINT) pilot.set_task(Maneuver.STAY_FORMATION);
                }
                if (((Aircraft) this.actor).aircIndex() == 0 && this.Speak5minutes == 0 && i == WayPoint.GATTACK && f < 30000D) {
            ...
            case Maneuver.DEFENCE:
                if (this.get_maneuver() == Maneuver.NONE) this.set_maneuver(Maneuver.WAYPOINT);
                if (this.danger == null) {
                    this.set_task(Maneuver.FLY_WAYPOINT);
                    break;
                }
                if (this.actor instanceof TypeFighter && !MistelTools.isDockedQueen(this.actor)) {
                    this.bombsOut = true;
                    this.CT.dropFuelTanks();
                }
                ...
                vecDanger.normalize();
                if (this.onBack && this.near && this.danger instanceof TypeFighter && !MistelTools.isDockedQueen(this.danger) && (this.actor instanceof TypeTransport || this.Wingman == null || this.killed(this.Wingman) || ((Pilot) this.Wingman).target != this.danger)) if (this.isLeader()) {
                    if ((this.actor instanceof TypeFighter || this.actor instanceof TypeStormovik && this.Skill > 1 && this.AP.way.curr().Action == 0) && !MistelTools.isDockedQueen(this.actor) && this.Wingman != null && !this.killed(this.Wingman) && !((Pilot) this.Wingman).requestCoverFor(this))
                    ...
                    if (aircraft1 instanceof TypeFighter && !MistelTools.isDockedQueen(aircraft1) && aircraft1.FM instanceof Pilot) ((Pilot) aircraft1.FM).requestCoverFor(this);
                }
//                this.target = this.danger; // TODO: AI TEST, didn't work.
            ...
            case Maneuver.DEFENDING:
                if (this.airClient != null && !this.killed(this.airClient)) {
                    ...
                    this.set_maneuver(Maneuver.COVER);
                    break;
                }
                this.airClient = null;
            ...
            case Maneuver.ATTACK_AIR:
                ...
                if (this.actor instanceof TypeFighter && !MistelTools.isDockedQueen(this.actor) && !this.bKeepOrdnance) {
                    this.bombsOut = true;
                    this.CT.dropFuelTanks();
                }
                if (this.target == null || !this.hasCourseWeaponBullets()) {
                    if (this.AP.way.curr().Action == 3) {
                        this.set_task(Maneuver.ATTACK_GROUND);
                        this.set_maneuver(Maneuver.NONE);
                        break;
                    }
                    this.set_task(Maneuver.FLY_WAYPOINT);
                    if (this.Leader == null) this.set_maneuver(21);
                    else this.set_maneuver(Maneuver.FOLLOW);
                    break;
                }
                int j = ((Aircraft) this.actor).aircIndex();
                if (this.target.actor instanceof TypeBomber || MistelTools.isDockedQueen(this.target.actor)) {
                    this.attackBombers();
                    break;
                }
            ...
            case Maneuver.ATTACK_GROUND:
                ...
                if (this.CT.Weapons[3] != null && this.CT.Weapons[3][0] != null && this.CT.Weapons[3][0].countBullets() != 0) {
                    if (this.CT.Weapons[3][0] instanceof ParaTorpedoGun) {
                        this.set_maneuver(Maneuver.GATTACK);
                        break;
                    }
                    if (this.CT.Weapons[3][0] instanceof TorpedoGun) {
                        if (this.target_ground instanceof TgtShip) {
                            if (this instanceof TypeHasToKG && this.Skill >= 2) this.set_maneuver(Maneuver.GATTACK_TORPEDO_TOKG);
                            else this.set_maneuver(Maneuver.GATTACK_TORPEDO);
                        } else this.set_maneuver(Maneuver.GATTACK);
                        break;
                    }
                    ...
                    if (this.CT.Weapons[3][0].bulletMassa() < 5F) {
                        this.set_maneuver(Maneuver.GATTACK_CASSETTE);
                        break;
                    }
                    if (this.actor instanceof TypeDiveBomber && this.Alt > 1200F) this.set_maneuver(Maneuver.GATTACK_DIVE);
                    else this.set_maneuver(Maneuver.GATTACK);
                    break;
                }
                ...
                if ((this.actor instanceof TypeFighter || this.actor instanceof TypeStormovik) && !MistelTools.isDockedQueen(this.actor)) {
                    this.set_maneuver(Maneuver.GATTACK);
                    break;
                }
                this.set_task(Maneuver.STAY_FORMATION);
            ...
            case Maneuver.NO_TASK:
                if (this.isReadyToDie()) this.set_maneuver(Maneuver.EMERGENCY_LANDING);
                break;

            case Maneuver.WAIT:
                this.set_maneuver(Maneuver.HANG_ON);
                break;
        }
    }
    ...
    public boolean requestCoverFor(FlightModel flightmodel) {
        ...
        if (this.danger == null || ((Pilot) this.danger).target != this || this.danger.Loc.distance(this.Loc) > 600D + 200D * this.Skill || this.danger.actor instanceof TypeStormovik || this.danger.actor instanceof TypeBomber || MistelTools.isDockedQueen(this.danger.actor)) {
    ...
    public void setAsDanger(Actor actor) {
        ...
        if (!Actor.isValid(this.actor)) {
            if (World.cur().isArcade()) {
                ...
                if (actor instanceof TypeFighter && !MistelTools.isDockedQueen(actor) && ((Aircraft) actor).FM instanceof Pilot) ((Pilot) ((Aircraft) actor).FM).set_maneuver(35);
            }
        ...
        if (actor instanceof Aircraft) {
            if (actor instanceof TypeFighter && !MistelTools.isDockedQueen(actor)) {
            ...
            if (this.actor instanceof TypeFighter && !MistelTools.isDockedQueen(this.actor) && this.get_task() != 4) {
    ...
    private void transportDefence() {
        int numOperableTurrets = 0;
        for (int j = 0; j < this.turret.length; j++)
            if (this.turret[j].bIsOperable) numOperableTurrets++;
        
        // TODO: By SAS~Storebror: Avoid breaking up bomber formations!
        if (this.Group != null && this.Group.nOfAirc > 1 && numOperableTurrets > 0) return;
        // ---

        int aircNumber = ((Aircraft) this.danger.actor).aircNumber();
        int nextManeuver = this.doTransportDefense((float) vecDanger.x, this.dist, this.Alt, this.Skill, this.courage, this.flying, numOperableTurrets, aircNumber);

        // TODO: Cheater Protection +++
        if (nextManeuver == Maneuver.WAYPOINT || nextManeuver == Maneuver.FOLLOW) if (AircraftState.isCheaterAdvancedTransportDefence(this.danger.actor)) {
            ...
            if ((float) onMe.x > onMeThreshold && this.dist < 1500F) switch (TrueRandom.nextInt(5)) {
                case 0:
                    nextManeuver = Maneuver.EVADE_UP;
                    break;
                case 1:
                    nextManeuver = Maneuver.EVADE_ATTACK;
                    break;
                case 2:
                    nextManeuver = Maneuver.PULL_UP_EMERGENCY;
                    break;
                case 3:
                    nextManeuver = Maneuver.IVAN;
                    break;
                case 4:
                    nextManeuver = Maneuver.PANIC_MANIC;
                    break;
            }
//                    System.out.println("" + Group.numInGroup((Aircraft)this.actor) + " " + l + " " + onMeThreshold + " " + (float) onMe.x + " " + this.dist);
        }

        // TODO: By SAS~Storebror: Regroup Formation if leader is under attack (and breaks away), undock if no friendly fighter is available!
        if (nextManeuver != Maneuver.NONE && nextManeuver != Maneuver.WAYPOINT && nextManeuver != Maneuver.FOLLOW && nextManeuver != Maneuver.LANDING && nextManeuver != Maneuver.TAKEOFF/* && MistelTools.isDockedQueen(this.actor)*/) {
            if (this.Group != null && this.Group.numInGroup((Aircraft)this.actor) == 0) {
//              System.out.println("transportDefence() delete Mistel from Group since it's docked and it's the group leader!");
//              this.Group.delAircraft((Aircraft)this.actor);
                System.out.println("transportDefence() make Aircraft the last Group element since it's under attack and it's the group leader!");
//                this.Group.changeAircraft((Aircraft)this.actor, this.Group.airc[this.Group.nOfAirc - 1]);
//                this.Group.swapAircraft(0, this.Group.nOfAirc - 1);
                this.Group.delAircraft(this.Group.airc[0]);
            }
            
//            if (War.getNearestFriendlyFighter((Aircraft)this.actor, 3000F) == null) {
//                System.out.println("transportDefence() detach Mistel Drone because we are under attack and there is no friendly fighter nearby!");
//                ((Mistel)this.actor).setBombChargeArmed(false);
//                ((TypeDockable)this.actor).typeDockableAttemptDetach();
//                this.set_task(Maneuver.DEFENCE);
//                this.setManeuverByTask();
//            }
        }
//        if (this.Group != null && this.Group.nOfAirc > 1 && numOperableTurrets > 0) return;
        ...
        this.set_maneuver(nextManeuver);
    }
    ...
    private void fighterDefence() {
        // TODO: Avoid Mistels to immediately detach when fighters come around +++
//        if (this.bKeepOrdnance) this.bKeepOrdnance = false;
        if (this.bKeepOrdnance && (!(this.actor instanceof Mistel) || ((Mistel)this.actor).getDrone() == null || !this.isCapableOfACM())) this.bKeepOrdnance = false;
        // ---
    ...
    private boolean isDiveBetterThanEnemy() {
        return this.Alt > 2000F && this.actor instanceof TypeBNZFighter && !MistelTools.isDockedQueen(this.actor) && this.VmaxAllowed > this.danger.VmaxAllowed + 10F;
    }
