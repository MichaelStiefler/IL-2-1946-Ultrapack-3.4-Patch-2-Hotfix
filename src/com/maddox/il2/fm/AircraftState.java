...
import com.maddox.il2.objects.air.BF_109F4MSTL;
...
//    public static final int _AS_LENGTH               = 54;
    ...
    public static final int       MAX_ENGINES_FOR_UNCOMPRESSED_REPLICATION = 6;
...
//        if (this.astateTankStates[tankNo] < 5 && newTankState >= 5) {
        if (oldTankState <= 5 && newTankState > 5) {
        ...
        for (int tankStateEffectIndex = 0; tankStateEffectIndex < 3; tankStateEffectIndex++) {
            ...
            int astateTankStringsIndex = tankStateStringIndex + tankStateEffectIndex + newTankState * 3;
            if (astateTankStringsIndex < 0 || astateTankStringsIndex >= astateTankStrings.length) continue;
...
//        System.out.println("AircraftState doSetEngineState(" + (engineOwner==null?"null":engineOwner.getClass().getName()) + ", " + engineIndex + ", " + newEngineState + ")");
...
//			) this.aircraft.FM.CT.BayDoorControl = 0.0F;
    ...
    public void netReplicate(NetMsgGuaranted netMsgGuaranted) throws IOException {
        int numEngines = this.aircraft.FM.EI.getNum();
        if (this.aircraft instanceof FW_190A8MSTL) numEngines = 1;
        if (this.aircraft instanceof BF_109F4MSTL) numEngines = 1;
        if (this.aircraft instanceof JU_88MSTL) numEngines = 2;
        NetMsgGuaranted saveNetMsgGuaranted = netMsgGuaranted;
        if (numEngines > MAX_ENGINES_FOR_UNCOMPRESSED_REPLICATION) {
...
//            System.out.println("netReplicate " + this.aircraft.name() + " (" + this.aircraft.getClass().getName() + ") astateEngineStates[" + i+ "]=" + this.astateEngineStates[i]);
...
////        for (int nmgIdx=0; nmgIdx<netMsgGuaranted.dataLength(); nmgIdx++) sb.append(Integer.toHexString(netMsgGuaranted.data()[nmgIdx]) + " ");
//        System.out.println("netReplicate " + this.aircraft.name() + " (" + this.aircraft.getClass().getName() + ") netMsgGuaranted:");
////        System.out.println(sb.toString());
//        System.out.println(bytesToHex(netMsgGuaranted.data()));
//        // TODO: Netmsg TEST!!! ---
...
//        // TODO: Netmsg TEST!!! +++
//        int avail = myNetMsgInput.available();
//        myNetMsgInput.fixed();
////        StringBuffer sb = new StringBuffer();
////        for (int nmgIdx=0; nmgIdx<avail; nmgIdx++) sb.append(Integer.toHexString(netMsgInput.readUnsignedByte()) + " ");
//        System.out.println("netFirstUpdate " + this.aircraft.name() + " (" + this.aircraft.getClass().getName() + ") myNetMsgInput:");
////        System.out.println(sb.toString());
//        byte[] data = new byte[avail];
//        myNetMsgInput.readFully(data);
//        System.out.println(bytesToHex(data));
//        myNetMsgInput.reset();
//        // TODO: Netmsg TEST!!! ---
//        
//        // TODO: Netmsg TEST2!!! +++
//        avail = myNetMsgInput.available();
//        myNetMsgInput.fixed();
////        StringBuffer sb = new StringBuffer();
////        for (int nmgIdx=0; nmgIdx<avail; nmgIdx++) sb.append(Integer.toHexString(netMsgInput.readUnsignedByte()) + " ");
//        System.out.println("netFirstUpdate " + this.aircraft.name() + " (" + this.aircraft.getClass().getName() + ") myNetMsgInput2:");
////        System.out.println(sb.toString());
//        data = new byte[avail];
//        myNetMsgInput.readFully(data);
//        System.out.println(bytesToHex(data));
...
//            System.out.println("netFirstUpdate " + this.aircraft.name() + " (" + this.aircraft.getClass().getName() + ") astateEngineStates[" + i + "]=" + j);
