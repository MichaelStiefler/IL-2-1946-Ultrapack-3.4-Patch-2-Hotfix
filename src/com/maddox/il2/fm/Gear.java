...
public class Gear {
    ...
    private boolean            bCheckPlateFrictionFatigue = false;
    ...
    public Gear() {
        ...
        this.bHeavyAC = false;
        // ------------------------------------------------------------------------------------
        // TODO: +++ By SAS~Storebror: Prohibit players ignoring runways/taxiways
        this.bCheckPlateFrictionFatigue = false;
        // ------------------------------------------------------------------------------------
    }
    ...
    public void ground(FlightModel flightmodel, boolean isMaster, boolean isOnGround) {
        this.FM = flightmodel;
        this.bIsMaster = isMaster;
        this.onGround = isOnGround;
        this.FM.Vrel.x = -this.FM.Vwld.x;
        this.FM.Vrel.y = -this.FM.Vwld.y;
        this.FM.Vrel.z = -this.FM.Vwld.z;
        for (int fatigueIndex = 0; fatigueIndex < 2; fatigueIndex++)
            if (this.fatigue[fatigueIndex] > 0) this.fatigue[fatigueIndex]--;

        Pn.set(this.FM.Loc);
        Pn.z = Engine.cur.land.HQ(Pn.x, Pn.y);
        double groundAltitude = Pn.z;
        this.screenHQ = groundAltitude;
        if (this.FM.Loc.z - groundAltitude > 50D && !this.bFlatTopGearCheck) {
            this.nearGround = false;
            this.turnOffEffects();
        ...
        if (this.bFlatTopGearCheck) {
            ...
            corn1.z -= 20D;
            Actor ship = Engine.collideEnv().getLine(corn, corn1, false, clipFilter, Pship);
            if (ship instanceof BigshipGeneric) {
                Pn.z = Pship.z + 0.5D;
                groundAltitude = Pn.z;
                this.isWater = false;
                ...
                bigshipgeneric = (BigshipGeneric) ship;
                ...
                if (bigshipgeneric.isTowAircraft((Aircraft) flightmodel.actor)) {
                    int portNum = bigshipgeneric.towPortNum;
                    Point3d apoint3d[] = bigshipgeneric.getShipProp().propAirport.towPRel;
                    bigshipgeneric.pos.getAbs(this.l0);
                    this.l0.transform(apoint3d[portNum * 2], this.p0);
                    this.l0.transform(apoint3d[portNum * 2 + 1], this.p1);
        ...
        if (flightmodel.CT.bHasArrestorControl) {
            ...
            Loc arrestorLoc = new Loc(0.0D, 0.0D, 0.0D, 0.0F, 0.0F, 0.0F);
            this.arrestorHook.computePos(flightmodel.actor, Aircraft.tmpLoc1, arrestorLoc);
            this.arrestorVSink = (float) (Pn.z - arrestorLoc.getPoint().z);
        }
        Fd.set(this.FM.Vrel);
        Vnf.set(Normal);
        ...
        for (int pntiIndex = 0; pntiIndex < this.pnti.length; pntiIndex++) {
            int curPnti = this.pnti[pntiIndex];
            if (curPnti <= 0) Pnt[pntiIndex].set(0.0F, 0.0F, 0.0F);
            else {
                this.HM.hookMatrix(curPnti, M4);
                Pnt[pntiIndex].set((float) M4.m03, (float) M4.m13, (float) M4.m23);
            }
        }
...
//        System.out.println("pnti.length=" + this.pnti.length);
        for (int pntiIndex = 0; pntiIndex < this.pnti.length; pntiIndex++) {
            ...
            if (pntiIndex <= 2) {
                this.bIsGear = true;
                if (pntiIndex == 0 && (!this.lgear || this.FM.CT.getGear() < 0.01F) || pntiIndex == 1 && (!this.rgear || this.FM.CT.getGear() < 0.01F) || pntiIndex == 2 && !this.cgear) continue;
            } else this.bIsGear = false;
            PnT.set(Pnt[pntiIndex]);
...
//            System.out.println("d=" + d + ", Normal.dot(PnT)=" + Normal.dot(PnT) + ", D=" + this.D);
            if (this.d < 0.0D) {
                if (!this.testPropellorCollision(pntiIndex) || (this.isWater ? !this.testWaterCollision(pntiIndex) : this.bIsGear ? !this.testGearCollision(pntiIndex) : !this.testNonGearCollision(pntiIndex))) continue;
                clp[pntiIndex] = true;
                this.nP++;
//                System.out.println("testNonGearCollision1(" + pntiIndex + ")=true, nP=" + this.nP);
            } else {
                if (this.d >= 0.1D || this.isWater || this.bIsGear || !this.testNonGearCollision(pntiIndex)) continue;
                clp[pntiIndex] = true;
                this.nP++;
//                System.out.println("testNonGearCollision2(" + pntiIndex + ")=true, nP=" + this.nP);
            }
            PnT.x += this.FM.Arms.GC_GEAR_SHIFT;
            Fx.cross(PnT, Tn);
            Fv.set(Fx);
            if (this.bIsSail && this.bInWaterList(pntiIndex)) {
                ...
                if (this.FM.brakeShoe) {
                    if (!this.isAnyDamaged()) {
                        ...
                        if (this.bCatapultAllow && !this.FM.EI.getCatapult() && this.bCatapultSet) {
                            ...
                            Point3d curPoint = L.getPoint();
                            CellAirField cellairfield = bigshipgeneric1.getCellTO();
                            double catX = -cellairfield.leftUpperCorner().x - this.dCatapultOffsetX;
                            double catY = cellairfield.leftUpperCorner().y - this.dCatapultOffsetY;
                            Loc brakeShoeLoc = new Loc(catY, catX, 0.0D, 0.0F, 0.0F, 0.0F);
                            brakeShoeLoc.add(this.FM.brakeShoeLastCarrier.pos.getAbs());
                            Point3d brakeShoePoint = brakeShoeLoc.getPoint();
                            double brakeShoeDistance = (curPoint.x - brakeShoePoint.x) * (curPoint.x - brakeShoePoint.x) + (curPoint.y - brakeShoePoint.y) * (curPoint.y - brakeShoePoint.y);
                            if (brakeShoeDistance <= 100D) {
                                L.set(catY, catX, this.FM.brakeShoeLoc.getZ(), this.FM.brakeShoeLoc.getAzimut(), this.FM.brakeShoeLoc.getTangage(), this.FM.brakeShoeLoc.getKren());
                                L.add(this.FM.brakeShoeLastCarrier.pos.getAbs());
                                this.FM.Loc.set(L.getPoint());
                                ...
                                double brakeShoeX = -cellairfield.leftUpperCorner().x - this.dCatapultOffsetX2;
                                double brakeShoeY = cellairfield.leftUpperCorner().y - this.dCatapultOffsetY2;
                                brakeShoeLoc.set(brakeShoeY, brakeShoeX, 0.0D, 0.0F, 0.0F, 0.0F);
                                brakeShoeLoc.add(this.FM.brakeShoeLastCarrier.pos.getAbs());
                                Point3d altBrakeShoePoint = brakeShoeLoc.getPoint();
                                double altBrakeShoeDistance = (curPoint.x - altBrakeShoePoint.x) * (curPoint.x - altBrakeShoePoint.x) + (curPoint.y - altBrakeShoePoint.y) * (curPoint.y - altBrakeShoePoint.y);
                                if (altBrakeShoeDistance <= 100D) {
                                    L.set(brakeShoeY, brakeShoeX, this.FM.brakeShoeLoc.getZ(), this.FM.brakeShoeLoc.getAzimut(), this.FM.brakeShoeLoc.getTangage(), this.FM.brakeShoeLoc.getKren());
                                    L.add(this.FM.brakeShoeLastCarrier.pos.getAbs());
                                    this.FM.Loc.set(L.getPoint());
                                    this.FM.Or.setYPR(this.FM.brakeShoeLastCarrier.pos.getAbsOrient().getYaw(), this.Pitch, this.FM.brakeShoeLastCarrier.pos.getAbsOrient().getRoll());
                                    this.FM.actor.pos.setAbs(this.FM.Loc, this.FM.Or);
                        ...
                        for (int velocityIndex = 0; velocityIndex < 3; velocityIndex++)
                            this.gVelocity[velocityIndex] = 0.0D;
...
//                    gVelocity[k1] = 0.0D;
        ...
        if (!this.bIsMaster && !NetMissionTrack.isPlaying()) return;                                // If Plane is Net Mirror, no need to proceed further. Forest hits are dealt with by Master side.
        if (this.bIsMaster && this.onGround && !this.isWater) this.processingCollisionEffect();
        double forestHeight = Engine.cur.land.HQ_ForestHeightHere(this.FM.Loc.x, this.FM.Loc.y);    // Height of Forest here
        if (forestHeight < 1D) return;                                                              // Plausibility Check: No forest, no tree crash.
        
        // TODO: By SAS~Storebror
        //       New Forest collision code.
        // While an aircraft flies through forest, it gets slowed down and damage applies to a certain probability.
        // The deeper inside the forest it is, the more it gets slowed down (tree/branch strength factor).
        // The larger the wingspan, the more it gets slowed down. The heavier it is, the less this effect applies.
        // On touching the lowest 25% of forest height, any forest collision would result in immediate disintegration,
        // otherwise the plane will suffer sustained damage until critical parts fall off.
        // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        
        double agl = this.FM.Loc.z - groundAltitude;                                // Air-to-Ground level of this aircraft
        float inForestFactor = this.clamp((float)(agl / forestHeight), 0F, 1F);     // Factor telling whether we are within forest height.
                                                                                    // 0 = Ground
                                                                                    // 0 ~ 1 = in Forest height
                                                                                    // 1 = Above Forest
        float forestHitFactor = (1F - (float)inForestFactor)
                              / (1F - (LETHAL_TREE_HEIGHT_PERCENT / 100F)) + 0.5F;  // Depth inside forest converted to hit factor. Canopy = 0.5F, 25% tree height = 1.5F
        
        this.playForestHitSound(false);                                             // Temporarily Stop Playing Tree Hit Sound (we might play it again later)
        if (inForestFactor > 0.999F) {                                                      // We are above Forest height, nothing more to do here.
            this.outOfForestFor += (long)Time.tickLenFms();                                 // Increase the time we've been out of forest (continuously) by one tick len (30ms)
            if (this.outOfForestFor > FOREST_RECOVERY_TIME_MIN) this.inForestFor = 0L;      // Reset the time we've been in forest if recovery time threshold has been reached
            return;
        }
        this.inForestFor += (long)Time.tickLenFms();    // Increase the time we've been in of forest by one tick len (30ms)
        this.outOfForestFor = 0L;                       // Reset the time we've been out of forest (we're in after all)
            
        // Probability to hit a tree (per Tick, i.e. per time this method is run).
        // Equation is: Speed (in meters per second) * Ticklen (0.03s) * (fInForestFactor + 0.5F) (Depth in Forest, Canopy = 1, base 25% = 2) / 15 meters (TREE_DISTANCE, average distance between two trees).
        // Range: 0 ~ 1
        // Result samples: At >= 1800 km/h you hit a tree with every tick no matter what.
        //                 At 900 km/h you hit a tree each tick with 50% probability.
        //                 At 450 km/h you theoretically hit a tree every other tick with 50% probability, or every tick with 25% probability.
        //                 On PTO maps, tree distance gets reduced from 15m to 10m, simulating denser jungle
        //                 Finally, Tree Hit Probability gets linearly increased by factor 1 at top to 2 at 25% height
        float treeHitPerTickProbability = this.FM.getSpeed() * Time.tickConstLenFs() * (forestHitFactor + 1.0F) / (World.cur().camouflage == World.CAMOUFLAGE_PACIFIC?TREE_DISTANCE_PACIFIC:TREE_DISTANCE) / TREE_HIT_PROBABILITY_FACTOR;
        
        float random = TrueRandom.nextFloat();                                  // Random Number 0~1 to calculate with further on
        if (random < treeHitPerTickProbability) this.playForestHitSound(true);  // Play Sound of Trees hitting aircraft (using BK 7.5 sound as stand-in)
        
        String effectFileName = "Effects/Explodes/Objects/Forest/ForestDebris";                                                         // Default Effect File for Forest Debris
        if(World.cur().camouflage == World.CAMOUFLAGE_WINTER  || World.cur().camouflage == World.CAMOUFLAGE_DESERT) effectFileName += "W";  // Winter or Desert
            
        Eff3DActor.New(new Loc(flightmodel.Loc, new Orient(0F, 90F, 0F)), 1.0F, effectFileName + ".eff", -1F); // Aircraft is flying through forest, create forest debris!
        if (inForestFactor < (LETHAL_TREE_HEIGHT_PERCENT / 100F)) { // Aircraft is within the lowest 25% of Forest, which means it will disintegrate no matter what.
            this.killByForest();
            return;
        }
        
        if (random > treeHitPerTickProbability) return;
        
        if (this.FM.actor == World.getPlayerAircraft() && this.FM instanceof RealFlightModel) { // Apply aircraft shake to player aircraft
            RealFlightModel rfm = (RealFlightModel)this.FM;
            if (rfm.producedShakeLevel < (forestHitFactor)* TREE_HIT_SHAKE_LEVEL_FACTOR) rfm.producedShakeLevel = forestHitFactor * TREE_HIT_SHAKE_LEVEL_FACTOR;
        }
        
        if (!this.bIsMaster) return;        
        
        if (this.inForestFor < FOREST_GRACE_PERIOD) return; // If we are within grace period, don't apply any brake pulse nor damage
        
        // Add a "pulse" in opposite direction for the braking effect of hitting trees.
        // Factor depends on aircraft wingspan and the "depth" which the plane passes the trees at.
        // At 25% depth, the braking force is multiplied by 1.5F, at canopy level it's multiplied by 0.5F.
        // The factor "TREE_HIT_BRAKE_FORCE_FACTOR" is just a static factor in order to yield realistic results.
        Vector3d vTreeHit = new Vector3d(this.FM.Vwld.x, this.FM.Vwld.y, 0D);
        vTreeHit.scale(this.FM.Wingspan * TREE_HIT_BRAKE_FORCE_FACTOR * forestHitFactor + 0.5F);
        this.FM.gunPulse(vTreeHit);
        
        if (this.bIsMaster && TrueRandom.nextFloat() > TREE_HIT_DAMAGE_PERCENT / 100F)  this.damageByForest();
    }
    ...
    // Create list of plane chunks that can get hit by forest
    private void initForestHitChunks(Aircraft aircraft) {
        
        // Create List of damageable parts...
        forestHitChunks = new ArrayList();
        String[] partNames = (String[]) Reflection.getValue(Aircraft.class, "partNames");   // Possible Aircraft Part Names from Aircraft Class (Stock Code)
        for (int partNameIndex = 0; partNameIndex < partNames.length; partNameIndex++) {
            if (partNames[partNameIndex].toLowerCase().startsWith("turret")) continue;      // Don't steal guns from gunners ;-)
            if (!aircraft.isChunkAnyDamageVisible(partNames[partNameIndex])) continue;      // Potentially non-damageable chunk
            forestHitChunks.add(partNames[partNameIndex]);                                  // This is a damageable part, add it to the list of parts that can suffer damage from tree hits!
        }
        ...
        while (true) {
            propIndex++;
            if (aircraft.hierMesh().chunkFindCheck("Prop" + propIndex + "_D0") < 0) break;  // If no Prop with this index exists anymore, stop searching for further ones
            forestHitChunks.add("Prop" + propIndex);                                        // Add this prop to the list of parts that can suffer damage from tree hits!
        }
    }
    
    // Damage by forest collision
    private void damageByForest() {
        if (!(this.FM.actor instanceof Aircraft)) return;                                       // Not an aircraft? Not our business.
        Aircraft aircraft = (Aircraft)this.FM.actor;                                            // Typecast this actor to "Aircraft"
        if (forestHitChunks == null) this.initForestHitChunks(aircraft);                        // Create list of plane chunks if not done so before
        if (forestHitChunks.isEmpty()) return;                                                  // If no damageable parts exist / are left, exit (no further damage can be applied)
        String chunkName;                                                                       // The Chunk name which we are going to use for tree hit damage
        try {
            while (true) {                                                                      // Helper Loop to skip Prop hits if hit probability is below Prop hit probability
                int hitChunkIndex = forestHitChunks.size();                                     // Size of plane chunks list
                chunkName = (String)forestHitChunks.get(TrueRandom.nextInt(hitChunkIndex));     // Currently selected chunk name
                if (hitChunkIndex == 0) break;                                                  // If first element of the chunks list got selected, use it, regardless which element it is.
                if (!chunkName.toLowerCase().startsWith("prop")) break;                         // If selected element is not a prop, use it.
                if (TrueRandom.nextFloat() < 1F - (PROP_HIT_PROBABILITY_PERCENT / 100F)) break; // If Random Number is within prop hit probability, damage the Prop.
                
            }
            if (chunkName.toLowerCase().startsWith("prop")) {                                   // Prop Damage Code
                try {
                    String sPropNum = chunkName.substring(4);                                   // String containing the Prop Index Number
                    int propNum = Integer.parseInt(sPropNum) - 1;                               // Zero-Based Prop Index Number
                    if (propNum >= 0) {                                                         // Safety Check
                        aircraft.hitProp(propNum, 0, Engine.actorLand());                       // Hit the Prop!
                        aircraft.FM.AS.setEngineDies(Engine.actorLand(), propNum);              // Kill the Engine!
                        forestHitChunks.remove(chunkName);                                      // Remove this Prop from our list, it cannot be damaged further.
                    }
                } catch (Exception e) {
                    e.printStackTrace();                                                        // If an error occurs, log it.
                }
                return;                                                                         // That's it, Prop damage finished, bye bye!
            }
            
            // Damaging Aircraft Parts other than Props
            aircraft.setDamager(Engine.actorLand(), 1);                                         // Tell the aircraft that it got damaged by Land (Trees)
            
            // Call the Method that selects the next available damage level from the chunk we want to damage.
            // We need to use the Reflection API because access to this method is usually not allowed from other than aircraft classes
//            Reflection.invokeMethod(aircraft, "nextDMGLevel", new Class[] {String.class, int.class, Actor.class}, new Object[] {new String(chunkName + "_D0"), new Integer(0), Engine.actorLand()});
            Reflection.invokeMethod(aircraft, "nextDMGLevels", new Class[] {int.class, int.class, String.class, Actor.class}, new Object[] {new Integer(1), new Integer(2), new String(chunkName + "_D0"), Engine.actorLand()});
            
            if (!aircraft.isChunkAnyDamageVisible(chunkName)) {                                 // If the damaged part fell off...
                forestHitChunks.remove(chunkName);                                              // ...remove it from our list, cause it cannot receive any further damage.
            }
        } catch (Exception e) {
            e.printStackTrace();                                                                // If an error occurs, log it.
        }
    ...
    // Kill Plane when hitting the lethal part of the forest trees
    private void killByForest() {
        Eff3DActor.New(new Loc(this.FM.Loc, new Orient(0F, 90F, 0F)), 1.0F, "Effects/Explodes/Objects/Forest/ForestBranches.eff", -1F);     // Flying Tree Branches
        String effectFileName = "Effects/Explodes/Objects/Forest/ForestLeaves";                                                             // Default Effect File for Forest Debris
        if(World.cur().camouflage == World.CAMOUFLAGE_WINTER  || World.cur().camouflage == World.CAMOUFLAGE_DESERT) effectFileName += "W";  // Winter or Desert
        Eff3DActor.New(new Loc(this.FM.Loc, new Orient(0F, 90F, 0F)), 1.0F, effectFileName + ".eff", -1F);                                  // Fyling Tree Leaves
        if (!this.bIsMaster) return;                                                                                                                // Net Mirror
        if (((Aircraft) this.FM.actor).isEnablePostEndAction(0.0D)) ((Aircraft) this.FM.actor).postEndAction(0.0D, Engine.actorLand(), 2, null);    // Kill the Aircraft Object on Master Side
    }
    
    // Play sound of hitting Forest Trees
    private void playForestHitSound(boolean doPlay) {
        if (!this.bIsMaster) return;                                            // Only play on the machine that actually controls the aircraft, no net mirrors
        if (!doPlay) {                                                          // If sound shall be stopped...
            if (this.forestHitSound != null) this.forestHitSound.stop();        // ...stop it (if the sound exists)...
            return;                                                             // ...and that's it, bye.
        }
        if (!Config.isUSE_RENDER()) return;                                     // Play sound on Client (not Server)
        if (this.FM.AS.actor != World.getPlayerAircraft()) return;              // Play sound for player aircraft only
        if (Main3D.cur3D().isViewOutside()) return;                             // Play sound in cockpit only
        if (this.forestHitSound == null) {                                      // Init Sound if not done so yet
            this.forestHitSound = this.FM.actor.newSound("hit.air", false);     // Use "hit.air" stand-in sound
            if (this.forestHitSound != null) {                                  // If init was successful...
                SoundFX soundFX = this.FM.actor.getRootFX();                    // ...get Plane Sound environment...
                if (soundFX != null) {                                          // ...is successful...
                    this.forestHitSound.setParent(soundFX);                     // ...attach sound to it.
                    this.forestHitSound.setPosition(0D, 0D, 0D);                // right in the center
                }
                this.forestHitSound.setVolume(100F);                            // Use full volume!
            }
        }
        if (this.forestHitSound == null) return;                                // If no hit sound could be initialized, skip!
        this.forestHitSound.play();                                             // Play Tree Hit Sound.
    }
    
    private ArrayList forestHitChunks = null;
    private SoundFX forestHitSound = null;
    private long inForestFor = 0L;
    private long outOfForestFor = FOREST_RECOVERY_TIME_MIN;
    private static final float TREE_HIT_BRAKE_FORCE_FACTOR = 20F;   // Factor for the "pulse" force applied when plane hits trees
    private static final float TREE_DISTANCE = 15F;                 // Distance between two trees (in meters)
    private static final float TREE_DISTANCE_PACIFIC = 10F;         // Distance between two trees (in meters) on pacific maps (palm trees and sorts, closer than others)
    private static final float TREE_HIT_PROBABILITY_FACTOR = 2F;    // Factor for the "pulse" force applied when plane hits trees
    private static final float TREE_HIT_SHAKE_LEVEL_FACTOR = 8F;    // Airplane shake level when hitting trees
    private static final float LETHAL_TREE_HEIGHT_PERCENT = 25F;    // Precentage of total tree height where hitting them is lethal (here: Lowest 1/4th is lethal)
    private static final float TREE_HIT_DAMAGE_PERCENT = 10F;       // Precentage of tree hits causing airplane damage
    private static final float PROP_HIT_PROBABILITY_PERCENT = 30F;  // Precentage of prop hit probability - this suppresses excessive early prop/engine damage
    private static final long FOREST_RECOVERY_TIME_MIN = 60000L;    // Minimum time (in milliseconds) a plane needs to stay out of forest before another grace period is being granted
    private static final long FOREST_GRACE_PERIOD = 1000L;          // Grace Period (in milliseconds) before forest damage and brake pulse apply
...
//                        double d21 = 38000.0 + this.FM.M.massEmpty * 6.0;
    ...
    public double plateFriction(FlightModel flightmodel) {
        ...
        if (bPlateExist) {
            // TODO: +++ By SAS~Storebror: Prohibit players ignoring runways/taxiways
            this.bCheckPlateFrictionFatigue = true;
            // ------------------------------------------------------------------------------------
            if (bPlateGround) {
//                HUD.training("0.8 (1)");
                return 0.8;
...
//                HUD.training("5.0");
                // TODO: +++ By SAS~Storebror: Prohibit players ignoring runways/taxiways
                this.plateFrictionFatigue(5.0F);
                // ------------------------------------------------------------------------------------
                return 5.0;
            default:
...
//                HUD.training("3.8");
                // TODO: +++ By SAS~Storebror: Prohibit players ignoring runways/taxiways
                this.plateFrictionFatigue(3.8F);
                // ------------------------------------------------------------------------------------
                return 3.8;
        }
    }
    ...
    // TODO: +++ By SAS~Storebror: Prohibit players ignoring runways/taxiways
    private void plateFrictionFatigue(float friction) {
        if (!this.bCheckPlateFrictionFatigue) return;
        if (!Config.isUSE_RENDER()) return;
        Aircraft aircraft = (Aircraft) this.FM.actor;
        if (aircraft != World.getPlayerAircraft()) return;
        if (aircraft.FM.isWasAirborne()) return;
        if (!World.cur().diffCur.Realistic_Landings || !World.cur().diffCur.Takeoff_N_Landing || !World.cur().diffCur.Vulnerability) return;
        if (aircraft.FM.getSpeed2KMH() < 50F) return;
        for (int i = 0; i < 2; i++) {
            if (gWheelSinking[i] <= 0F) continue;
            float addFatigueFactor = 0F;
            if (aircraft.FM.getSpeed2KMH() < 150F) {
                addFatigueFactor = CommonTools.smoothCvt(aircraft.FM.getSpeed2KMH(), 50, 150, 0.0F, 25.0F);
            } else {
                addFatigueFactor = CommonTools.smoothCvt(aircraft.FM.getSpeed2KMH(), 150, 200, 25.0F, 5.0F);
            }
            this.fatigue[i] += (int) (friction * addFatigueFactor);
            if (this.fatigue[i] > 100) {
                if (i == 0)
...
//        HUD.training("s=" + df.format(aircraft.FM.getSpeed2KMH())+ ", f=" + df.format(fatigue[0]) + "/" + df.format(fatigue[1]));
//        System.out.println("s=" + df.format(aircraft.FM.getSpeed2KMH())+ ", f=" + df.format(fatigue[0]) + "/" + df.format(fatigue[1]));
    }
    // ------------------------------------------------------------------------------------
