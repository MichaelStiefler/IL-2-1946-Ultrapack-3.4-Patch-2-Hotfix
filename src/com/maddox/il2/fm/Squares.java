    ...
    public void computeParasiteDrag(Controls controls, BulletEmitter abulletemitter[][]) {
        ...
        for (int i = 0; i < abulletemitter.length; i++) {
            ...
            for (int j = 0; j < abulletemitter[i].length; j++) {
                ...
                if ((abulletemitter[i][j] instanceof BombGun || abulletemitter[i][j] instanceof RocketBombGun) && abulletemitter[i][j].haveBullets() && abulletemitter[i][j].getHookName().startsWith("_E") && this.dragParasiteCx < 0.704F)
...
//				if (abulletemitter[i][j] instanceof PylonRO_WfrGr21Dual) dragParasiteCx += 0.02F;
                ...
                else if (abulletemitter[i][j] instanceof Pylon && ((Pylon)abulletemitter[i][j]).getDragCx() > 0.0F) dragParasiteCx += ((Pylon)abulletemitter[i][j]).getDragCx() * (((Pylon)abulletemitter[i][j]).isMinusDrag()?-1F:1F);
