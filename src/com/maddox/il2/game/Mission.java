...
public class Mission implements Destroy {
    ...
    private static Map           triggerSpawnMap       = new HashMap();
...
//                System.out.println("Wing " + this.wingName + " delay has elapsed so it spawns now!");
                Wing wing = new Wing();
...
//        }
            ...
            try {
                if (!this.loadFakeViewPoint()) // TODO: Added by SAS~Storebror - Define single static camera in conf.ini for track playback purpose
...
// World.land().LoadMap(string, is);
    ...
    private List loadWings(SectFile sectfile) throws Exception {
        ...
        int wingSectionIndex = sectfile.sectionIndex("Wing");
        if (wingSectionIndex < 0) return null;
        ...
        int numVars = sectfile.vars(wingSectionIndex);
        ArrayList arraylist = null;
        if (numVars > 0) arraylist = new ArrayList(numVars);
...
//            System.out.println("Trigger Aircraft Spawn no." + (t+1) + "=" + World.cur().triggersGuard.getListTriggerAircraftSpawn().get(t).toString());
        ...
        for (int varIndex = 0; varIndex < numVars; varIndex++) {
            this.LOADING_STEP(30 + Math.round((float) varIndex / (float) numVars * 30.0F), "task.Load_aircraft");
            String string = sectfile.var(wingSectionIndex, varIndex);
//            System.out.println("Aircraft to Spawn = " + string);
            this._loadPlayer = string.equals(this.player);
            int startTimeMinutes = sectfile.get(string, "StartTime", 0);
...
//                System.out.println("Wing " + string + " is a trigger target with start time of " + startTimeMinutes + " minutes, hence it will not spawn on mission start!");
                triggerSpawnMap.put(string, new Integer(startTimeMinutes));
            } else {
            // TODO: --- Trigger backport from HSFX 7.0.3 MODIFIED by SAS~Storebror ---
                if (startTimeMinutes > 0 && !this._loadPlayer) {
                    if (netchannel == null) {
                        double d = (double)startTimeMinutes * 60D;
    //                    // TODO: +++ Trigger backport from HSFX 7.0.3 MODIFIED by SAS~Storebror +++
    //                    // TODO: Changed by SAS~Storebror: Don't delay-spawn aircraft when they have a Trigger spawning them!
    //                    if(World.cur().triggersGuard.getListTriggerAircraftSpawn().contains(string)) {
    //                        this.triggerSpawnMap.put(string, new Double(d));
    //                        //d = Double.MAX_VALUE;
    //                    }
    //                    // ---
    //                    // TODO: --- Trigger backport from HSFX 7.0.3 MODIFIED by SAS~Storebror ---
//                        System.out.println("Wing " + string + " has a start time of " + startTimeMinutes + " minutes, hence it will not spawn on mission start!");
...
//                    System.out.println("Wing " + string + " spawns on mission start!");
        ...
        if (arraylist != null) {
            for (int wingIndex = 0; wingIndex < arraylist.size(); wingIndex++) {
                ((Wing)arraylist.get(wingIndex)).setInitialSpawnParamsForSquad();
            }
        }
    ...
    public void loadWingOnTrigger(String s)
    {
        ...
        try
        {
            ...
            try {
                startTimeMinutes = ((Integer)(triggerSpawnMap.get(s))).intValue();
            } catch (Exception e) {
                e.printStackTrace();
            }
            this._loadPlayer = s.equals(this.player);
            NetChannel netchannel = null;
            if (!isServer()) netchannel = this.net.masterChannel();
            if (startTimeMinutes > 0 && !this._loadPlayer) {
                if (netchannel == null) {
                    double d = (double)startTimeMinutes * 60D;
//                    System.out.println("Wing " + s + " has been triggered with a start time of " + startTimeMinutes + " minutes, hence it will spawn with delay!");
...
//                System.out.println("Wing " + s + " has been triggered with and spawns now!");
...
//             localWing.load(sectFile, s, null);
        ...
        {
            System.out.println("Mission error, ID_31 : Wing " + s + " not load : " + e.toString());
            e.printStackTrace();
        }
    }
    // TODO: --- Trigger backport from HSFX 7.0.3 by SAS~Storebror ---
...
//        System.out.println("Mission.prepareSkinInWing(" + sectfile.fileName() + ", " + wing.airc.length + ")");
        if (Config.isUSE_RENDER()) {
            Aircraft[] aircrafts = wing.airc;
...
//        System.out.println("Mission.prepareSkinInWing(" + sectfile.fileName() + ", " + aircraft.getClass().getName() + ", " + string + ", " + i + ")");
        if (Config.isUSE_RENDER()) if (World.getPlayerAircraft() == aircraft) {
            ...
            String skinFileName = sectfile.get(string, "skin" + i, (String) null);
//            System.out.println("skinFileName1 = " + (skinFileName==null?"null":skinFileName));
            if (skinFileName != null) {
                String propertyMesh = Aircraft.getPropertyMesh(aircraft.getClass(), aircraft.getRegiment().country());
//                System.out.println("propertyMesh = " + (propertyMesh==null?"null":propertyMesh));
                skinFileName = GUIAirArming.validateFileName(Property.stringValue(aircraft.getClass(), "keyName", null)) + "/" + skinFileName;
//                System.out.println("skinFileName2 = " + (skinFileName==null?"null":skinFileName));
                if (NetMissionTrack.isPlaying()) {
                    skinFileName = NetFilesTrack.getLocalFileName(Main.cur().netFileServerSkin, Main.cur().netServerParams.host(), skinFileName);
//                    System.out.println("skinFileName3 = " + (skinFileName==null?"null":skinFileName));
                    if (skinFileName != null) skinFileName = Main.cur().netFileServerSkin.alternativePath() + "/" + skinFileName;
                } else {
                    skinFileName = Main.cur().netFileServerSkin.primaryPath() + "/" + skinFileName;
//                    System.out.println("skinFileName4 = " + (skinFileName==null?"null":skinFileName));
                }
                if (skinFileName != null) {
                    String cacheFileName = "PaintSchemes/Cache/" + Finger.file(0L, skinFileName, -1);
//                    System.out.println("cacheFileName = " + (cacheFileName==null?"null":cacheFileName));
                    Aircraft.prepareMeshSkin(propertyMesh, aircraft.hierMesh(), skinFileName, cacheFileName);
                }
            }
...
//                        System.out.println("skin" + i3 + "=" + string4);
...
//        System.out.println("Mission.recordNetFile(" + netfileserverdef.primaryPath() + "," + string + ")");
        if (string != null) {
...
//      System.out.println("getUnitNetIdRemote, actors size: " + this.actors.size() + ", curActor: " + this.curActor + ", actor=" + actor.name() + "(" + actor.getClass().getName() + ")");
  ...
  private Actor loadStationaryActor(String actorName, String actorClassName, int army, double xPos, double yPos, float direction, float timeLen, String regiment, String skill, String slowfire) {
      return this.loadStationaryActor(actorName, actorClassName, army, xPos, yPos, direction, timeLen, regiment, skill, slowfire, false);
  }
  
    private Actor loadStationaryActor(String actorName, String actorClassName, int army, double xPos, double yPos, float direction, float timeLen, String regiment, String skill, String slowfire, boolean fromTrigger) {
//        System.out.println("loadStationaryActor(" + actorName + ", " + actorClassName+ ", " + army + ", " + xPos + ", " + yPos + ", " + direction + ", " + timeLen + ", " + regiment + ", " + skill + ", " + slowfire + ") net.isMirror()=" + this.net.isMirror() + ", fromTrigger=" + fromTrigger);
        Class var_class;
        try {
            var_class = ObjIO.classForName(actorClassName);
        } catch (Exception exception) {
            System.out.println("Mission: class '" + actorClassName + "' not found");
            return null;
        }
        ActorSpawn actorspawn = (ActorSpawn) Spawn.get(var_class.getName(), false);
        if (actorspawn == null) {
            System.out.println("Mission: ActorSpawn for '" + actorClassName + "' not found");
            return null;
        }
        spawnArg.clear();
        if (actorName != null) {
            if ("NONAME".equals(actorName)) {
                System.out.println("Mission: 'NONAME' - not valid actor name");
                return null;
            }
            if (Actor.getByName(actorName) != null) {
                System.out.println("Mission: actor '" + actorName + "' alredy exist");
                return null;
            }
            spawnArg.name = actorName;
        }
        spawnArg.army = army;
        spawnArg.armyExist = true;
        spawnArg.country = regiment;
        Chief.new_DELAY_WAKEUP = 0.0F;
        ArtilleryGeneric.new_RADIUS_HIDE = 0.0F;
        if (regiment != null) try {
            Chief.new_DELAY_WAKEUP = Integer.parseInt(regiment);
        ...
        if (skill != null) {
            try {
                Chief.new_SKILL_IDX = Integer.parseInt(skill);
            } catch (Exception exception) {
                /* empty */
            }
        ...
        if (slowfire != null) {
            try {
                Chief.new_SLOWFIRE_K = Float.parseFloat(slowfire);
            } catch (Exception exception) {
                /* empty */
            }
        ...
        p.set(xPos, yPos, 0.0);
        spawnArg.point = p;
        o.set(direction, 0.0F, 0.0F);
        spawnArg.orient = o;
        if (timeLen > 0.0F) {
            spawnArg.timeLenExist = true;
            spawnArg.timeLen = timeLen;
        }
        ...
        if (this.net.isMirror() && !NetMissionTrack.isPlaying()) {
            ...
            if (this.curActor < 0 || this.curActor >= this.actors.size()) {
//                System.out.println("curActor=" + curActor + ", this.actors.size()=" + this.actors.size());
                if (this.curActor < 0) return null;
                if (!fromTrigger) return null;
                try {
//                    System.out.println("Mission.loadStationaryActor 11");
                    actor = actorspawn.actorSpawn(spawnArg);
                    if (!(actorspawn instanceof NetSpawn)) { // FIXME: Nutjob to fight NetMsgSpawn Exceptions...
//                        System.out.println("Mission.loadStationaryActor 11b");
                        Reflection.setInt(actor, "flags", Reflection.getInt(actor, "flags") | 0x1000);
                    }
//                    System.out.println("Mission.loadStationaryActor 12, actor=" + (actor==null?"null":actor.getClass().getName()));
                } catch (Exception exception) {
                    System.out.println("Mission.loadStationaryActor Exception in actorspawn.actorSpawn(" + spawnArg.name + ")");
                    System.out.println(exception.getMessage());
                    exception.printStackTrace();
                }
                this.getUnitNetIdRemote(actor);
                
//                int netIdLocal = this.getUnitNetIdRemote(actor);
//                System.out.println("Mission.loadStationaryActor netIdLocal = " + netIdLocal);
                return actor;
            }
            // ---
...
//        System.out.println("Mission.loadStationaryActor 04");
    ...
    private void loadNStationary(SectFile sectfile) {
        ...
        if (sectionIndex >= 0) {
            ...
            for (int varIndex = 0; varIndex < numVars; varIndex++)
                try {
                    this.LOADING_STEP(85 + Math.round((float) varIndex / (float) numVars * 5.0F), "task.Load_stationary_objects");
...
//                          numbertokenizer.next((String) null), numbertokenizer.next((String) null), numbertokenizer.next((String) null));
                  ...
                  // TODO: --- Trigger backport from HSFX 7.0.3 by SAS~Storebror ---
                    ...
                    if (this.isDestroyed()) return;
                    System.out.println("Error when reading .mis NStationary section line " + varIndex + " (" + ex.toString() + " )");
                }
        }
    }
...
//        if (!Mission.isSingle() && !Mission.isServer()) return; // TODO: removed by SAS~Storebror, otherwise net replication won't work!
//        if (this.net.isMirror()) return; // TODO: TEST Added by SAS~Storebror to fight dedicated Server / Client issues!
//        if (this.net.isMirror()) this.actors.add(null); // TODO: TEST Added by SAS~Storebror to fight dedicated Server / Client issues!
        int sectionIndex = sectFile.sectionIndex("NStationary");
        if(sectionIndex >= 0)
...
//                System.out.println("before loadStationaryActor");
                this.loadStationaryActor(curName, numbertokenizer.next(""), numbertokenizer.next(0), numbertokenizer.next(0.0), numbertokenizer.next(0.0), numbertokenizer.next(0.0F), numbertokenizer.next(0.0F), numbertokenizer.next((String) null), numbertokenizer.next((String) null), numbertokenizer.next((String) null), true);
                        ...
                        if (hasSpawnPointsDefined || zutiIsStandAloneBornPlace) {
                            ...
                            if (this.actors != null) {
                                ...
                                for (int i_109_ = 0; i_109_ < i_108_; i_109_++) {
                                    Object o = this.actors.get(i_109_);
...
//        }
    ...
    private boolean loadFakeViewPoint() {
        String fakeViewPoint = Config.cur.ini.get("Mods", "StaticCamera", "");
        if (fakeViewPoint.trim().length() < 1) return false;
        try {
            NumberTokenizer numbertokenizer = new NumberTokenizer(fakeViewPoint.trim(),";");
            float x = numbertokenizer.next(0);
            float y = numbertokenizer.next(0);
            float z = numbertokenizer.next(100, 2, 10000);
            ActorViewPoint actorviewpoint = new ActorViewPoint();
            Point3d pointStaticCamera = new Point3d(x, y, z + Landscape.HQ_Air(x, y));
            actorviewpoint.pos.setAbs(pointStaticCamera);
            actorviewpoint.pos.reset();
            actorviewpoint.dreamFire(true);
            actorviewpoint.setName("StaticCamera_0");
            if (this.net.isMirror()) {
    ...
    private void loadViewPoint(SectFile sectfile) {
        ...
        for (int varIndex = 0; varIndex < numVars; varIndex++) {
            ...
            float z = numbertokenizer.next(100, 2, 10000);
            ActorViewPoint actorviewpoint = new ActorViewPoint();
            Point3d point3d = new Point3d(x, y, z + Landscape.HQ_Air(x, y));
...
//                    this.sectFile.lineAdd(i, netmsginput.read255(), netmsginput.read255());
                    ...
                    if (value.startsWith("\u001e")) { // Separator was "!" before, which caused issues with skin filenames starting with such character
                        if (joinedValue == null) joinedValue = new StringBuffer();
                        joinedValue.append(value.substring(1));
                        if (value.endsWith("\u001e")) { // Separator was "!" before, which caused issues with skin filenames starting with such character
...
//                            System.out.println("Mission splitting sectfile line: " + outString);
                            ...
                            for (int splitOutStringIndex = 0; splitOutStringIndex < splitOutString.size(); splitOutStringIndex++) {
                                ...
                                StringBuffer sb = new StringBuffer("\u001e").append((String)splitOutString.get(splitOutStringIndex)); // Separator was "!" before, which caused issues with skin filenames starting with such character
                                if (splitOutStringIndex == splitOutString.size() - 1) sb.append("\u001e"); // Separator was "!" before, which caused issues with skin filenames starting with such character
