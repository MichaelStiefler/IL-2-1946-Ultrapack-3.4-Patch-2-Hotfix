...
public class GUIMenu
{
    ...
    public static void setHotKeyESC(boolean value) {
        bHotKeyESC = value;
    }

    public static void initHotKeyESC()
    {
        if(bHotKeyESC) return;
        bHotKeyESC = true;
//        System.out.println("initHotKeyESC()");
        String envName = "gui";
        ...
        HotKeyCmdEnv.addCmd(envName, new HotKeyCmd(true, "activate") {
            ...
            public void end()
            {
//                System.out.println("initHotKeyESC() end()");
//                Exception test = new Exception("initHotKeyESC");
        ...
        if(HotKeyEnv.isEnabled("aircraftView"))
        {
            ...
            for (int actorType = 0; actorType < 4; actorType++) {
                switch (actorType) {
                    ...
                    case 1:
                    case 3:
                        if (!Main3D.cur3D().aircraftHotKeys.viewAllowed(Main3D.cur3D().aircraftHotKeys.isbExtViewEnemy())) {
                            mView.subMenu.addItem(new GWindowMenuItem(mView.subMenu, menuCaption[actorType], menuHelp[actorType]){}).bEnable=false;
                            continue;
                        }
                }
