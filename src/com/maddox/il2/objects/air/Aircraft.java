...
public abstract class Aircraft extends NetAircraft implements MsgCollisionListener, MsgCollisionRequestListener, MsgExplosionListener, MsgShotListener, MsgEndActionListener, Predator {
    ...
    static {
        planesWithZBReceiver = new Class[] { F4U.class, F4F.class, TBF.class, SBD.class, PBYX.class, F6F.class, F2A2.class, SEAFIRE3.class, SEAFIRE3F.class, Fulmar.class, Swordfish.class, MOSQUITO.class, B_25.class, A_20.class, B_17.class,
...
//            World.cur();
    ...
    public void msgExplosion(Explosion explosion) {
        ...
        // TODO: Avoid Mistels killing each other when detonating +++
        if (this instanceof Mistel && explosion.initiator instanceof Mistel && explosion.initiator instanceof Aircraft) {
            if ( ((Mistel)this).isDrone() && ((Mistel)explosion.initiator).isQueen() && this.getArmy() == ((Aircraft)explosion.initiator).getArmy()) {
//                System.out.println("Aircraft.msgExplosion skipped to avoid Mistel self-kill!");
                return;
            }
        }
        // ---
        ...
        if (explosion.power <= 0.0F || explosion.chunkName != null && explosion.chunkName.equals(partNames[43]))
            ...
            int powerType = explosion.powerType;
            if (powerType == 1) this.splintersHit(explosion);
            else {
                ...
                float tnt_1meter;
                if (explosion.chunkName == null) tnt_1meter = explosion.receivedTNT_1meter(this);
                else tnt_1meter = power;
                if (!(tnt_1meter <= 5E-7F)) {
                    this.debugprintln("Splash hit from " + (explosion.initiator instanceof Aircraft ? ((Aircraft) explosion.initiator).typedName() : explosion.initiator.name()) + " in " + explosion.chunkName + " for "
                            + (int) (100.0F * tnt_1meter / (0.01F + 3.0F * this.FM.Sq.getToughness(this.part(explosion.chunkName)))) + " % ( " + tnt_1meter + " kg)..");
                    if (explosion.chunkName == null) tnt_1meter /= 0.01F;
                    else {
                        if (explosion.chunkName.endsWith("_D0") && !explosion.chunkName.startsWith("Gear")) {
                            if (tnt_1meter > 0.01F) tnt_1meter = 1.0F + (tnt_1meter - 0.01F) / this.FM.Sq.getToughness(this.part(explosion.chunkName));
                            else tnt_1meter /= 0.01F;
                        } else tnt_1meter /= this.FM.Sq.getToughness(this.part(explosion.chunkName));
                        tnt_1meter += this.FM.Sq.eAbsorber[this.part(explosion.chunkName)];
                    }
                    if (tnt_1meter >= 1.0F) this.setDamager(explosion.initiator, (int) tnt_1meter);
                    if (explosion.chunkName != null) {
                        if ((int) tnt_1meter > 0) {
                        ...
                        this.nextDMGLevels((int) tnt_1meter, 1, explosion.chunkName, explosion.initiator);
                    } else for (int partNamesIndex = 0; partNamesIndex < partNamesForAll.length; partNamesIndex++) {
                        int randomPartNameIndex = World.Rnd().nextInt(partNamesForAll.length);
                        if (this.isChunkAnyDamageVisible(partNamesForAll[randomPartNameIndex])) {
                            this.nextDMGLevels((int) tnt_1meter, 1, partNamesForAll[randomPartNameIndex] + "_D0", explosion.initiator);
                            break;
                        }
                    }
                    if (explosion.chunkName != null) this.FM.Sq.eAbsorber[this.part(explosion.chunkName)] = tnt_1meter - (int) tnt_1meter;
                    if (tnt_1meter > 8.0F) if (tnt_1meter / partNamesForAll.length > 1.5F) {
                        for (int partNamesIndex = 0; partNamesIndex < partNamesForAll.length; partNamesIndex++)
                            if (this.isChunkAnyDamageVisible(partNamesForAll[partNamesIndex])) this.nextDMGLevels(3, 1, partNamesForAll[partNamesIndex] + "_D0", explosion.initiator);
                    } else {
                        int tnt_1meter_directional = (int) tnt_1meter / 3 - 1;
                        if (tnt_1meter_directional > partNamesForAll.length * 2) tnt_1meter_directional = partNamesForAll.length * 2;
                        for (int tntRaising = 0; tntRaising < tnt_1meter_directional; tntRaising++) {
                            int randomPartNameIndex = World.Rnd().nextInt(partNamesForAll.length);
                            if (this.isChunkAnyDamageVisible(partNamesForAll[randomPartNameIndex])) this.nextDMGLevels(3, 1, partNamesForAll[randomPartNameIndex] + "_D0", explosion.initiator);
                        }
                    }
                    ...
                    if (netPower > 0.0F) {
                        ...
                        msgExplosionPostVarSet.power = netPower;
                        msgExplosionPostVarSet.radius = explosion.radius;
                        new MsgAction(false, msgExplosionPostVarSet) {
                            public void doAction(Object object) {
                                MsgExplosionPostVarSet mepvs = (MsgExplosionPostVarSet) object;
                                if (Actor.isValid(mepvs.THIS)) MsgExplosion.send(mepvs.THIS, mepvs.chunkName, mepvs.p, mepvs.initiator,
                                        48.0F * mepvs.power, mepvs.power, 1, Math.max(mepvs.radius, 30.0F));
                            }
                        };
                    }
                }
    ...
    public static void forceGear(Class var_class, HierMesh hiermesh, float f) {
        try {
            Method method = var_class.getMethod("moveGear", new Class[] { HierMesh.class, Float.TYPE });
    ...
    public void prepareCamouflage() {
        String s = getPropertyMesh(this.getClass(), this._loadingCountry);
//        prepareMeshCamouflage(s, this.hierMesh());
        prepareMeshCamouflage(s, hierMesh(), getClass(), getRegiment());
    }

    public static void prepareMeshCamouflage(String s, HierMesh hiermesh) {
        prepareMeshCamouflage(s, hiermesh, null);
    }
    ...
    public static void prepareMeshCamouflage(String s, HierMesh hiermesh, String s1) {
        prepareMeshCamouflage(s, hiermesh, s1, null);
    }
    ...
    public static void prepareMeshCamouflage(String s, HierMesh hiermesh, String s1, Mat[] amat) {
        prepareMeshCamouflage(s, hiermesh, s1, amat, null, null);
    }
...
//                    s5 = "summer";
//                    if (!existSFSFile(s2 + s5 + "/skin1o.tga")) return;
//                }
//                s1 = s2 + s5;
                String s4 = s3 + s5;
            ...
            for (int i = 0; i < _skinMat.length; i++) {
                ...
                if (j >= 0) {
                    ...
                    if (flag) {
                        ...
                        Mat mat1;
                        if (FObj.Exist(s6)) mat1 = (Mat) FObj.Get(s6);
                        else {
                            mat1 = (Mat) mat.Clone();
                            mat1.Rename(s6);
                        ...
                        if (amat != null) for (int j1 = 0; j1 < 4; j1++)
    ...
    public static void prepareMeshSkin(String s, HierMesh hiermesh, String s1, String s2) {
        prepareMeshSkin(s, hiermesh, s1, s2, null);
    }
...
//        if (BmpUtils.bmp8PalTo4TGA4(s1, s3, s2) && s2 != null) prepareMeshCamouflage(s, hiermesh, s2, null);
...
//                            e.printStackTrace();
    ...
    public static boolean isWeaponDateOk(Class class1, String s) {
        ...
        String as[] = Aircraft.getWeaponHooksRegistered(class1);
        _WeaponSlot a_lweaponslot[] = (_WeaponSlot[]) hashmapint.get(i);
        if (as.length != a_lweaponslot.length) System.out.println("Weapon Slot Length mismatch on: " + class1.getName() + ", Weapon Slot: " + s);
        for (int k = 0; k < as.length; k++) {
            if (k >= a_lweaponslot.length) continue;
    ...
    protected static void weaponsLoad(Aircraft aircraft, int i, HashMapInt hashmapint) throws Exception {
        ...
        _WeaponSlot[] a_lweaponslot = (_WeaponSlot[]) hashmapint.get(i);
        if (strings.length != a_lweaponslot.length) {
            String slotName = "unknown";
            try {
                String[] weaponsRegistered = getWeaponsRegistered(aircraft.getClass());
                for (int weaponsRegisteredIndex=0; weaponsRegisteredIndex < weaponsRegistered.length; weaponsRegisteredIndex++) {
                    if (Finger.Int(weaponsRegistered[weaponsRegisteredIndex]) == i) {
                        slotName = weaponsRegistered[weaponsRegisteredIndex];
                    }
                }
            } catch (Exception e) {}
            System.out.println("Weapon Slot Length mismatch on: " + aircraft.getClass().getName() + ", Weapon Slot Info: thisWeaponsName=" + aircraft.thisWeaponsName + ", slotName=" + slotName + " - hooks length: " + strings.length + ", slot length: " + a_lweaponslot.length);
        }
        for (int hookIndex = 0; hookIndex < strings.length; hookIndex++) {
            try {
                if (hookIndex >= a_lweaponslot.length) continue;
                if (a_lweaponslot[hookIndex] != null) {
                    if (aircraft.mesh().hookFind(strings[hookIndex]) != -1) {
                        BulletEmitter bulletemitter = (BulletEmitter) a_lweaponslot[hookIndex].clazz.newInstance();
                        ...
                        if (aircraft.isNet() && aircraft.isNetMirror()) {
                            ...
                            else if (a_lweaponslot[hookIndex].trigger == 2 || a_lweaponslot[hookIndex].trigger == 3 || a_lweaponslot[hookIndex].trigger >= 10) {
                                if (a_lweaponslot[hookIndex].bullets == -12345)
                                    bulletemitter.loadBullets();
                                else
                                    bulletemitter._loadBullets(a_lweaponslot[hookIndex].bullets);
                            } else
                                bulletemitter.loadBullets(-1);
                        } else if (a_lweaponslot[hookIndex].bullets == -12345)
                            bulletemitter.loadBullets();
                        else
                            bulletemitter.loadBullets(a_lweaponslot[hookIndex].bullets);
                        aircraft.addGun(bulletemitter, a_lweaponslot[hookIndex].trigger);
                        Property.set(bulletemitter, "_count", a_lweaponslot[hookIndex].bullets);
                        switch (a_lweaponslot[hookIndex].trigger) {
                            case 0:
                        ...
                        System.err.println("Hook '" + strings[hookIndex] + "' NOT found in mesh of " + aircraft.getClass());
                    }
                }
            } catch (Exception e) {
                System.out.println("Error in weaponsLoad(" + aircraft.getClass().getName() + ", " + i + ", HashMapInt of size " + hashmapint.size());
                System.out.println("WeaponHooksRegistered length = " + strings.length + ", WeaponSlots length = " + a_lweaponslot.length);
                e.printStackTrace();
            }
        }
    }
    ...
    public boolean dropExternalStores(boolean flag) {
        ...
        for (int i = 0; i < this.FM.CT.Weapons.length; i++) {
            ...
            for (int j = 0; j < abulletemitter.length; j++) {
                ...
                if (!(bulletemitter instanceof BombGun) || bulletemitter instanceof FuelTankGun || bulletemitter instanceof BombGunNull) continue;
                if (bulletemitter.countBullets() <= 0) continue;
                ((BombGun) bulletemitter).setArmingTime(0xfffffffL);
                flag1 = true;
    ...
    private boolean dropWfrGr21() {
        if (!this.wfrGr21dropped) {
            ...
            if (this.wfrGr21dropped) {
                ...
                if (this.hierMesh().chunkFindCheck("WfrGr21L") < 0 || this.hierMesh().chunkFindCheck("WfrGr21R") < 0) return true;
                Vector3d vector3d = new Vector3d();
                vector3d.set(this.FM.Vwld);
                vector3d.z = vector3d.z - 6D;
                Wreckage wreckage = new Wreckage(this, this.hierMesh().chunkFind("WfrGr21L"));
                ...
                Wreckage wreckage1 = new Wreckage(this, this.hierMesh().chunkFind("WfrGr21R"));
