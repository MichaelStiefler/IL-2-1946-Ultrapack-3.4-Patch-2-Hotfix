...
//    private int yawMod360 = 0;
//    private boolean rniChange = true;
    
    private static Point3d acPos = new Point3d();
    private static Orientation acOr = new Orientation();

    public void onAircraftLoaded() {
        super.onAircraftLoaded();
        if (this == World.getPlayerAircraft() && Config.isCompassTestActive()) {
            this.pos.getAbs(acPos);
            acOr.set(this.FM.Or);
        }
    }
    ...
    public void update(float f) {
        super.update(f);
        // FIXME: +++ Compass Test +++
        if (this == World.getPlayerAircraft() && Config.isCompassTestActive() && this.FM instanceof RealFlightModel && ((RealFlightModel)this.FM).isRealMode()) {
            World.cur().diffCur.RealisticNavigationInstruments = Main3D.cur3D().cockpitCur.cockpitLightControl | this.FM.AS.bNavLightsOn;
            acOr.increment(this.FM.CT.RudderControl, this.FM.Gears.onGround()?0:this.FM.CT.ElevatorControl, this.FM.Gears.onGround()?0:this.FM.CT.AileronControl);
            this.pos.setAbs(acPos);
            this.FM.Or.set(acOr);
            this.FM.Vrel.set(0,0,0);
            this.FM.Vwld.set(0,0,0);
            Vector3d ACmeter = (Vector3d)Reflection.getValue(this.FM, "ACmeter");
            ACmeter.set(0, 0, 1);
            Reflection.setFloat(this.FM, "knockUpTime", 0);
            Reflection.setFloat(this.FM, "knockDnTime", 0);
            Reflection.setFloat(this.FM, "currDeep", 0);
            Reflection.setFloat(this.FM, "indiffDnTime", 100);
            Reflection.setFloat(this.FM, "indiffUpTime", 100);
        }
        //        --- Compass Test ---
...
//    private boolean isNavLightOn = false;
...
//            System.out.println("Hit Chunk added: " + partNames[partNameIndex]);
//        }
//        
//        // Add Props...
//        int propIndex = 0;
...
//            System.out.println("Hit Chunk added: Prop" + propIndex);
//        }
//    }
//    
//    // Damage by forest collision
//    private void damageByForest() {
//        if (this.FM.AS.bNavLightsOn == isNavLightOn)
//            return;
//        isNavLightOn = this.FM.AS.bNavLightsOn;
...
//                        System.out.println("Damage applied to prop " + sPropNum);
...
//            System.out.println("Damage applied to " + chunkName);
//            
...
//                System.out.println("Chunk " + chunkName + " removed from hit list, apparently it's been cut!");
//            }
//        } catch (Exception e) {
//            e.printStackTrace();                                                                // If an error occurs, log it.
//        }
...
//        }
    ...
    public void applyEngineShake() {
        ...
        if (this.fEngineShakeLevel == null || this.fEngineShakeLevel.length < this.FM.EI.getNum()) this.fEngineShakeLevel = new float[this.FM.EI.getNum()]; // initialize damaged engines array if necessary.
        ...
        for (int i = 0; i < this.FM.EI.getNum(); i++) { // check each engine's stage
            ...
            fShakeFactor *= Aircraft.cvt(this.FM.M.massEmpty, 400F, 2000F, 0.25F, 1F); // Reduce Shake effect for leightweight AC
            fShakeFactor *= CommonTools.smoothCvt(this.FM.getSpeedKMH(), 0F, 100F, 0.5F, 1F); // Reduce Shake effect while AC is standing still / rolling slow
            if (!((RealFlightModel)this.FM).isRealMode()) fShakeFactor *= 0.2F; // Significantly reduce Shake effect for autopilot
