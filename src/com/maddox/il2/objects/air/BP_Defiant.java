...
public class BP_Defiant extends DefiantRAF implements TypeTNBFighter, TypeNetAircraft2023 {
    ...
    public void update(float f) {
        super.update(f);
        if ((this.FM == null) || (this.FM.CT == null)) {
            return;
        }
        ...
        switch (this.turretMode) {
            ...
            case TURRET_MODE_LOCKED_FWD:
                if ((this.FM.CT.WeaponControl != null) && (this.FM.CT.WeaponControl.length > 10)) {
                    this.FM.CT.WeaponControl[10] = this.FM.CT.WeaponControl[0];
                }
            case TURRET_MODE_LOCKED_AFT:
                if ((this.FM.turret != null) && (this.FM.turret.length > 0) && (this.FM.turret[0] != null)) {
        ...
        if ((this.FM.CT.Weapons == null) || (this.FM.CT.Weapons.length < 11) || (this.FM.CT.Weapons[0] == null) || (this.FM.CT.Weapons[0].length == 0)) {
            return;
        }

        if (this.FM.CT.Weapons[0][0] instanceof MGunDummy) {
            MGunDummy gun = (MGunDummy) this.FM.CT.Weapons[0][0];
            if ((this.FM.CT.Weapons[10].length > 0) && this.FM.CT.Weapons[10][0].haveBullets() && (this.FM.turret[0].health > 0F)) {
                gun.loadBullets();
            } else {
                gun.emptyGun();
            }
        }
//        if (this.net != null && !this.net.isMaster()) this.FM.turret[0].bIsAIControlled = false;
    }
...
//    float oldElevation = 0F;
    ...
    private void turretFollowsPilotHead(float tickLen) {
        ...
        if (!this.FM.isPlayers() || !(this.FM instanceof RealFlightModel) || !((RealFlightModel) this.FM).isRealMode() || !Config.isUSE_RENDER()) {
            return;
        }
...
//        this.FM.turret[0].tu[0] = this.FM.turret[0].tuLim[0];
//        this.FM.turret[0].tu[1] = this.FM.turret[0].tuLim[1];
    ...
    private int turretElevationToNet(float elevation) {
        return Math.round(CommonTools.cvt(elevation, BP_Defiant.TURRET_MIN_ELEVATION, BP_Defiant.TURRET_MAX_ELEVATION, 0, BP_Defiant.TURRET_MAX_ELEVATION_INT));
    }

    private int turretAzimuthToNet(float azimuth) {
        while (azimuth < 0F) {
            azimuth += 360F;
        }
        return Math.round(CommonTools.cvt(azimuth, BP_Defiant.TURRET_MIN_AZIMUTH, BP_Defiant.TURRET_MAX_AZIMUTH, 0, BP_Defiant.TURRET_MAX_AZIMUTH_INT));
    }

    private float turretNetElevationToFloat(int elevation) {
        return CommonTools.cvt(elevation, 0, BP_Defiant.TURRET_MAX_ELEVATION_INT, BP_Defiant.TURRET_MIN_ELEVATION, BP_Defiant.TURRET_MAX_ELEVATION);
    }

    private float turretNetAzimuthToFloat(int azimuth) {
        return CommonTools.cvt(azimuth, 0, BP_Defiant.TURRET_MAX_AZIMUTH_INT, BP_Defiant.TURRET_MIN_AZIMUTH, BP_Defiant.TURRET_MAX_AZIMUTH);
    }

    // TODO: Storebror: +++ New Generic Net Replication for Aircraft Classes
    public void netUpdateAircraft(NetMsgFiltered out) {
        try {
            out.writeByte(((int) CommonTools.cvt(this.getTurretLockPosition(), 0F, 1F, 0, 0x3F) << 2) + (this.getTurretMode() & 0x03F));
            out.writeShort((short) (((this.turretAzimuthToNet(this.FM.turret[0].tuLim[0]) & BP_Defiant.TURRET_MAX_AZIMUTH_INT) << 7) + (this.turretElevationToNet(this.FM.turret[0].tuLim[1]) & BP_Defiant.TURRET_MAX_ELEVATION_INT)));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public void netInputAircraft(NetMsgInput in) {
        try {
            if (in.available() < 3) {
                return;
            }
            int turretState = in.readUnsignedByte();
            this.turretLockPosition = CommonTools.cvt((turretState >>> 2) & 0x3F, 0, 0x3F, 0F, 1F);
            this.animateFairings(this.turretLockPosition);
            this.turretMode = (byte) (turretState & 0x03F);
            if (this.turretMode < BP_Defiant.TURRET_MODE_NORMAL) {
                this.turretMode = BP_Defiant.TURRET_MODE_NORMAL;
            }
            if (this.turretMode > BP_Defiant.TURRET_MODE_MAX) {
                this.turretMode = BP_Defiant.TURRET_MODE_MAX;
            }
            short turretAngles = (short) in.readUnsignedShort();
            int elevation = turretAngles & BP_Defiant.TURRET_MAX_ELEVATION_INT;
            int azimuth = (turretAngles >>> 7) & BP_Defiant.TURRET_MAX_AZIMUTH_INT;
            this.updateRotation(this.turretNetAzimuthToFloat(azimuth), this.turretNetElevationToFloat(elevation), Time.tickLenFs());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private static final float    TURRET_MIN_ELEVATION          = 0F;
    private static final float    TURRET_MAX_ELEVATION          = 84F;
    private static final int      TURRET_MAX_ELEVATION_INT      = 0x7F;
    private static final float    TURRET_MIN_AZIMUTH            = 0F;
    private static final float    TURRET_MAX_AZIMUTH            = 360F;
    private static final int      TURRET_MAX_AZIMUTH_INT        = 0x1FF;
