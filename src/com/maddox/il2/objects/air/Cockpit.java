...
public abstract class Cockpit extends Actor {
    ...
    protected void doShowPilot() {
        this.setPilotMatVisDist(0.0F);
    }

    public void doHidePilot() {
        this.setPilotMatVisDist(15F);
    }

    private void setPilotMatVisDist(float f) {
        if (this.bExistMirror || this.hidePilot) {
            Integer ainteger[] = this.findPilotMats();
            if (ainteger != null) {
    ...
    class NullDraw extends ActorDraw {
        ...
        public void render() {
            if ((this.iPreRender != 0) && Actor.isValid(Cockpit.this.pos.base()) && (Cockpit.this.nullMesh != null) && (Cockpit.this.nullMeshIL2 != null)) {
                if (Cockpit.this.bEnableRenderingBall) {
    ...
    class Draw extends ActorDraw {
        ...
        public void render(boolean bool) {
            if ((this.iPreRender != 0) && Actor.isValid(Cockpit.this.pos.base()) && (Cockpit.this.mesh != null)) {
                ...
                } else if (!Cockpit.this.bExistMirror) {
                    Cockpit.this.mesh.render();
                } else {
    ...
    static class HookCamera3DMirror extends HookRender {
        ...
        public void computePos(Actor actor, Loc loc, Loc loc1, boolean bool) {
            if (Actor.isValid(Main3D.cur3D().cockpitCur) && Main3D.cur3D().cockpitCur.bExistMirror && Main3D.cur3D().isViewMirror() && Main3D.cur3D().cockpitCur.isFocused() && Actor.isValid(World.getPlayerAircraft())) {
                ...
                float f = this.computeMirroredCamera(loc2, this.mir2w_loc, this.mirLowP, this.mirHigP, Main3D.cur3D().mirrorWidth(), Main3D.cur3D().mirrorHeight(), 1.0F, loc1, this.bInCockpit ? Main3D.cur3D().cockpitCur.cockpit_renderwin : Main3D.cur3D().cockpitCur.world_renderwin);
                if (bool) {
                    if (this.bInCockpit) {
    ...
    protected void resetYPRmodifier() {
        Cockpit.ypr[0] = Cockpit.ypr[1] = Cockpit.ypr[2] = Cockpit.xyz[0] = Cockpit.xyz[1] = Cockpit.xyz[2] = 0.0F;
    }

    public int astatePilotIndx() {
        if (this._astatePilotIndx == -1) {
            this._astatePilotIndx = Property.intValue(this.getClass(), "astatePilotIndx", 0);
        }
    ...
    protected void initSounds() {
        if (Cockpit.sfxPreset == null) {
            Cockpit.sfxPreset = new SoundPreset("aircraft.cockpit");
        }
    ...
    protected void sfxStart(int i) {
        if ((this.sounds != null) && (this.sounds.length > i)) {
            ...
            if (soundfx == null) {
                soundfx = this.aircraft().newSound(Cockpit.sfxPreset, false, false);
                if (soundfx == null) {
                    return;
                }
    ...
    protected void sfxStop(int i) {
        if ((this.sounds != null) && (this.sounds.length > i) && (this.sounds[i] != null)) {
            this.sounds[i].stop();
        }
    }
    ...
    protected void sfxSetAcoustics(Acoustics acoustics) {
        for (int i = 0; i < this.sounds.length; i++) {
            if (this.sounds[i] != null) {
                this.sounds[i].setAcoustics(acoustics);
            }
        }
    }
    ...
    public void onDoorMoved(float f) {
        if ((this.acoustics != null) && (this.acoustics.globFX != null)) {
            this.acoustics.globFX.set(1.0F - f);
        }
    }
            ...
            for (int i = 0; i < this.cockpitNightMats.length; i++) {
                ...
                if (i1 < 0) {
                    if (World.cur().isDebugFM()) {
                        System.out.println(" * * * * * did not find " + this.cockpitNightMats[i] + "_night");
                    }
                } else {
                    ...
                    if (mat.isValidLayer(0)) {
                        ...
                        System.out.println(" * * * * * " + this.cockpitNightMats[i] + "_night layer 0 invalid");
                    }
                }
            }
        }
    ...
    public boolean isEnableFocusing() {
        if (!Actor.isValid(this.aircraft()) || this.aircraft().FM.AS.isPilotParatrooper(this.astatePilotIndx())) {
            return false;
        }
        return true;
    }
    ...
    public boolean focusEnter() {
        if (!this.isEnableFocusing()) {
            return false;
        }
        if (this.bFocus) {
            return true;
        }
        if (!this.doFocusEnter()) {
            return false;
        }
        this.bFocus = true;
    ...
    public void focusLeave() {
        if (this.bFocus) {
            ...
            if (!this.isEnableHotKeysOnOutsideView()) {
    ...
    public void preRender(boolean bool) {
        ...
        if (this.bEnableRendering) {
            if (this.bNullShow) {
                this.drawNullMesh.preRender();
            } else {
    ...
    public void render(boolean bool) {
        if (this.bEnableRendering) {
            if (this.bNullShow) {
                this.drawNullMesh.render();
            } else {
                this.drawMesh.render(bool);
            }
        }
    }
    ...
    public Cockpit(String string, String string_48_) {
        ...
        // ---
        this.fm = Cockpit._newAircraft.FM;
    ...
    public static void lightningStrike(Point3d point3d) {
        Cockpit.lightningPoint = point3d;
        Cockpit.lightningStriked = true;
    }

    private void updateBeacons() {
        if (Cockpit.lightningStriked) {
            Cockpit.lightningStriked = false;
            Cockpit.V.sub(this.fm.Loc, Cockpit.lightningPoint);
            float f = (float) Cockpit.V.length();
            Cockpit.atmosphereInterference = this.cvt(f, 1000.0F, 9000.0F, 1.0F, 0.0F);
        } else if (Cockpit.atmosphereInterference > 0.01F) {
            Cockpit.atmosphereInterference *= 0.98;
        }
        if (this.bFocus) {
            Aircraft aircraft = this.aircraft();
            if (aircraft.FM.AS.listenLorenzBlindLanding) {
                this.listenLorenzBlindLanding(aircraft);
            } else if (aircraft.FM.AS.listenNDBeacon) {
                this.listenNDBeacon(aircraft, false);
            } else if (aircraft.FM.AS.listenRadioStation) {
                this.listenNDBeacon(aircraft, true);
                ...
                if (actor.isAlive()) {
                    this.playYEYGMorseCode(aircraft, actor, "DWRKANUGMLFS".toCharArray());
                }
        ...
        // TODO: +++ Make sure azimuth, waypointAzimuth and waypointDirection have been initialized.
        this.initAnglesForkFields();
        //       ---
    }
    
    // TODO: +++ Make sure azimuth, waypointAzimuth and waypointDirection have been initialized.
    private boolean initAnglesForkFieldsDone = false;
    
    private void initAnglesForkFields() {
        if (initAnglesForkFieldsDone) return;
        initAnglesForkFieldsDone = true;
        this.initAnglesForkFields(this.getClass());
        Class superClass = this.getClass().getSuperclass();
        if (!superClass.equals(Actor.class) && !superClass.equals(Cockpit.class) && !superClass.getSuperclass().equals(Cockpit.class) && !superClass.getSuperclass().equals(Actor.class)) {
            this.initAnglesForkFields(this.getClass().getSuperclass());
        }
        
//        System.out.println("+++ " + this.getClass().getName() + " Fields...");
    }
    
    private void initAnglesForkFields(Class clazz) {
        Field fields[] = clazz.getDeclaredFields();
        for (int fieldIndex=0; fieldIndex<fields.length; fieldIndex++) {
            if (!checkAnglesFork(this, fields[fieldIndex])) {
                fields[fieldIndex].setAccessible(true);
                try {
                    Object obj = fields[fieldIndex].get(this);
                    if (obj == null) continue;
//                    System.out.println("+++ " + obj.getClass().getName() + " Inner Fields...");
                    Field innerFields[] = obj.getClass().getDeclaredFields();
                    for (int innerFieldIndex=0; innerFieldIndex<innerFields.length; innerFieldIndex++) {
                        checkAnglesFork(obj, innerFields[innerFieldIndex]);
                    }
    ...
    private boolean checkAnglesFork(Object obj, Field field) {
//        System.out.println("+++ " + this.getClass().getName() + " field.getType()=" + field.getType().getName());
        if (field.getType().isPrimitive()) return false;
        if (field.getType().equals(AnglesFork.class)) {
            field.setAccessible(true);
            initAnglesFork(obj, field);
            return true;
        }
        return false;
    }
    
    private void initAnglesFork(Object obj, Field field) {
        try {
            if (field.get(obj) == null) {
                System.out.println("+++ " + this.getClass().getName() + " needs AnglesFork Init!!! +++");
                field.setAccessible(true);
                field.set(obj, new AnglesFork());
            }
        } catch (IllegalArgumentException e) {
            e.printStackTrace();
        } catch (IllegalAccessException e) {
    ...
    private void playYEYGMorseCode(Aircraft aircraft, Actor actor, char[] cs) {
        ...
        if (f2 > f3) {
            ...
            if (f4 != 1.0F) {
                ...
                if (((i % 13) == 0) && (!aircraft.isMorseSequencePlaying() || this.clearToPlay)) {
                    if (bool) {
    ...
    public float getBeaconRange() {
        Aircraft aircraft = this.aircraft();
        if (aircraft.FM.AS.listenLorenzBlindLanding) {
            return 7.0422F * ((1.0F - this.blData.signalStrength) + World.Rnd().nextFloat(-Cockpit.atmosphereInterference * 2.0F, Cockpit.atmosphereInterference * 2.0F));
        }
        if (this.useRealisticNavigationInstruments()) {
            return this.ndBeaconRange + ((float) Math.random() * 0.2F * this.ndBeaconRange) + World.Rnd().nextFloat(-Cockpit.atmosphereInterference * 2.0F, Cockpit.atmosphereInterference * 2.0F);
        }
        return 1.0F;
    }
    ...
    public float getGlidePath() {
        ...
        if (aircraft1.FM.AS.listenLorenzBlindLanding) {
            ...
            double d6 = Cockpit.glideScopeInRads - d5;
            float f2 = ((float) Math.toDegrees(d6) * this.blData.signalStrength) + World.Rnd().nextFloat(-Cockpit.atmosphereInterference * 2.0F, Cockpit.atmosphereInterference * 2.0F);
            if (f2 > 1.0F) {
                f2 = 1.0F;
            } else if (f2 < -1F) {
                f2 = -1F;
    ...
    public float getBeaconDirection() {
        Aircraft aircraft = this.aircraft();
        if (this.bFocus && aircraft.FM.AS.listenLorenzBlindLanding) {
            return this.blData.blindLandingAzimuthPB + World.Rnd().nextFloat(-Cockpit.atmosphereInterference * 90.0F, Cockpit.atmosphereInterference * 90.0F);
        }
        if (this.useRealisticNavigationInstruments() && Main.cur().mission.hasBeacons(this.fm.actor.getArmy())) {
            return this.ndBeaconDirection + World.Rnd().nextFloat(-Cockpit.atmosphereInterference * 90.0F, Cockpit.atmosphereInterference * 90.0F);
        }
        return 0.0F;
    }
    ...
    private void listenNDBeacon(Aircraft aircraft, boolean bool) {
        ...
        if ((actor == null) || !actor.isAlive()) {
            ...
            if (Math.random() < 0.02) {
                Cockpit.terrainAndNightError = BeaconGeneric.getTerrainAndNightError(aircraft);
            }
            f1 += Cockpit.terrainAndNightError;
            float f2 = this.floatindex(this.cvt(1.0F - this.ndBeaconRange, 0.0F, 1.0F, 0.0F, 9.0F), Cockpit.volumeLogScale);
    ...
    private float getRadioCompassWaypoint(boolean flag, boolean flag1, boolean flag2) {
        ...
        this.tempPoint1.y = actor.pos.getAbsPoint().y;
        this.tempPoint1.z = actor.pos.getAbsPoint().z + 20.0;
        Cockpit.V.sub(this.tempPoint1, this.fm.Loc);
        float f;
        if (flag) {
            if (flag1) {
                f = (float) Math.toDegrees(Math.atan2(-Cockpit.V.y, Cockpit.V.x));
            } else {
                f = (float) Math.toDegrees(Math.atan2(Cockpit.V.y, Cockpit.V.x));
            }
        } else {
            f = (float) Math.toDegrees(Math.atan2(Cockpit.V.x, Cockpit.V.y));
        }
        while (f <= -180.0F) {
            f += 180.0F;
        }
        ...
        f += Cockpit.terrainAndNightError;
        if (this.ndBeaconRange > 0.99) {
            f = this.aircraft().FM.Or.azimut();
        } else {
            f += World.Rnd().nextFloat(-this.ndBeaconRange - (Cockpit.atmosphereInterference * 5.0F), this.ndBeaconRange + (Cockpit.atmosphereInterference * 5.0F)) * 30.0F;
        }
        return f;
    }
    ...
    private float getWaypoint(boolean flag, boolean flag1, float f, boolean flag2) {
        ...
        if ((this.fm == null) || (this.fm.AP == null) || (this.fm.AP.way == null)) {
            return 0F;
        }
        ...
        if (flag) {
            if (flag1) {
                f2 = (float) Math.toDegrees(Math.atan2(-Cockpit.V.y, Cockpit.V.x));
            } else {
                f2 = (float) Math.toDegrees(Math.atan2(Cockpit.V.y, Cockpit.V.x));
            }
        } else {
            f2 = (float) Math.toDegrees(Math.atan2(Cockpit.V.x, Cockpit.V.y));
        }
        while (f2 <= -180.0F) {
            f2 += 180.0F;
        }
