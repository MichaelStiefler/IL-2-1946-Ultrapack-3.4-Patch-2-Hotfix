...
public class CockpitASVII_standin extends CockpitPilot {
    ...
    public CockpitASVII_standin() {
        ...
        this.targetAngle = new int[CockpitASVII_standin.MAX_TARGETS];
    ...
    public void draw() {
        ...
        if (i != this.oldBeamAngleInt) {
            for (int j = 0; j < CockpitASVII_standin.MAX_TARGETS; j++) {
                boolean flag = false;
                if (i > this.oldBeamAngleInt) {
            ...
            for (int k = 0; k < this.radarPlane.size(); k++) {
                ...
                float f2 = (float) (Math.sqrt((f * f) + (f1 * f1)) / Math.sqrt((f * f) + (f1 * f1))) / this.distanceMax / CockpitASVII_standin.SCREEN_PLOT_FACTOR;
                float f4 = f * f2;
                float f6 = f1 * f2;
                float f7;
                ...
                if (flag1) {
                    ...
                    if (this.targetTrackingNumber >= CockpitASVII_standin.MAX_TARGETS) {
                        this.targetTrackingNumber = 0;
                    }
    ...
    private static final float SCREEN_PLOT_FACTOR = 25F;
