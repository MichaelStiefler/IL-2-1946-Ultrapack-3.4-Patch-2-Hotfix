...
public class CockpitBuchon extends CockpitPilot {
    class Interpolater extends InterpolateRef {
        ...
        public boolean tick() {
            CockpitBuchon.this.setTmp = CockpitBuchon.this.setOld;
            CockpitBuchon.this.setOld = CockpitBuchon.this.setNew;
            CockpitBuchon.this.setNew = CockpitBuchon.this.setTmp;
            CockpitBuchon.this.setNew.altimeter = CockpitBuchon.this.fm.getAltitude();
            if (CockpitBuchon.this.cockpitDimControl) {
                if (CockpitBuchon.this.setNew.dimPosition > 0.0F) {
                    CockpitBuchon.this.setNew.dimPosition = CockpitBuchon.this.setOld.dimPosition - 0.05F;
                }
            } else if (CockpitBuchon.this.setNew.dimPosition < 1.0F) {
                CockpitBuchon.this.setNew.dimPosition = CockpitBuchon.this.setOld.dimPosition + 0.05F;
            }
            CockpitBuchon.this.setNew.throttle = ((10F * CockpitBuchon.this.setOld.throttle) + CockpitBuchon.this.fm.CT.PowerControl) / 11F;
            CockpitBuchon.this.setNew.mix = ((8F * CockpitBuchon.this.setOld.mix) + CockpitBuchon.this.fm.EI.engines[0].getControlMix()) / 9F;
            float f = CockpitBuchon.this.waypointAzimuth();
            if (CockpitBuchon.this.useRealisticNavigationInstruments()) {
                CockpitBuchon.this.setNew.waypointAzimuth.setDeg(f - 90F);
                CockpitBuchon.this.setOld.waypointAzimuth.setDeg(f - 90F);
            } else {
                CockpitBuchon.this.setNew.waypointAzimuth.setDeg(CockpitBuchon.this.setOld.waypointAzimuth.getDeg(0.1F), f - CockpitBuchon.this.setOld.azimuth.getDeg(1.0F));
            }
            if (Math.abs(CockpitBuchon.this.fm.Or.getKren()) < 30F) {
                CockpitBuchon.this.setNew.azimuth.setDeg(CockpitBuchon.this.setOld.azimuth.getDeg(1.0F), CockpitBuchon.this.fm.Or.azimut());
            }
            CockpitBuchon.this.w.set(CockpitBuchon.this.fm.getW());
            CockpitBuchon.this.fm.Or.transform(CockpitBuchon.this.w);
            CockpitBuchon.this.setNew.turn = ((12F * CockpitBuchon.this.setOld.turn) + CockpitBuchon.this.w.z) / 13F;
            CockpitBuchon.this.buzzerFX((CockpitBuchon.this.fm.CT.getGear() < 0.999999F) && (CockpitBuchon.this.fm.CT.getFlap() > 0.0F));
            return true;
        }
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        this.mesh.chunkSetAngles("Z_Speedometer1", this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeedKMH()), 0.0F, 800F, 0.0F, 16F), CockpitBuchon.speedometerScale), 0.0F, 0.0F);
        this.mesh.chunkSetAngles("Z_RPM1", this.floatindex(this.cvt(this.fm.EI.engines[0].getRPM(), 0.0F, 4000F, 0.0F, 8F), CockpitBuchon.rpmScale), 0.0F, 0.0F);
        this.mesh.chunkSetAngles("Z_FuelQuantity1", -44.5F + this.floatindex(this.cvt(this.fm.M.fuel / 0.72F, 0.0F, 400F, 0.0F, 8F), CockpitBuchon.fuelScale), 0.0F, 0.0F);
