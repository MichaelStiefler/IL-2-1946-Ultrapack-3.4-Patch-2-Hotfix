...
//                setNew.azimuth.setDeg(setOld.azimuth.getDeg(1.0F), fm.Or.azimut());
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        if (f3 > 360F) {
            this.mesh.chunkSetAngles("Z_Need_speed", this.cvt(f3, 360F, 600F, -329F, -550F), 0.0F, 0.0F);
        } else {
            this.mesh.chunkSetAngles("Z_Need_speed", this.cvt(f3, 60F, 360F, 0.0F, -329F), 0.0F, 0.0F);
        }
        // TODO: +++ Fix DXXI Danish/Dutch/Sarja4 Compass
...
//        mesh.chunkSetAngles("Z_Need_compass", f1, 0.0F, 0.0F);
    ...
    private void initCompass() {
        ...
        this.segLen1 = 2D * Math.sqrt(1.0D - (CockpitDXXI_SARJA4.compassZ * CockpitDXXI_SARJA4.compassZ));
        this.segLen2 = this.segLen1 / Math.sqrt(2D);
        this.compassLimit = -1D * Math.sin(0.01745328888888889D * CockpitDXXI_SARJA4.compassLimitAngle);
    ...
    private void updateCompass() {
        ...
        if (this.accel.length() > (-CockpitDXXI_SARJA4.compassZ * 0.7D)) {
            this.accel.scale((-CockpitDXXI_SARJA4.compassZ * 0.7D) / this.accel.length());
        }
        for (int i = 0; i < 4; i++) {
        ...
        vector3d1.normalize();
        vector3d1.scale(-CockpitDXXI_SARJA4.compassZ);
