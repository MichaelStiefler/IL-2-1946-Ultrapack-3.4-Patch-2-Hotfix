...
public class CockpitF2A2 extends CockpitPilot {
    class Interpolater extends InterpolateRef {

        public boolean tick() {
            if (CockpitF2A2.this.fm != null) {
                CockpitF2A2.this.setTmp = CockpitF2A2.this.setOld;
                CockpitF2A2.this.setOld = CockpitF2A2.this.setNew;
                CockpitF2A2.this.setNew = CockpitF2A2.this.setTmp;
                CockpitF2A2.this.setNew.throttle = ((10F * CockpitF2A2.this.setOld.throttle) + CockpitF2A2.this.fm.CT.PowerControl) / 11F;
                CockpitF2A2.this.setNew.altimeter = CockpitF2A2.this.fm.getAltitude();
                CockpitF2A2.this.setNew.waypointAzimuth.setDeg(CockpitF2A2.this.setOld.waypointAzimuth.getDeg(0.1F), (CockpitF2A2.this.waypointAzimuth() - CockpitF2A2.this.setOld.azimuth.getDeg(1.0F)) + World.Rnd().nextFloat(-10F, 10F));
                if (Math.abs(CockpitF2A2.this.fm.Or.getKren()) < 30F) {
                    CockpitF2A2.this.setNew.azimuth.setDeg(CockpitF2A2.this.setOld.azimuth.getDeg(1.0F), CockpitF2A2.this.fm.Or.azimut());
                }
                CockpitF2A2.this.setNew.vspeed = ((199F * CockpitF2A2.this.setOld.vspeed) + CockpitF2A2.this.fm.getVertSpeed()) / 200F;
            }
            return true;
        }
    ...
    protected void doFocusLeave() {
        if (!this.isFocused()) {
            ...
            this.leave();
            ((F2A2) this.aircraft()).bChangedExts = true;
            ((F2A2) this.aircraft()).doFixBellyDoor();
            super.doFocusLeave();
            return;
        }
    }
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        if (F2A2.bChangedPit) {
            ...
            this.aircraft().hierMesh().chunkVisible("Engine11_D1", false);
            this.aircraft().hierMesh().chunkVisible("Engine11_D2", false);
            ((F2A2) this.aircraft()).bChangedExts = false;
            this.aircraft();
            F2A2.bChangedPit = false;
        }
        if (this.gun[0] == null) {
        ...
        this.mesh.chunkSetAngles("Z_Speed", this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeedKMH()), 0.0F, 981.5598F, 0.0F, 53F), CockpitF2A2.speedometerScale), 0.0F, 0.0F);
