...
public class CockpitF4F3 extends CockpitPilot {
    class Interpolater extends InterpolateRef {
        ...
        public boolean tick() {
            if (CockpitF4F3.this.fm != null) {
                CockpitF4F3.this.setTmp = CockpitF4F3.this.setOld;
                CockpitF4F3.this.setOld = CockpitF4F3.this.setNew;
                CockpitF4F3.this.setNew = CockpitF4F3.this.setTmp;
                CockpitF4F3.this.setNew.throttle = (0.85F * CockpitF4F3.this.setOld.throttle) + (CockpitF4F3.this.fm.CT.PowerControl * 0.15F);
                CockpitF4F3.this.setNew.prop = (0.85F * CockpitF4F3.this.setOld.prop) + (CockpitF4F3.this.fm.CT.getStepControl() * 0.15F);
                CockpitF4F3.this.setNew.mix = (0.85F * CockpitF4F3.this.setOld.mix) + (CockpitF4F3.this.fm.EI.engines[0].getControlMix() * 0.15F);
                CockpitF4F3.this.setNew.altimeter = CockpitF4F3.this.fm.getAltitude();
                float f = CockpitF4F3.this.waypointAzimuth(10F);
                if (Math.abs(CockpitF4F3.this.fm.Or.getKren()) < 30F) {
                    CockpitF4F3.this.setNew.azimuth.setDeg(CockpitF4F3.this.setOld.azimuth.getDeg(1.0F), CockpitF4F3.this.fm.Or.azimut());
                }
                CockpitF4F3.this.setNew.waypointAzimuth.setDeg(CockpitF4F3.this.setOld.waypointAzimuth.getDeg(0.1F), f);
                CockpitF4F3.this.setNew.vspeed = ((199F * CockpitF4F3.this.setOld.vspeed) + CockpitF4F3.this.fm.getVertSpeed()) / 200F;
            }
            boolean flag = false;
            if (CockpitF4F3.this.setNew.gCrankAngle < (CockpitF4F3.this.fm.CT.getGear() - 0.05F)) {
                if ((Math.abs(CockpitF4F3.this.setNew.gCrankAngle - CockpitF4F3.this.fm.CT.getGear()) > 0.05F) && (Math.abs(CockpitF4F3.this.setNew.gCrankAngle - CockpitF4F3.this.fm.CT.getGear()) < 0.9F)) {
                    CockpitF4F3.this.setNew.gCrankAngle += 0.0025F;
                    flag = true;
                } else {
                    CockpitF4F3.this.setNew.gCrankAngle = CockpitF4F3.this.fm.CT.getGear();
                    CockpitF4F3.this.setOld.gCrankAngle = CockpitF4F3.this.fm.CT.getGear();
                }
            }
            if (CockpitF4F3.this.setNew.gCrankAngle > (CockpitF4F3.this.fm.CT.getGear() + 0.05F)) {
                if ((Math.abs(CockpitF4F3.this.setNew.gCrankAngle - CockpitF4F3.this.fm.CT.getGear()) > 0.05F) && (Math.abs(CockpitF4F3.this.setNew.gCrankAngle - CockpitF4F3.this.fm.CT.getGear()) < 0.9F)) {
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        this.mesh.chunkSetAngles("Z_Speedometer1", 0.0F, this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeedKMH()), 0.0F, 692.018F, 0.0F, 11F), CockpitF4F3.speedometerScale), 0.0F);
        ...
        this.mesh.chunkSetAngles("Z_Climb1", 0.0F, this.floatindex(this.cvt(this.setNew.vspeed, -30.48F, 30.48F, 0.0F, 12F), CockpitF4F3.variometerScale), 0.0F);
    ...
    protected void reflectPlaneMats() {
        ...
        if (PaintScheme.countryUSA.equals(this.aircraft().getRegiment().country())) {
            Mat mat1 = hiermesh.material(hiermesh.materialFind("Overlay1"));
            this.mesh.materialReplace("Overlay1", mat1);
            mat1 = hiermesh.material(hiermesh.materialFind("OverlayD1o"));
            this.mesh.materialReplace("OverlayD1o", mat1);
            mat1 = hiermesh.material(hiermesh.materialFind("OverlayD2o"));
            this.mesh.materialReplace("OverlayD2o", mat1);
        }
    }
