...
public class CockpitFW_187A extends CockpitPilot {
    class Interpolater extends InterpolateRef {
        ...
        public boolean tick() {
            ...
            float f = CockpitFW_187A.this.waypointAzimuth();
            System.out.println("waypointAzimuth=" + f);
            if (CockpitFW_187A.this.useRealisticNavigationInstruments()) {
                CockpitFW_187A.this.setNew.waypointAzimuth.setDeg(f - 90.0F);
                CockpitFW_187A.this.setOld.waypointAzimuth.setDeg(f - 90.0F);
            } else {
                CockpitFW_187A.this.setNew.waypointAzimuth.setDeg(CockpitFW_187A.this.setOld.waypointAzimuth.getDeg(0.1F), f - CockpitFW_187A.this.setOld.azimuth.getDeg(1.0F));
            }
            if (Math.abs(CockpitFW_187A.this.fm.Or.getKren()) < 30F) {
                CockpitFW_187A.this.setNew.azimuth.setDeg(CockpitFW_187A.this.setOld.azimuth.getDeg(1.0F), CockpitFW_187A.this.fm.Or.azimut());
            }
...
//        this.tmpV = new Vector3d();
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        Cockpit.ypr[2] = this.cvt(this.fm.CT.getCockpitDoor(), 0.1F, 0.99F, 0.0F, -2.0F);
        ...
        this.mesh.chunkSetAngles("Z_FuelQuantity1", -44.5F + this.floatindex(this.cvt(this.fm.M.fuel / 0.72F, 0.0F, 900F, 0.0F, 8F), CockpitFW_187A.fuelScale), 0.0F, 0.0F);
...
//        this.mesh.chunkSetAngles("Z_Azimuth1", this.setNew.waypointAzimuth.getDeg(f), 0.0F, 0.0F);
    ...
    protected boolean doFocusEnter() {
        if (super.doFocusEnter()) {
            ...
            if (this.aircraft().hierMesh().chunkFindCheck("Blister2_D0") != -1) {
