...
public class CockpitG10N1 extends CockpitPilot {
    ...
    class Interpolater extends InterpolateRef {
        ...
        public boolean tick() {
            if (CockpitG10N1.this.fm != null) {
                ...
                if (Math.abs(CockpitG10N1.this.fm.Or.getKren()) < 30F) {
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        for (int i = 0; i < 6; i++) {
            CockpitG10N1.tmpstr = "Z_PropPitch" + ((i * 2) + 1);
            this.mesh.chunkSetAngles(CockpitG10N1.tmpstr, 0.0F, -54F * this.interp(this.setNew.prop[i], this.setOld.prop[i], f), 0.0F);
        }

        for (int j = 0; j < 6; j++) {
            CockpitG10N1.tmpstr = "zThrottle" + (j + 1);
            this.mesh.chunkSetAngles(CockpitG10N1.tmpstr, 0.0F, -49F * this.interp(this.setNew.throttle[j], this.setOld.throttle[j], f), 0.0F);
        }
        ...
        if ((this.fm.AS.astateCockpitState & 0x40) == 0) {
            this.mesh.chunkSetAngles("Z_Climb1", 0.0F, this.floatindex(this.cvt(this.setNew.vspeed, -30.48F, 30.48F, 0.0F, 12F), CockpitG10N1.variometerScale), 0.0F);
            this.mesh.chunkSetAngles("Z_Climb2", 0.0F, this.floatindex(this.cvt(this.setNew.vspeed, -30.48F, 30.48F, 0.0F, 12F), CockpitG10N1.variometerScale), 0.0F);
        }
        ...
        this.mesh.chunkSetAngles("zSpeed", 0.0F, this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, super.fm.getSpeedKMH()), 0.0F, 836.859F, 0.0F, 13F), CockpitG10N1.speedometerScale), 0.0F);
        this.mesh.chunkSetAngles("zSpeed2", 0.0F, this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, super.fm.getSpeedKMH()), 0.0F, 836.859F, 0.0F, 13F), CockpitG10N1.speedometerScale), 0.0F);
        ...
        if ((this.fm.AS.astateCockpitState & 0x40) == 0) {
            for (int k = 0; k < 6; k++) {
                CockpitG10N1.tmpstr = "zRPM" + (k + 1);
                this.mesh.chunkSetAngles(CockpitG10N1.tmpstr, 0.0F, this.cvt(this.fm.EI.engines[k].getRPM(), 0.0F, 4500F, 0.0F, 323F), 0.0F);
                CockpitG10N1.tmpstr = "Z_Pres" + (k + 1);
                this.mesh.chunkSetAngles(CockpitG10N1.tmpstr, 0.0F, this.pictManf[k] = (0.9F * this.pictManf[k]) + (0.1F * this.cvt(this.fm.EI.engines[k].getManifoldPressure(), 0.3386378F, 2.539784F, 0.0F, 346.5F)), 0.0F);
            }
        }
    ...
    protected boolean doFocusEnter() {
        if (super.doFocusEnter()) {
            this.aircraft().hierMesh().chunkVisible("nose", false);
            this.aircraft().hierMesh().chunkVisible("nose1", false);
            this.aircraft().hierMesh().chunkVisible("nose2", false);
            return true;
        } else {
            return false;
        }
    ...
    protected void doFocusLeave() {
        if (!this.isFocused()) {
            ...
            this.aircraft().hierMesh().chunkVisible("nose", this.aircraft().isChunkAnyDamageVisible("CF_D"));
            this.aircraft().hierMesh().chunkVisible("nose1", this.aircraft().isChunkAnyDamageVisible("CF_D"));
            this.aircraft().hierMesh().chunkVisible("nose2", this.aircraft().isChunkAnyDamageVisible("CF_D"));
            super.doFocusLeave();
            return;
        }
    }
