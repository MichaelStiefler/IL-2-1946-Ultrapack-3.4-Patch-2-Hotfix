...
public class CockpitG50 extends CockpitPilot {
    class Interpolater extends InterpolateRef {
        ...
        public boolean tick() {
            CockpitG50.this.setTmp = CockpitG50.this.setOld;
            CockpitG50.this.setOld = CockpitG50.this.setNew;
            CockpitG50.this.setNew = CockpitG50.this.setTmp;
            if (((CockpitG50.this.fm.AS.astateCockpitState & 2) != 0) && (CockpitG50.this.setNew.stbyPosition < 1.0F)) {
                CockpitG50.this.delay--;
                if (CockpitG50.this.delay <= 0) {
                    ...
                    CockpitG50.this.sightDamaged = true;
                }
            }
            CockpitG50.this.setNew.throttle = ((10F * CockpitG50.this.setOld.throttle) + CockpitG50.this.fm.CT.PowerControl) / 11F;
            CockpitG50.this.setNew.prop = ((10F * CockpitG50.this.setOld.prop) + CockpitG50.this.fm.EI.engines[0].getControlProp()) / 11F;
            CockpitG50.this.setNew.mix = ((10F * CockpitG50.this.setOld.mix) + CockpitG50.this.fm.EI.engines[0].getControlMix()) / 11F;
            CockpitG50.this.setNew.vspeed = ((199F * CockpitG50.this.setOld.vspeed) + CockpitG50.this.fm.getVertSpeed()) / 200F;
            CockpitG50.this.setNew.waypointAzimuth.setDeg(CockpitG50.this.setOld.waypointAzimuth.getDeg(0.1F), (CockpitG50.this.waypointAzimuth() - CockpitG50.this.setOld.azimuth.getDeg(1.0F)) + World.Rnd().nextFloat(-10F, 10F));
            if (Math.abs(CockpitG50.this.fm.Or.getKren()) < 30F) {
                CockpitG50.this.setNew.azimuth.setDeg(CockpitG50.this.setOld.azimuth.getDeg(1.0F), CockpitG50.this.fm.Or.azimut());
            }
            return true;
        }
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        this.mesh.chunkSetAngles("Z_Speedometer1", -this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeedKMH()), 0.0F, 600F, 0.0F, 12F), CockpitG50.speedometerScale), 0.0F, 0.0F);
        this.mesh.chunkSetAngles("Z_Speedometer2", -this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeedKMH()), 0.0F, 600F, 0.0F, 12F), CockpitG50.speedometerScale), 0.0F, 0.0F);
        ...
        if (this.aircraft().isFMTrackMirror()) {
            ...
            f1 = this.cvt((this.setNew.azimuth.getDeg(f) - this.setOld.azimuth.getDeg(f)) / Time.tickLenFs(), -9F, 9F, -24F, 24F);
            if (this.aircraft().fmTrack() != null) {
                this.aircraft().fmTrack().setCockpitAzimuthSpeed(f1);
            }
        }
        this.mesh.chunkSetAngles("Z_TurnBank2", f1, 0.0F, 0.0F);
        ...
        this.mesh.chunkSetAngles("Z_Prescockp1", -this.floatindex(this.cvt(this.fm.EI.engines[0].getManifoldPressure(), 0.533288F, 1.33322F, 0.0F, 6F), CockpitG50.manifoldScale), 0.0F, 0.0F);
