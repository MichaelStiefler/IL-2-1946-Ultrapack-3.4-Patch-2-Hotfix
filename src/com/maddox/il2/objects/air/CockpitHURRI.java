...
public class CockpitHURRI extends CockpitPilot {
    class Interpolater extends InterpolateRef {

        public boolean tick() {
            if (CockpitHURRI.this.bNeedSetUp) {
                CockpitHURRI.this.reflectPlaneMats();
                CockpitHURRI.this.bNeedSetUp = false;
            }
            if (CockpitHURRI.this.fm != null) {
                CockpitHURRI.this.setTmp = CockpitHURRI.this.setOld;
                CockpitHURRI.this.setOld = CockpitHURRI.this.setNew;
                CockpitHURRI.this.setNew = CockpitHURRI.this.setTmp;
                CockpitHURRI.this.setNew.throttle = ((10F * CockpitHURRI.this.setOld.throttle) + CockpitHURRI.this.fm.CT.PowerControl) / 11F;
                CockpitHURRI.this.setNew.altimeter = CockpitHURRI.this.fm.getAltitude();
                CockpitHURRI.this.setNew.waypointAzimuth.setDeg(CockpitHURRI.this.setOld.waypointAzimuth.getDeg(0.1F), (CockpitHURRI.this.waypointAzimuth() - CockpitHURRI.this.setOld.azimuth.getDeg(1.0F)) + World.Rnd().nextFloat(-10F, 10F));
                if (Math.abs(CockpitHURRI.this.fm.Or.getKren()) < 30F) {
                    CockpitHURRI.this.setNew.azimuth.setDeg(CockpitHURRI.this.setOld.azimuth.getDeg(1.0F), CockpitHURRI.this.fm.Or.azimut());
                }
                CockpitHURRI.this.setNew.vspeed = ((199F * CockpitHURRI.this.setOld.vspeed) + CockpitHURRI.this.fm.getVertSpeed()) / 200F;
            }
            return true;
        }
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        this.mesh.chunkSetAngles("STRELKA_RPM", 0.0F, 0.0F, -this.floatindex(this.cvt(this.fm.EI.engines[0].getRPM(), 1000F, 5000F, 2.0F, 10F), CockpitHURRI.rpmScale));
        this.mesh.chunkSetAngles("STRELK_TEMP_OIL", 0.0F, 0.0F, -this.cvt(this.fm.EI.engines[0].tOilOut, 40F, 100F, 0.0F, 270F));
        this.mesh.chunkSetAngles("STRELK_TEMP_RAD", 0.0F, 0.0F, -this.floatindex(this.cvt(this.fm.EI.engines[0].tWaterOut, 0.0F, 140F, 0.0F, 14F), CockpitHURRI.radScale));
        this.resetYPRmodifier();
        ...
        if (this.bFAF) {
            this.mesh.chunkSetAngles("STRELK_V_LONG", 0.0F, 0.0F, -this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeed()), 0.0F, 143.0528F, 0.0F, 32F), CockpitHURRI.speedometerScaleFAF));
        } else {
            this.mesh.chunkSetAngles("STRELK_V_LONG", 0.0F, 0.0F, -this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeed()), 0.0F, 180.0555F, 0.0F, 35F), CockpitHURRI.speedometerScale));
        }
        this.mesh.chunkSetAngles("STRELKA_VY", 0.0F, 0.0F, -this.floatindex(this.cvt(this.setNew.vspeed, -20.32F, 20.32F, 0.0F, 8F), CockpitHURRI.variometerScale));
