...
public class CockpitHURRII extends CockpitPilot {
    class Interpolater extends InterpolateRef {
        ...
        public boolean tick() {
            if (CockpitHURRII.this.bNeedSetUp) {
                ...
                CockpitHURRII.this.bNeedSetUp = false;
            }
            if (CockpitHURRII.this.fm != null) {
                CockpitHURRII.this.setTmp = CockpitHURRII.this.setOld;
                CockpitHURRII.this.setOld = CockpitHURRII.this.setNew;
                CockpitHURRII.this.setNew = CockpitHURRII.this.setTmp;
                CockpitHURRII.this.setNew.throttle = ((10F * CockpitHURRII.this.setOld.throttle) + CockpitHURRII.this.fm.CT.PowerControl) / 11F;
                CockpitHURRII.this.setNew.altimeter = CockpitHURRII.this.fm.getAltitude();
                if (Math.abs(CockpitHURRII.this.fm.Or.getKren()) < 30F) {
                    CockpitHURRII.this.setNew.waypointAzimuth.setDeg(CockpitHURRII.this.setOld.waypointAzimuth.getDeg(0.1F), (CockpitHURRII.this.waypointAzimuth() - CockpitHURRII.this.setOld.azimuth.getDeg(1.0F)) + World.Rnd().nextFloat(-10F, 10F));
                }
                if (Math.abs(CockpitHURRII.this.fm.Or.getKren()) < 30F) {
                    CockpitHURRII.this.setNew.azimuth.setDeg(CockpitHURRII.this.setOld.azimuth.getDeg(1.0F), CockpitHURRII.this.fm.Or.azimut());
                }
                CockpitHURRII.this.setNew.vspeed = ((199F * CockpitHURRII.this.setOld.vspeed) + CockpitHURRII.this.fm.getVertSpeed()) / 200F;
            }
            return true;
        }
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        this.mesh.chunkSetAngles("STRELKA_RPM", 0.0F, 0.0F, -this.floatindex(this.cvt(this.fm.EI.engines[0].getRPM(), 1000F, 5000F, 2.0F, 10F), CockpitHURRII.rpmScale));
        this.mesh.chunkSetAngles("STRELK_TEMP_OIL", 0.0F, 0.0F, -this.cvt(this.fm.EI.engines[0].tOilOut, 40F, 100F, 0.0F, 270F));
        this.mesh.chunkSetAngles("STRELK_TEMP_RAD", 0.0F, 0.0F, -this.floatindex(this.cvt(this.fm.EI.engines[0].tWaterOut, 0.0F, 140F, 0.0F, 14F), CockpitHURRII.radScale));
        this.resetYPRmodifier();
        ...
        if (this.bFAF) {
            this.mesh.chunkSetAngles("STRELK_V_LONG", 0.0F, 0.0F, -this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeed()), 0.0F, 143.0528F, 0.0F, 32F), CockpitHURRII.speedometerScaleFAF));
        } else {
            this.mesh.chunkSetAngles("STRELK_V_LONG", 0.0F, 0.0F, -this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeed()), 0.0F, 180.0555F, 0.0F, 35F), CockpitHURRII.speedometerScale));
        }
        this.mesh.chunkSetAngles("STRELKA_VY", 0.0F, 0.0F, -this.floatindex(this.cvt(this.setNew.vspeed, -20.32F, 20.32F, 0.0F, 8F), CockpitHURRII.variometerScale));
