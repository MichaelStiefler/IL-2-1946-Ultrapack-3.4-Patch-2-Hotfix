...
public class CockpitMig_17Radar extends CockpitPilot {
    class Interpolater extends InterpolateRef {
        ...
        public boolean tick() {
            if (CockpitMig_17Radar.this.fm != null) {
                long t = Time.current();
                long tB = t / CockpitMig_17Radar.DIVIDER;
            ...
            if (Actor.isValid(this.aircraft()) && Actor.isAlive(this.aircraft())) {
                ...
                if (curTime != this.to) {
                    ...
                    for (int targetsIndex = 0; targetsIndex < targets.size(); targetsIndex++) {
                        ...
                        if ((pointOrtho.x > CockpitMig_17Radar.RANGE_CLOSE) && (pointOrtho.x < CockpitMig_17Radar.RADAR_RANGE)) {
                ...
                for (int radarPlaneIndex = 0; radarPlaneIndex < this.radarPlane.size(); radarPlaneIndex++) {
                    ...
                    if ((x <= CockpitMig_17Radar.RANGE_CLOSE) || (targetNumber >= CockpitMig_17Radar.MAX_TARGETS)) {
                        continue;
                    }
                    this.FOV = 26D / x;
                    ...
                    double NewY = ((Point3d) this.radarPlane.get(radarPlaneIndex)).x / CockpitMig_17Radar.RADAR_RANGE;
                    if ((NewZ > CockpitMig_17Radar.ELEVATION_MAX_POSITIVE) || (NewZ < -CockpitMig_17Radar.ELEVATION_MAX_NEGATIVE)) {
                        continue;
                    }
                    float xCoord = (float) (NewX * CockpitMig_17Radar.SCREEN_X);
                    float yCoord = CockpitMig_17Radar.BASE_COORD_Y + (float) (NewY * CockpitMig_17Radar.SCREEN_Y);
                ...
                for (int invisibleSignalIndex = targetNumber + 1; invisibleSignalIndex <= CockpitMig_17Radar.MAX_TARGETS; invisibleSignalIndex++) {
    ...
    private static final int   DIVIDER                = CockpitMig_17Radar.REFRESH / CockpitMig_17Radar.STEPS;
