...
public class CockpitSpit1 extends CockpitPilot {
    class Interpolater extends InterpolateRef {
        ...
        public boolean tick() {
            if (CockpitSpit1.this.fm != null) {
                CockpitSpit1.this.setTmp = CockpitSpit1.this.setOld;
                CockpitSpit1.this.setOld = CockpitSpit1.this.setNew;
                CockpitSpit1.this.setNew = CockpitSpit1.this.setTmp;
                CockpitSpit1.this.setNew.throttle = (0.92F * CockpitSpit1.this.setOld.throttle) + (0.08F * CockpitSpit1.this.fm.CT.PowerControl);
                CockpitSpit1.this.setNew.prop = (0.92F * CockpitSpit1.this.setOld.prop) + (0.08F * CockpitSpit1.this.fm.EI.engines[0].getControlProp());
                CockpitSpit1.this.setNew.mix = (0.92F * CockpitSpit1.this.setOld.mix) + (0.08F * CockpitSpit1.this.fm.EI.engines[0].getControlMix());
                CockpitSpit1.this.setNew.altimeter = CockpitSpit1.this.fm.getAltitude();
                CockpitSpit1.this.setNew.waypointAzimuth.setDeg(CockpitSpit1.this.setOld.waypointAzimuth.getDeg(0.1F), (CockpitSpit1.this.waypointAzimuth() - CockpitSpit1.this.setOld.azimuth.getDeg(1.0F)) + World.Rnd().nextFloat(-10F, 10F));
                if (Math.abs(CockpitSpit1.this.fm.Or.getKren()) < 30F) {
                    CockpitSpit1.this.setNew.azimuth.setDeg(CockpitSpit1.this.setOld.azimuth.getDeg(1.0F), CockpitSpit1.this.fm.Or.azimut());
                }
                CockpitSpit1.this.setNew.vspeed = (0.99F * CockpitSpit1.this.setOld.vspeed) + (0.01F * CockpitSpit1.this.fm.getVertSpeed());
            }
            return true;
        }
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        this.mesh.chunkSetAngles("STRELKA_RPM", 0.0F, 0.0F, -this.floatindex(this.cvt(this.fm.EI.engines[0].getRPM(), 1000F, 5000F, 2.0F, 10F), CockpitSpit1.rpmScale));
        this.mesh.chunkSetAngles("STRELK_TEMP_OIL", 0.0F, 0.0F, -this.cvt(this.fm.EI.engines[0].tOilOut, 50F, 100F, 0.0F, 271F));
        this.mesh.chunkSetAngles("STRELK_TEMP_RAD", 0.0F, 0.0F, -this.floatindex(this.cvt(this.fm.EI.engines[0].tWaterOut, 0.0F, 140F, 0.0F, 14F), CockpitSpit1.radScale));
        ...
        this.mesh.chunkSetAngles("STRELK_V_LONG", 0.0F, 0.0F, -this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeed()), 0.0F, 223.52F, 0.0F, 25F), CockpitSpit1.speedometerScale));
        this.mesh.chunkSetAngles("STRELKA_VY", 0.0F, 0.0F, -this.floatindex(this.cvt(this.setNew.vspeed, -20.32F, 20.32F, 0.0F, 8F), CockpitSpit1.variometerScale));
