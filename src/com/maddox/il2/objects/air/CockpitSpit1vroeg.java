...
public class CockpitSpit1vroeg extends CockpitPilot {
    class Interpolater extends InterpolateRef {

        public boolean tick() {
            if (CockpitSpit1vroeg.this.fm != null) {
                CockpitSpit1vroeg.this.setTmp = CockpitSpit1vroeg.this.setOld;
                CockpitSpit1vroeg.this.setOld = CockpitSpit1vroeg.this.setNew;
                CockpitSpit1vroeg.this.setNew = CockpitSpit1vroeg.this.setTmp;
                CockpitSpit1vroeg.this.setNew.throttle = (0.92F * CockpitSpit1vroeg.this.setOld.throttle) + (0.08F * CockpitSpit1vroeg.this.fm.CT.PowerControl);
                CockpitSpit1vroeg.this.setNew.prop = (0.92F * CockpitSpit1vroeg.this.setOld.prop) + (0.08F * CockpitSpit1vroeg.this.fm.EI.engines[0].getControlProp());
                CockpitSpit1vroeg.this.setNew.mix = (0.92F * CockpitSpit1vroeg.this.setOld.mix) + (0.08F * CockpitSpit1vroeg.this.fm.EI.engines[0].getControlMix());
                CockpitSpit1vroeg.this.setNew.altimeter = CockpitSpit1vroeg.this.fm.getAltitude();
                CockpitSpit1vroeg.this.setNew.waypointAzimuth.setDeg(CockpitSpit1vroeg.this.setOld.waypointAzimuth.getDeg(0.1F), (CockpitSpit1vroeg.this.waypointAzimuth() - CockpitSpit1vroeg.this.setOld.azimuth.getDeg(1.0F)) + World.Rnd().nextFloat(-10F, 10F));
                if (Math.abs(CockpitSpit1vroeg.this.fm.Or.getKren()) < 30F) {
                    CockpitSpit1vroeg.this.setNew.azimuth.setDeg(CockpitSpit1vroeg.this.setOld.azimuth.getDeg(1.0F), CockpitSpit1vroeg.this.fm.Or.azimut());
                }
                CockpitSpit1vroeg.this.setNew.vspeed = (0.99F * CockpitSpit1vroeg.this.setOld.vspeed) + (0.01F * CockpitSpit1vroeg.this.fm.getVertSpeed());
            }
            return true;
        }
    ...
    public CockpitSpit1vroeg() {
        super("3DO/Cockpit/SpitfireMkI/CockpitSpit1vroeg.him", "bf109");
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        this.mesh.chunkSetAngles("STRELKA_RPM", 0.0F, 0.0F, -this.floatindex(this.cvt(this.fm.EI.engines[0].getRPM(), 1000F, 5000F, 2.0F, 10F), CockpitSpit1vroeg.rpmScale));
        this.mesh.chunkSetAngles("STRELK_TEMP_OIL", 0.0F, 0.0F, -this.cvt(this.fm.EI.engines[0].tOilOut, 50F, 100F, 0.0F, 271F));
        this.mesh.chunkSetAngles("STRELK_TEMP_RAD", 0.0F, 0.0F, -this.floatindex(this.cvt(this.fm.EI.engines[0].tWaterOut, 0.0F, 140F, 0.0F, 14F), CockpitSpit1vroeg.radScale));
        ...
        this.mesh.chunkSetAngles("STRELK_V_LONG", 0.0F, 0.0F, -this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeed()), 0.0F, 223.52F, 0.0F, 25F), CockpitSpit1vroeg.speedometerScale));
        this.mesh.chunkSetAngles("STRELKA_VY", 0.0F, 0.0F, -this.floatindex(this.cvt(this.setNew.vspeed, -20.32F, 20.32F, 0.0F, 8F), CockpitSpit1vroeg.variometerScale));
