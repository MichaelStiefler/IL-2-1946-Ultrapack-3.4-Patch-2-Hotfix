...
public class CockpitSpit9C extends CockpitPilot {
    class Interpolater extends InterpolateRef {

        public boolean tick() {
            if (CockpitSpit9C.this.bNeedSetUp) {
                CockpitSpit9C.this.reflectPlaneMats();
                CockpitSpit9C.this.bNeedSetUp = false;
            }
            if (CockpitSpit9C.this.fm != null) {
                CockpitSpit9C.this.setTmp = CockpitSpit9C.this.setOld;
                CockpitSpit9C.this.setOld = CockpitSpit9C.this.setNew;
                CockpitSpit9C.this.setNew = CockpitSpit9C.this.setTmp;
                CockpitSpit9C.this.setNew.throttle = (0.92F * CockpitSpit9C.this.setOld.throttle) + (0.08F * CockpitSpit9C.this.fm.CT.PowerControl);
                CockpitSpit9C.this.setNew.prop = (0.92F * CockpitSpit9C.this.setOld.prop) + (0.08F * CockpitSpit9C.this.fm.EI.engines[0].getControlProp());
                CockpitSpit9C.this.setNew.mix = (0.92F * CockpitSpit9C.this.setOld.mix) + (0.08F * CockpitSpit9C.this.fm.EI.engines[0].getControlMix());
                CockpitSpit9C.this.setNew.altimeter = CockpitSpit9C.this.fm.getAltitude();
                CockpitSpit9C.this.setNew.waypointAzimuth.setDeg(CockpitSpit9C.this.setOld.waypointAzimuth.getDeg(0.1F), (CockpitSpit9C.this.waypointAzimuth() - CockpitSpit9C.this.setOld.azimuth.getDeg(1.0F)) + World.Rnd().nextFloat(-10F, 10F));
                if (Math.abs(CockpitSpit9C.this.fm.Or.getKren()) < 30F) {
                    CockpitSpit9C.this.setNew.azimuth.setDeg(CockpitSpit9C.this.setOld.azimuth.getDeg(1.0F), CockpitSpit9C.this.fm.Or.azimut());
                }
                CockpitSpit9C.this.setNew.vspeed = (0.99F * CockpitSpit9C.this.setOld.vspeed) + (0.01F * CockpitSpit9C.this.fm.getVertSpeed());
            }
            return true;
        }
    ...
    public CockpitSpit9C() {
        super("3DO/Cockpit/SpitfireMkIXc/hier.him", "bf109");
        ...
        if (this.aircraft() instanceof SPITFIRE9M61) {
            this.mesh.materialReplace("BORT2", "BORT2a");
            this.mesh.materialReplace("text14", "text14a");
        } else if (this.aircraft() instanceof SPITFIRE9M63) {
            this.mesh.materialReplace("BORT2", "BORT2b");
            this.mesh.materialReplace("text14", "text14b");
        } else if ((this.aircraft() instanceof SPITFIRE925LBS) || (this.aircraft() instanceof SPITFIRE925LBSCW)) {
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        this.mesh.chunkSetAngles("STRELKA_RPM", 0.0F, 0.0F, -this.floatindex(this.cvt(this.fm.EI.engines[0].getRPM(), 1000F, 5000F, 2.0F, 10F), CockpitSpit9C.rpmScale));
        this.mesh.chunkSetAngles("STRELK_TEMP_OIL", 0.0F, 0.0F, -this.cvt(this.fm.EI.engines[0].tOilOut, 50F, 100F, 0.0F, 271F));
        this.mesh.chunkSetAngles("STRELK_TEMP_RAD", 0.0F, 0.0F, -this.floatindex(this.cvt(this.fm.EI.engines[0].tWaterOut, 0.0F, 140F, 0.0F, 14F), CockpitSpit9C.radScale));
        ...
        this.mesh.chunkSetAngles("STRELK_V_LONG", 0.0F, 0.0F, -this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeed()), 0.0F, 223.52F, 0.0F, 25F), CockpitSpit9C.speedometerScale));
        this.mesh.chunkSetAngles("STRELKA_VY", 0.0F, 0.0F, -this.floatindex(this.cvt(this.setNew.vspeed, -20.32F, 20.32F, 0.0F, 8F), CockpitSpit9C.variometerScale));
