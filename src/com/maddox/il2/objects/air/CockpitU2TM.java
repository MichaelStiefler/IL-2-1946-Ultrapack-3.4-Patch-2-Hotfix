...
public class CockpitU2TM extends CockpitPilot {
    ...
    class Interpolater extends InterpolateRef {
        ...
        public boolean tick() {
            if (CockpitU2TM.this.fm != null) {
                if (CockpitU2TM.this.bNeedSetUp) {
                    CockpitU2TM.this.reflectPlaneMats();
                    CockpitU2TM.this.bNeedSetUp = false;
                }
                CockpitU2TM.this.setTmp = CockpitU2TM.this.setOld;
                CockpitU2TM.this.setOld = CockpitU2TM.this.setNew;
                CockpitU2TM.this.setNew = CockpitU2TM.this.setTmp;
                CockpitU2TM.this.setNew.throttle = ((10F * CockpitU2TM.this.setOld.throttle) + CockpitU2TM.this.fm.CT.PowerControl) / 11F;
                CockpitU2TM.this.setNew.prop = ((10F * CockpitU2TM.this.setOld.prop) + CockpitU2TM.this.fm.EI.engines[0].getControlProp()) / 11F;
                CockpitU2TM.this.setNew.altimeter = CockpitU2TM.this.fm.getAltitude();
                CockpitU2TM.this.setNew.waypointAzimuth.setDeg(CockpitU2TM.this.setOld.waypointAzimuth.getDeg(0.1F), (CockpitU2TM.this.waypointAzimuth() - CockpitU2TM.this.setOld.azimuth.getDeg(1.0F)) + World.Rnd().nextFloat(-10F, 10F));
                if (Math.abs(CockpitU2TM.this.fm.Or.getKren()) < 30F) {
                    CockpitU2TM.this.setNew.azimuth.setDeg(CockpitU2TM.this.setOld.azimuth.getDeg(1.0F), CockpitU2TM.this.fm.Or.azimut());
                }
                CockpitU2TM.this.setNew.vspeed = ((199F * CockpitU2TM.this.setOld.vspeed) + CockpitU2TM.this.fm.getVertSpeed()) / 200F;
                boolean flag = false;
                if (CockpitU2TM.this.setNew.gCrankAngle < (CockpitU2TM.this.fm.CT.getGear() - 0.005F)) {
                    if (Math.abs(CockpitU2TM.this.setNew.gCrankAngle - CockpitU2TM.this.fm.CT.getGear()) < 0.33F) {
                        CockpitU2TM.this.setNew.gCrankAngle += 0.0025F;
                        flag = true;
                    } else {
                        CockpitU2TM.this.setNew.gCrankAngle = CockpitU2TM.this.fm.CT.getGear();
                        CockpitU2TM.this.setOld.gCrankAngle = CockpitU2TM.this.fm.CT.getGear();
                    }
                }
                if (CockpitU2TM.this.setNew.gCrankAngle > (CockpitU2TM.this.fm.CT.getGear() + 0.005F)) {
                    if (Math.abs(CockpitU2TM.this.setNew.gCrankAngle - CockpitU2TM.this.fm.CT.getGear()) < 0.33F) {
                ...
                if (flag != this.sfxPlaying) {
                    if (flag) {
                        CockpitU2TM.this.sfxStart(16);
                    } else {
                        CockpitU2TM.this.sfxStop(16);
                    }
    ...
    public CockpitU2TM() {
        super("3DO/Cockpit/U-2TM/CockpitU2TM.him", "i16");
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        this.mesh.chunkSetAngles("zGas1a", 0.0F, this.floatindex(this.cvt(this.fm.M.fuel, 0.0F, 190F, 0.0F, 14F), CockpitU2TM.fuelQuantityScale), 0.0F);
        this.mesh.chunkSetAngles("zSpeed1a", 0.0F, this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeedKMH()), 0.0F, 550F, 0.0F, 11F), CockpitU2TM.speedometerScale), 0.0F);
        ...
        if (((this.fm.AS.astateCockpitState & 8) == 0) && ((this.fm.AS.astateCockpitState & 0x20) == 0)) {
            this.mesh.chunkSetAngles("zRPS1a", 0.0F, this.floatindex(this.cvt(this.fm.EI.engines[0].getRPM(), 0.0F, 2400F, 0.0F, 13F), CockpitU2TM.engineRPMScale), 0.0F);
