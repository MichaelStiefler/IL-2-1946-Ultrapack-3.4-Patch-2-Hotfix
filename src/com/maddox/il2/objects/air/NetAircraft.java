...
//            System.out.println("netInput(" + netmsginput.available() + "), NetAircraft=" + this.hashCode() + " (" + this.netName + "), Tick: " + Time.tick());
            ...
            if (NetAircraft.this.isFMTrackMirror()) {
                ...
                // TODO: Storebror: --- Bomb Release Bug hunting
                int ctl = netmsginput.readUnsignedByte();
//                if (lastGearControl != (ctl & 0x01)) {
//                    lastGearControl = (ctl & 0x01);
////                    System.out.println("NetAircraft " + this.netName + " Mirror netInput() GearControl = " + lastGearControl);
//                }
                this.netControls(ctl);
...
//                Reflection.setFloat(NetAircraft.this.FM.CT, "Ailerons", f18);
            ...
            // TODO: By SAS~Storebror: Replicate Turret AI control state
            if (netmsginput.available() < 2) return true;
            short turretStates = (short)netmsginput.readUnsignedShort();
            if (NetAircraft.this.FM.turret != null && NetAircraft.this.FM.turret.length > 0) {
                for (int turretIndex = 0; turretIndex < NetAircraft.this.FM.turret.length && turretIndex < 16; turretIndex++) {
                    NetAircraft.this.FM.turret[turretIndex].bIsAIControlled = ((turretStates & (1 << turretIndex)) != 0);
                }
            }
            // ---
...
//			if (this instanceof TypeGuidedMissileCarrier) ((TypeGuidedMissileCarrier)this).getGuidedMissileUtils().setMissileLockState(i12);
            ...
            if (NetAircraft.this instanceof TypeNetAircraft2023) ((TypeNetAircraft2023)NetAircraft.this).netInputAircraft(netmsginput);
            // TODO: Storebror: --- New Generic Net Replication for Aircraft Classes
...
//                if (lastGearControl != (NetAircraft.this.FM.CT.CTL & 0x01)) {
//                    lastGearControl = NetAircraft.this.FM.CT.CTL & 0x01;
////                    System.out.println("NetAircraft " + this.netName + " Master netUpdate() GearControl = " + lastGearControl);
//                }
...
//				}
                ...
                // TODO: By SAS~Storebror: Replicate Turret AI control state
                short turretStates = 0;
                if (NetAircraft.this.FM.turret != null && NetAircraft.this.FM.turret.length > 0) {
                    for (int turretIndex = 0; turretIndex < NetAircraft.this.FM.turret.length && turretIndex < 16; turretIndex++) {
                        if (NetAircraft.this.FM.turret[turretIndex].bIsAIControlled) turretStates += 1 << turretIndex;
                    }
                }
                this.out.writeShort(turretStates & 0xFFFF);
                // ---

                // TODO: Storebror: +++ New Generic Net Replication for Aircraft Classes
                if (NetAircraft.this instanceof TypeNetAircraft2023) ((TypeNetAircraft2023)NetAircraft.this).netUpdateAircraft(this.out);
                // TODO: Storebror: --- New Generic Net Replication for Aircraft Classes
...
//        System.out.println("NetAircraft " + this.name() + " (" + this.getClass().getName() + ") getWeaponsAmount() weaponsLength=" + weaponsLength);
        if (this.FM.CT.Weapons.length == 0) return 0;
        int retVal = 0;
        for (int triggerIndex = 0; triggerIndex < weaponsLength; triggerIndex++) {
            BulletEmitter[] bulletemitters = this.FM.CT.Weapons[triggerIndex];
            if (bulletemitters != null) {
                for (int emitterIndex = 0; emitterIndex < bulletemitters.length; emitterIndex++) {
//                    System.out.println("getWeaponsAmount() this.FM.CT.Weapons[" + triggerIndex + "][" + emitterIndex + "] = " + (bulletemitters[emitterIndex]==null?"null":bulletemitters[emitterIndex].getClass().getName()));
                    if (bulletemitters[emitterIndex] != null) retVal++;
                }
            }
        }
//        System.out.println("NetAircraft " + this.name() + " (" + this.getClass().getName() + ") getWeaponsAmount()=" + retVal);
        return retVal;
    }

    private boolean isWeaponsAllEmpty() {
        int weaponsLength = this.FM.CT.Weapons.length;
//        System.out.println("NetAircraft " + this.name() + " (" + this.getClass().getName() + ") getWeaponsAmount() isWeaponsAllEmpty=" + weaponsLength);
        ...
        for (int weaponsIndex = 0; weaponsIndex < weaponsLength; weaponsIndex++) {
            ...
            if (bulletemitters != null) for (int bulletEmitterIndex = 0; bulletEmitterIndex < bulletemitters.length; bulletEmitterIndex++)
                if (bulletemitters[bulletEmitterIndex] != null && bulletemitters[bulletEmitterIndex].countBullets() != 0) return false;
        }
        return true;
    }

    private byte[] getWeaponsBitStatesBuf(byte[] is) {
        int weaponsAmount = this.getWeaponsAmount();
//        System.out.println("NetAircraft " + this.name() + " (" + this.getClass().getName() + ") getWeaponsBitStatesBuf([]) weaponsAmount=" + weaponsAmount);
        if (weaponsAmount == 0) return null;
        int weaponsAmountBytes = (weaponsAmount + 7) / 8;
//        System.out.println("NetAircraft " + this.name() + " (" + this.getClass().getName() + ") getWeaponsBitStatesBuf([]) weaponsAmountBytes=" + weaponsAmountBytes + ", is " + (is==null?"is null":"length is " + is.length));
        if (is == null || is.length != weaponsAmountBytes) is = new byte[weaponsAmountBytes];
        for (int weaponsAmountIndex = 0; weaponsAmountIndex < weaponsAmountBytes; weaponsAmountIndex++)
            is[weaponsAmountIndex] = (byte) 0;
//        System.out.println("NetAircraft " + this.name() + " (" + this.getClass().getName() + ") getWeaponsBitStatesBuf([]) is length=" + is.length);
        return is;
    }

    private byte[] getWeaponsBitStates(byte[] is) {
        is = this.getWeaponsBitStatesBuf(is);
//        System.out.println("NetAircraft " + this.name() + " (" + this.getClass().getName() + ") getWeaponsBitStates([]) is " + (is==null?"is null":"length is " + is.length));
        if (is == null) return null;
        int weaponsAmountBitIndex = 0;
        int weaponsLength = this.FM.CT.Weapons.length;
//        System.out.println("NetAircraft " + this.name() + " (" + this.getClass().getName() + ") getWeaponsBitStates([]) weaponsLength=" + weaponsLength);
        for (int weaponsIndex = 0; weaponsIndex < weaponsLength; weaponsIndex++) {
            BulletEmitter[] bulletemitters = this.FM.CT.Weapons[weaponsIndex];
            if (bulletemitters != null) for (int bulletEmitterIndex = 0; bulletEmitterIndex < bulletemitters.length; bulletEmitterIndex++)
                if (bulletemitters[bulletEmitterIndex] != null) {
                    if (bulletemitters[bulletEmitterIndex].countBullets() != 0) is[weaponsAmountBitIndex / 8] |= 1 << weaponsAmountBitIndex % 8;
                    weaponsAmountBitIndex++;
                }
        }
        return is;
    }
...
//        System.out.println("netSpawnCommon " + netaircraft.name() + " (" + netaircraft.getClass().getName() + ") is " + (is==null?"is null":"length is " + is.length));
        if (is != null) {
...
//        System.out.println("netReplicateFirstUpdate " + this.name() + " (" + this.getClass().getName() + ") is " + (is==null?"is null":"length is " + is.length));
        if (is != null) {
            // TODO: Added by SAS~Storebror +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            // In case of excessively long weapon slots (Fugaku anyone?), use deflated/inflated data instead.
            if (is.length > 80) {
