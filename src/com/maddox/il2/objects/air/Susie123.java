...
public abstract class Susie123 extends Scheme1 implements TypeStormovik, TypeDiveBomber {
    ...
    public void update(float f) {
        ...
        if (this.gunnerAiming) {
            if (this.gunnerAnimation < 1.0F) {
                this.gunnerAnimation += 0.025F;
                this.moveGunner();
            }
        } else if (this.gunnerAnimation > 0.0F) {
            this.gunnerAnimation -= 0.025F;
            this.moveGunner();
        }
        f1 = this.FM.CT.getArrestor();
        float f2 = 81F * (float) Math.pow(f1, 6);
        if (f1 > 0.01F) {
    ...
    private void moveGunner() {
        ...
        if (this.gunnerAnimation > 0.5D) {
            ...
            Aircraft.xyz[0] = 0.0F;
            Aircraft.xyz[1] = 0.0F;
            Aircraft.xyz[2] = this.gunnerAnimation * 0.0F;
            Aircraft.ypr[0] = 0.0F;
            Aircraft.ypr[1] = 0.0F;
            Aircraft.ypr[2] = this.gunnerAnimation * 0.0F;
    ...
    protected void hitBone(String s, Shot shot, Point3d point3d) {
        if (s.startsWith("xx")) {
            ...
            if (s.startsWith("xxtank") && (this.getEnergyPastArmor(0.1F, shot) > 0.0F) && (World.Rnd().nextFloat() < 0.99F)) {
            ...
            if (s.startsWith("xxmgun")) {
                if (s.endsWith("01")) {
                    Aircraft.debugprintln(this, "*** Wing Gun: Disabled..");
