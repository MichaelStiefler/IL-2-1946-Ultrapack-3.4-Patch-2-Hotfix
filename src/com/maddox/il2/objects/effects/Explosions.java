/*
 *  Explosions Hotfix by SAS~Storebror
 *  2016-05-30
 *  Fixing giant craters created by any explosion after big bombs
 */
...
//    private static boolean     ZUTI_VARIABLES_SET    = false;
    ...
    public static void HydrogenBalloonExplosion(Loc loc, Actor actor) {
        if (Config.isUSE_RENDER()) {
            ...
            for (int j = 0; j < k; j++) {
                vector3d.set(World.Rnd().nextFloat(-1.0F, 1.0F), World.Rnd().nextFloat(-1.0F, 1.0F), World.Rnd().nextFloat(-0.5F, 1.5F));
                vector3d.normalize();
                vector3d.scale(World.Rnd().nextFloat(4.0F, 15.0F));
    ...
    public static void runByName(String s, ActorHMesh actorhmesh, String s1, String s2, float f, Actor actor) {
        ...
        if (s1 != null && !s1.equals("")) {
            ...
            if (i >= 0) hooknamed = new HookNamed(actorhmesh, s1);
            else if (s2 != null && !s2.equals("")) {
                int j = actorhmesh.mesh().hookFind(s2);
                if (j >= 0) hooknamed = new HookNamed(actorhmesh, s2);
            }
        }
        if (s.equalsIgnoreCase("Tank")) Tank_Explode(actorhmesh.pos.getAbsPoint());
            ...
            if (f > 0.0F) {
                new MyMsgAction(0.43, actorhmesh, actor) {
                    public void doAction(Object obj) {
                        ...
                        int k = 0;
                        float f2 = 50.0F;
                        MsgExplosion.send((Actor) obj, "Body", point3d, (Actor) this.obj2, 0.0F, f1, k, f2);
                    }
                };
        ...
        } else if (s.equalsIgnoreCase("Aircraft")) Antiaircraft_Explode(actorhmesh.pos.getAbsPoint());
        else if (s.equalsIgnoreCase("Aircraft")) System.out.println("*** Unknown named explode: '" + s + "'");
            ...
            if (f > 0.0F) new MsgAction(1.2, new MydataForSmoke(actorhmesh, f)) {
                public void doAction(Object obj) {
                    Eff3DActor.New(((MydataForSmoke) obj).a, null, null, 1.0F, "Effects/Smokes/SmokeBlack_Locomotive.eff", ((MydataForSmoke) obj).tim);
                }
            };
    ...
    private static void ExplodeSurfaceWave(int i, float f, float f1) {
        if (i == 0) new ActorSnapToLand("3do/Effects/Explosion/DustRing.sim", true, l, 1.0F, f, 1.0F, 0.0F, f1);
        else if (i == 1) new ActorSnapToLand("3do/Effects/Explosion/WaterRing.sim", true, l, 0.2F, f, 1.0F, 0.0F, f1);
    }
    ...
    private static void SurfaceCrater(int surfaceType, float size, float ttl) {
        try {
            if (surfaceType == LAND) {
                ...
                // ----------------------------------------------------------------
                // TODO: Removed by SAS~Storebror: Absolutely unnecessary!
                ...
                if (bEnableActorCrater) {
                    ...
                    for (i = 64; i >= 2; i /= 2)
                        if (size >= i) break;
                    if (i >= 2) new ActorCrater("3do/Effects/Explosion/Crater" + i + "/Live.sim", l, ttl);
                }
    ...
    private static void fontain(Point3d point3d, float size, float ttl, int surfaceType, int weaponType, boolean flag) {
        ...
        switch (surfaceType) {
            ...
            case WATER:
                if (weaponType == 2) Eff3DActor.New(l, 1.0F, "effects/Explodes/Bomb1000/Water/Fontain.eff", size);
                else if (weaponType == 0) Eff3DActor.New(l, 1.0F, "effects/Explodes/Bomb250/Water/Fontain.eff", size);
                else Eff3DActor.New(l, 1.0F, "effects/Explodes/RS82/Water/Fontain.eff", size);
                o.set(0.0F, 0.0F, 0.0F);
                l.set(point3d, o);
                if (weaponType == 2) ExplodeSurfaceWave(surfaceType, 750.0F, 15.0F);
                else if (weaponType == 0) ExplodeSurfaceWave(surfaceType, 325.0F, 10.0F);
                else ExplodeSurfaceWave(surfaceType, 50.0F, 7.0F);
                break;
            case OBJECT:
    ...
    public static void Antiaircraft_Explode(Point3d point3d) {
        if (Config.isUSE_RENDER()) Tank_Explode(point3d);
    }
    ...
    public static void Car_ExplodeCollapse(Point3d point3d) {
        if (Config.isUSE_RENDER()) explodeCollapse(point3d);
    }
    ...
    public static void AirDrop_Land(Point3d point3d) {
        if (Config.isUSE_RENDER()) {
            float f = 4.0F;
            float f1 = 1.0F;
            if (Mission.isDeathmatch()) bEnableActorCrater = false;
...
//	        TODO: +++ Add Flash Effect to Flak Bursts +++
//			Eff3DActor.New(l, 1.0F, s + "Burn.eff", f);
//	        TODO: --- Add Flash Effect to Flak Bursts ---
            Eff3DActor.New(l, 1.0F, s + "Sparks.eff", f);
            Eff3DActor.New(l, 1.0F, s + "SparksP.eff", f);
//	        TODO: +++ Add Flash Effect to Flak Bursts +++
            if (World.Sun().ToSun.z > 0.1F) Eff3DActor.New(l, 1.0F, s + "Burn.eff", f);
            else { // Night time, create dynamic flash effect
                float burnSize = cvt(World.Sun().ToSun.z, -0.3F, 0.1F, 10F, 1F);
                float lightPointIntensity = 0.5F + World.Rnd().nextFloat();
                float lightPointRange = cvt(World.Sun().ToSun.z, -0.3F, 0.1F, 1000F, 100F);
                Eff3DActor eff3dactor = Eff3DActor.New(l, burnSize, s + "Burn_night.eff", f);
                LightPointActor lightpointactor = new LightPointActor(new LightPointWorld(), new Point3d());
...
//  TODO: --- Add Flash Effect to Flak Bursts ---
    ...
    public static void ExplodeFuel(Point3d point3d) {
        if (Config.isUSE_RENDER()) {
            ...
            double d = point3d.z;
            World.cur();
            if (d - World.land().HQ(point3d.x, point3d.y) > 3.0) SfxExplosion.crashAir(point3d, 1);
            else {
                World.cur();
                if (World.land().isWater(point3d.x, point3d.y)) SfxExplosion.crashAir(point3d, 2);
                else SfxExplosion.crashAir(point3d, 0);
            }
        }
    }
    ...
    public static void BOMB250_Land(Point3d point3d, float f, float f1, boolean flag) {
        if (Config.isUSE_RENDER()) // TODO: Modified by SAS~Storebror:
            ...
            // properties all across the class!
            fontain(point3d, ZutiSupportMethods_Effects.getCraterSize(250.0F), ZutiSupportMethods_Effects.getCraterTimeToLive(250.0F), 0, 0, flag);
//            fontain(point3d, f, f1, 0, 0, flag);
    }

    public static void BOMB250_Water(Point3d point3d, float f, float f1) {
        if (Config.isUSE_RENDER()) fontain(point3d, f, f1, 1, 0, false);
    }

    public static void BOMB250_Object(Point3d point3d, float f, float f1) {
        if (Config.isUSE_RENDER()) fontain(point3d, f, f1, 0, 0, false);
    }
    ...
    public static void BOMB1000a_Land(Point3d point3d, float f, float f1, boolean flag) {
        if (Config.isUSE_RENDER()) // TODO: Modified by SAS~Storebror:
            ...
            // instead of carrying some useless ZUTI_CRATER_SIZE, ZUTI_TIME_TO_LIVE and ZUTI_VARIABLES_SET
            // properties all across the class!
            fontain(point3d, ZutiSupportMethods_Effects.getCraterSize(250.0F), ZutiSupportMethods_Effects.getCraterTimeToLive(250.0F), 0, 2, flag);
//            fontain(point3d, f, f1, 0, 2, flag);
    }

    public static void BOMB1000a_Water(Point3d point3d, float f, float f1) {
        if (Config.isUSE_RENDER()) fontain(point3d, f, f1, 1, 2, false);
    }

    public static void BOMB1000a_Object(Point3d point3d, float f, float f1) {
        if (Config.isUSE_RENDER()) fontain(point3d, f, f1, 0, 2, false);
    }
...
//            // -----------------------------------------------------------------
            ...
            // TODO: Modified by SAS~Storebror: this is _NOT_ called by Mistel class anymore.
            // -----------------------------------------------------------------
            // TODO: Removed by SAS~Storebror: Absolutely unnecessary!
//            if (!Explosions.ZUTI_VARIABLES_SET) {
...
//            }
            ...
            // instead of carrying some useless ZUTI_CRATER_SIZE, ZUTI_TIME_TO_LIVE and ZUTI_VARIABLES_SET
            // properties all across the class!
            SurfaceCrater(0, ZutiSupportMethods_Effects.getCraterSize(1000.0F), ZutiSupportMethods_Effects.getCraterTimeToLive(1000.0F));
//            SurfaceCrater(0, 112.1F, 0.0F);
            // -----------------------------------------------------------------
...
//            }
            ...
            // instead of carrying some useless ZUTI_CRATER_SIZE, ZUTI_TIME_TO_LIVE and ZUTI_VARIABLES_SET
            // properties all across the class!
            SurfaceCrater(0, ZutiSupportMethods_Effects.getCraterSize(9999.0F), ZutiSupportMethods_Effects.getCraterTimeToLive(9999.0F));
//            SurfaceCrater(0, 312.1F, 0.0F);
    ...
    public static void RS82_Land(Point3d point3d, float f, float f1, boolean flag) {
        if (Config.isUSE_RENDER()) fontain(point3d, f, f1, 0, 1, flag);
    }

    public static void RS82_Water(Point3d point3d, float f, float f1) {
        if (Config.isUSE_RENDER()) fontain(point3d, f, f1, 1, 1, false);
    }

    public static void RS82_Object(Point3d point3d, float f, float f1) {
        if (Config.isUSE_RENDER()) fontain(point3d, f, f1, 0, 1, false);
    }
...
//        System.out.println("Explosions generateSound(actor=" + (actor == null ? "null" : actor.getClass().getName()) + ", pos=" + point3d.x + ":" + point3d.y + ":" + point3d.z + ", bombChargeMass=" + f + ", type=" + i + ", radius=" + f1 + ")" + ", isUSE_RENDER=" + Config.isUSE_RENDER() + ", isWater=" + Engine.land().isWater(point3d.x, point3d.y));
        if (Config.isUSE_RENDER()) {
            if (actor == null) {
...
//                System.out.println("Explosions generateSound at plane pos...");
//                SfxExplosion.shell(actor.pos.getAbsPoint(), 0, f, i, f1);
            }
        }
    }
    ...
    public static void generate(Actor actor, Point3d point3d, float bombChargeMas, int i, float f1, boolean flag, int isNuke) {
...
//        Explosions.ZUTI_VARIABLES_SET = true;
        ...
        if (Mission.isNet() && Main.cur().netServerParams.isMaster() && ZutiSupportMethods_Effects.isExplosionOnSpecialArea(point3d.x, point3d.y)) {
            ...
            if (isLand) if (ZutiSupportMethods_Effects.isSynced(bombChargeMas)) {
                ...
                // instead of carrying some useless ZUTI_CRATER_SIZE, ZUTI_TIME_TO_LIVE and ZUTI_VARIABLES_SET
                // properties all across the class!
                myCrater.expirationTime = Time.current() + ZutiSupportMethods_Effects.getCraterTimeToLive(bombChargeMas) * 1000L;
//                    myCrater.expirationTime = Time.current() + (Explosions.ZUTI_TIME_TO_LIVE * 1000);
                ...
                // instead of carrying some useless ZUTI_CRATER_SIZE, ZUTI_TIME_TO_LIVE and ZUTI_VARIABLES_SET
                // properties all across the class!
                myCrater.size = ZutiSupportMethods_Effects.getCraterSize(bombChargeMas);
...
//        // --------------------------------------------------
        ...
        if (Config.isUSE_RENDER()) if (actor != null) {
            if (isNuke == Bomb.NUKE_MISTEL)
                generateSound(actor, point3d, Float.MAX_VALUE, i, f1);
            else
                generateSound(actor, point3d, bombChargeMas, i, f1);
            rel.set(point3d);
            ...
            rel.sub(l);
            if (i == 2) {
                if (f1 < 3.0F) switch (rnd.nextInt(1, 2)) {
                    case 1:
                ...
                if (isNuke > Bomb.NUKE_DISABLED) {
                    if (isNuke == Bomb.NUKE_ENABLED) if (Engine.land().isWater(point3d.x, point3d.y)) bombFatMan_water(point3d, -1.0F, 1.0F);
                    else bombFatMan_land(point3d, -1.0F, 1.0F);
                } else if (bombChargeMas < 15.0F) {
                    if (Engine.land().isWater(point3d.x, point3d.y)) Explode10Kg_Water(point3d, 4.0F, 1.0F);
                    else Explode10Kg_Land(point3d, 4.0F, 1.0F);
                } else if (bombChargeMas < 50.0F) {
                    if (Engine.land().isWater(point3d.x, point3d.y)) RS82_Water(point3d, 4.0F, 1.0F);
                    else RS82_Land(point3d, 4.0F, 1.0F, flag);
                } else if (bombChargeMas < 450.0F) {
                    if (Engine.land().isWater(point3d.x, point3d.y)) BOMB250_Water(point3d, 4.0F, 1.0F);
                    else BOMB250_Land(point3d, 4.0F, 1.0F, flag);
                } else if (bombChargeMas < 3000.0F) {
                    if (Engine.land().isWater(point3d.x, point3d.y)) BOMB1000a_Water(point3d, 4.0F, 1.0F);
                    else BOMB1000a_Land(point3d, 4.0F, 1.0F, flag);
                } else if (Engine.land().isWater(point3d.x, point3d.y)) bomb1000_water(point3d, -1.0F, 1.0F);
                else bomb1000_land(point3d, -1.0F, 1.0F, flag);
            } else if (isNuke > Bomb.NUKE_DISABLED) {
                if (isNuke == Bomb.NUKE_ENABLED) {
                    if (point3d.z - Engine.land().HQ_Air(point3d.x, point3d.y) < 50.0) if (Engine.land().isWater(point3d.x, point3d.y)) bombFatMan_water(point3d, -1.0F, 1.0F);
                    else bombFatMan_land(point3d, -1.0F, 1.0F);
                    bomb50_land(point3d, -1.0F, 10.0F);
                }
            } else if (bombChargeMas < 50.0F) {
                if (point3d.z - Engine.land().HQ_Air(point3d.x, point3d.y) < 5.0) if (Engine.land().isWater(point3d.x, point3d.y)) RS82_Water(point3d, 4.0F, 1.0F);
                else RS82_Land(point3d, 4.0F, 1.0F, flag);
                bomb50_land(point3d, -1.0F, 1.0F);
            } else if (bombChargeMas < 450.0F) {
                if (point3d.z - Engine.land().HQ_Air(point3d.x, point3d.y) < 10.0) if (Engine.land().isWater(point3d.x, point3d.y)) BOMB250_Water(point3d, 4.0F, 1.0F);
                else BOMB250_Land(point3d, 4.0F, 1.0F, flag);
                bomb50_land(point3d, -1.0F, 2.0F);
            } else if (bombChargeMas < 3000.0F) {
                if (point3d.z - Engine.land().HQ_Air(point3d.x, point3d.y) < 20.0) if (Engine.land().isWater(point3d.x, point3d.y)) BOMB1000a_Water(point3d, 4.0F, 1.0F);
                else BOMB1000a_Land(point3d, 4.0F, 1.0F, flag);
                bomb50_land(point3d, -1.0F, 2.0F);
            } else {
                if (point3d.z - Engine.land().HQ_Air(point3d.x, point3d.y) < 50.0) if (Engine.land().isWater(point3d.x, point3d.y)) bomb1000_water(point3d, -1.0F, 1.0F);
                else bomb1000_land(point3d, -1.0F, 1.0F, flag);
                bomb50_land(point3d, -1.0F, 10.0F);
            }
        }
...
//        // -----------------------------------
    ...
    public static void generateShot(Actor actor, Shot shot1) {
        if (Config.isUSE_RENDER()) {
            ...
            rel.sub(l);
            if (World.cur().isArcade() && !(actor instanceof Aircraft)) Eff3DActor.New(actor, null, rel, 0.75F, "3DO/Effects/Fireworks/Sprite.eff", 30.0F);
            if (!(actor instanceof ActorLand)) switch (rnd.nextInt(1, 4)) {
                case 1:
            ...
            if (!(actor instanceof Aircraft)) switch (shot1.bodyMaterial) {
                case 0:
                    if (f < 1.0F) Cannon_Land(shot1.p, 4.0F, 1.0F);
                    else if (f < 5.0F) Explode10Kg_Land(shot1.p, 4.0F, 1.0F);
                    else if (f < 50.0F) RS82_Land(shot1.p, 4.0F, 1.0F, false);
                    else BOMB250_Land(shot1.p, 4.0F, 1.0F, false);
                    break;
                case 3:
                    break;
                case 1:
                    if (f < 0.023F) Bullet_Water(shot1.p, 0.5F, 1.0F);
                    else if (f < 0.701F) Cannon_Water(shot1.p, 4.0F, 1.0F);
                    else if (f < 8.55F) Explode10Kg_Water(shot1.p, 4.0F, 1.0F);
                    else if (f < 24.2F) RS82_Water(shot1.p, 4.0F, 1.0F);
                    else BOMB250_Water(shot1.p, 4.0F, 1.0F);
                    break;
                case 2:
    ...
    public static void generateExplosion(Actor actor, Point3d point3d, float f, int i, float f1, double d) {
        if (Config.isUSE_RENDER()) {
            ...
            if (actor != null) if (actor instanceof ActorLand) {
                boolean flag = Engine.land().isWater(point3d.x, point3d.y);
                if (f < 0.0010F) {
                    if (!flag) Eff3DActor.New(actor, null, rel, 1.0F, "3DO/Effects/Fireworks/12_Burn.eff", -1.0F);
                } else if (f < 0.0050F) {
                    if (!flag) Eff3DActor.New(rel, 1.0F, "3DO/Effects/Fireworks/20_SmokeBoiling.eff", -1.0F);
                } else if (f < 0.05F) {
                    if (flag) Explode10Kg_Water(point3d, 4.0F, 1.0F);
                    else Explode10Kg_Land(point3d, 4.0F, 1.0F);
                } else if (f < 1.0F) {
                    if (flag) RS82_Water(point3d, 4.0F, 1.0F);
                    else RS82_Land(point3d, 4.0F, 1.0F, false);
                } else if (f < 15.0F) {
                    if (flag) Explode10Kg_Water(point3d, 4.0F, 1.0F);
                    else Explode10Kg_Land(point3d, 4.0F, 1.0F);
                } else if (f < 50.0F) {
                ...
                } else if (flag) BOMB250_Water(point3d, 4.0F, 1.0F);
                else BOMB250_Land(point3d, 4.0F, 1.0F, false);
            } else if (f < 0.0010F) Eff3DActor.New(actor, null, rel, 1.0F, "3DO/Effects/Fireworks/12_Burn.eff", -1.0F);
            else if (f < 0.0030F) Eff3DActor.New(actor, null, rel, 1.0F, "3DO/Effects/Fireworks/12mmPluff.eff", 0.15F);
            else if (f < 0.0050F) {
    ...
    public static void generateComicBulb(Actor actor, String s, float f) {
        if (Config.isUSE_RENDER() && World.cur().isArcade()) Eff3DActor.New(actor, null, null, 1.0F, "3DO/Effects/Debug/msg" + s + ".eff", f);
    }
}
