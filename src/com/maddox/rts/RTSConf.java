...
public class RTSConf
{
    ...
    {
        if (!Config.isUSE_RENDER() && flag) { // TODO: By SAS~Storebror: Patch 2 Hotfix 18 - let us know what caused the game to terminate!
            Exception setRequestExitAppCheckException = new Exception("RTSConf.setRequestExitApp(true)");
            setRequestExitAppCheckException.printStackTrace();
            try {
                Thread.sleep(10L);
            } catch (Exception ex) {
                ex.printStackTrace();
